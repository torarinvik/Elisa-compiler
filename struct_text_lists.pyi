...
class TextBundle(TypedDict):
    names: list[str]
    labels: list[str]

class TextBundleInput(Protocol):
    names: Sequence[str | bytes]
    labels: Sequence[str | bytes]

def count(bundle: TextBundle | TextBundleInput) -> int: ...

def make(names: Sequence[str | bytes], labels: Sequence[str | bytes]) -> TextBundle: ...

__all__: list[str] = ["count", "make", "ElisaError", "TextBundle"]
