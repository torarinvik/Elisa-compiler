...
class NestedCatalog(TypedDict):
    values: dict[int, dict[str, Point]]

class NestedCatalogInput(Protocol):
    values: Mapping[int, Mapping[str | bytes, Point | PointInput]]

def roundtrip(values: Mapping[int, Mapping[str | bytes, Point | PointInput]]) -> dict[int, dict[str, Point]]: ...

def roundtrip_catalog(catalog: NestedCatalog | NestedCatalogInput) -> NestedCatalog: ...

def roundtrip_rows(values: Sequence[Mapping[int, Mapping[str | bytes, Point | PointInput]]]) -> list[dict[int, dict[str, Point]]]: ...

__all__: list[str] = ["roundtrip", "roundtrip_catalog", "roundtrip_rows", "ElisaError", "Point", "NestedCatalog"]
