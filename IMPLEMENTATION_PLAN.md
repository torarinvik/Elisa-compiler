...
**Inspection baseline:** compiler main `0bd938a9` with local uncommitted checker, runtime, and regression edits; Elisa-core main `c447c2ce` with its separate local zeroed-validity/runtime candidate. The detached proof-toolchain checkout remains at `4f3f7354` and is still used by the active proof build. This is a source-informed implementation plan, not a completed soundness audit. Targeted build and test evidence is recorded only in the dated verified slices below; the 2026-09-24 full gate run stopped at meaningful failures and is documented in §M.5. No full safety audit or sanitizer run is claimed. Historical green counts are historical evidence only. Every implementation item below starts pending unless explicitly described as existing infrastructure; a named checker is not evidence that its entire problem class is solved.

## Current gains review and revised priorities — 2026-09-25

**Review checkpoint (updated 2026-09-25):** compiler main `0bd938a9` and Core main `c447c2ce`, both with local uncommitted safety/runtime candidates. This snapshot reconciles the current candidate diffs, backend consumers, smoke cases, and prior results; it does not certify the current combined source tree. Earlier dated measurements retain their original compiler/source scope. No S01–S28 package is promoted to complete by this review.
...
The canonical Elisa-core checkout has an uncommitted companion zeroed-validity checker covering resolved references, aggregate fields, generic instances, arrays, tuples, enums, and type parameters. The last recorded `go test ./src/semantic` passes, but `go test ./...` fails in compiler/runtime integration tests. Reported failures include generic zeroed placeholders in `InlineVec[T]`, dictionary/set buckets, heap slots, and debug records; incomplete atomic-wrapper migration; `result_seed` initialization; and separate C-string bridge type mismatches. Runtime fixes have since been edited, but the full suite has not been rerun on the latest candidate. Keep its source intact; it is not ready to commit until placeholder idioms are soundly migrated and the broader tests/runtime smokes pass.

The compiler and Core runtime copies also carry an incomplete atomic-wrapper experiment: `atomic_cell[T]` constructs initialized `AtomicSlot[T]` and `AtomicCell[T]` values rather than a zeroed generic slot. The rename currently conflicts with compiler internals that recognize the exact nominal type name `atomic`: `check_raw_atomic_surface.elisa` only fences operations on `atomic[...]`, and `codegen_atomic.elisa` only selects LLVM atomic lowering when the resolved struct name is `atomic`. The Go compiler has the same exact-name dependency in type resolution, raw-operation validation, and `emitAtomicSlotPtr`. Until both compiler generations preserve intrinsic lowering and reject raw operations for the renamed slot, this migration can silently turn atomic accesses into ordinary loads/stores; it is a correctness blocker, not a cosmetic rename. There are also remaining `atomic[i64]` references in the runtime concurrency source. The Stage1 product is stale, and no current Stage0/Stage1 atomic-runtime build has validated the candidate.

The expanded zeroed-reference smoke now includes direct and generic reference returns, `sview` returns and fields, plus aggregate field/tuple/array cases. Its current product guard fails before tests because Stage1 is stale; a deliberate stale-product probe also showed that the old binary accepts the new direct-reference-return case, which is expected because that binary predates the source change. Treat all newly added cases as pending regression coverage until a fresh compiler runs them. The current source-level attempt still does not establish checks for assignment RHS values, call arguments, array/enum constructors, values crossing imported-module boundaries, resolved shadowing, generic aggregate substitution, or every AST statement/value form.
