...
- **Opaque-handle zeroing classification corrected:** Core `c1cdd77f` now treats `OpaqueType` as unable to contain an inline `sview`, while `zeroedInvalidRepresentationCount` still treats a non-null opaque handle as invalid in safe code. `26f1bf92` adds tests for both the accurate diagnostic and the trusted-boundary exception. `go test ./src/semantic -count=1`, `go test ./src/backend -count=1`, and the focused `go test ./src/semantic -run 'TestOpaqueHandleZero|TestZeroed' -count=1` pass. The source-built CLI at `c1cdd77f` confirms that opaque placeholders must migrate to explicit nullable/validity state; the 116 remaining uninitialized-read sites require separate proof of all-path initialization. This changes classification and error quality, not the safe-code zeroing rule or the compiler bootstrap outcome.
- **Current stricter-Core bootstrap gap:** a bounded seed attempt using source-built Core `c1cdd77f` (binary SHA-256 `3017801981299ecdb09ca864e5070b40a8b8526e3aa7b0dfea9582777c2dc88e`) and compiler main `89fb9b27` failed before product publication. The latest log `/tmp/elisac-core-ir-strict-seed-20260925.log` (SHA-256 `588ea5738697bb500f54e49eebe1ce86319c5567138a6e9b565b641829770df8`) has 99 invalid zeroed opaque-handle initializers and 116 uninitialized-read diagnostics, down from 101 and 119 after the Core-IR slot cleanup. It reports no remaining diagnostics in `backend/codegen_core_ir.elisa`. Core commits `c1cdd77f` and `26f1bf92` corrected the earlier misleading sview classification; safe non-null opaque zeroing remains rejected, and a focused test confirms that only an explicit trusted unsafe boundary admits raw opaque zeroing. The safe temporary bootstrap still uses proof-pinned Core Stage0 `a3f3ea3d` under the 5.5 GiB ceiling and produces the recorded Stage1 product. Next migrate nullable LLVM handles and internal storage to explicit optional/validity states, then analyze the remaining read sites individually for actual missing writes versus insufficient flow precision. Do not globally initialize pointers, add broad unsafe grants, or weaken fail-closed rules to obtain a seed.
