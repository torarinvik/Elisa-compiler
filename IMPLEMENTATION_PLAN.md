...
| S18 | Native FFI/ABI contracts and adapters | In progress — LLVM lowers typed `va_arg[T]` reads and `va_copy` with the builder/intrinsic APIs rather than treating them as unresolved external calls; C variadic default-promotion and wide-value runtime tests pass. The shared target C return classifier now also handles direct-register returns of payload-bearing error sets at native extern boundaries: the declaration uses the C return type, and named direct, `try`, and `catch` call paths reconstruct Elisa's internal error-set aggregate through an aligned transfer slot. The C-stub regressions pass at `-O0` and `-O2`; `backend_native_smoke.sh` passes 521/521. Evidence covers arm64 `{i32, i32}` only. Sret/unsupported named-call shapes decline before forming the call; external error function values/indirect calls, multi-payload C layout agreement, x86-64/Wasm execution, pointer ownership, string/buffer extents, callback lifetime, varargs type safety, and full platform ABI coverage remain open. |
...
**Verified S18 payload error-set direct C return slice (2026-09-25):** Clang characterized the native C definition `ElisaErrorSet external_probe(int64_t *, int64_t)` as `i64 @external_probe(ptr, i64)` on arm64, while Elisa's internal restricted composite status remains `{ i32, i32 }`. The shared C return classifier now accepts an LLVM aggregate type directly, declares this extern with the native return type, and normalizes named extern error calls by storing the returned register value into a target-aligned slot and loading Elisa's internal status aggregate. The adapter is used by ordinary named calls, expression/statement `catch`, propagating `try`, `try ... else`, and error-union-value calls; unclassified/sret status shapes decline before a direct call is emitted. `extern_composite_payload_c_abi_smoke.sh` links the real C stub and returns 42 for both a direct restricted-composite `catch` and a propagating `try` from both Stage1 LLVM and object outputs at `-O0`/`-O2`, with `opt -passes=verify`; `composite_subset_expression_catch_smoke.sh` likewise passes using a C stub for Stage1 and preserves the Stage0 LLVM-signature baseline. `extern_composite_payload_catch_smoke.sh`, the other focused error-set regressions, and `backend_native_smoke.sh` (521/521) pass. The old internal-LLVM stub is intentionally no longer treated as a Stage1 C-ABI oracle. This validates only the arm64 direct `{i32, i32}` register return and named-call routes: indirect/function-valued extern calls, sret/error sets beyond the supported direct shape, the C layout contract for multiple payload variants, non-arm64 execution, and ownership-safe wrappers remain open; S18 is not complete.
