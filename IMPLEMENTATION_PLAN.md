...
**Verified Stage1 slice (2026-09-25):** Field-refinement construction and recognized `<-` stores now use the error-level `StructFieldRefinementUnproven` diagnostic, so an unproved value blocks object emission instead of leaving a warning beside executable invalid storage. Struct receiver types are extracted through `mutable` and reference wrappers. The visitor carries declared local/loop/lambda shadow rows, seeds function-body facts from parameter `where` predicates, drops facts when their subject name is shadowed, and only lets an interval proof discharge an invariant when that field has a recognized interval (unknown predicates no longer masquerade as the full `i64` range). The revised `struct_field_refinement_shadow_smoke.sh` passes on a fresh Stage1 seed: it accepts a matching refined constructor and mutable-reference store on both stages, and Stage1 rejects a loop-shadowed constructor, an unrefined mutable-reference store, and a parameter whose refinement is incompatible with the field. Each rejection checks the diagnostic and that no object was emitted. The first Gen2 attempt exposed four name-based `StorageDependencyInvalidated` reports where branch-local cleanup buffers or LLVM row-type buffers reused the same source names. Those independent locals now have distinct names; `self_host_gen2.sh` then compiled the current driver and passed its Gen2 runtime fixture using the already-seeded Gen1 whose semantic checker was unchanged (`ELISA_ALLOW_STALE_STAGE1=1` for the renamed driver inputs). The resulting Gen2 SHA-256 is `3545a5380e0901c305bd36e860582528c9c160301f1b9e2efcffaf313594e7c4`; a fresh Stage0 seed is required to refresh `bin/elisac-stage1` for these source renames. The refinement visitor still uses source names rather than `BindingId`, omits expression/statement forms and pattern scopes, recognizes only a subset of invariant proofs, and does not cover every place a refined value can enter or mutate a struct. Name-based lifetime invalidation likewise remains a precision limitation. S15 and the full soundness acceptance remain open.
