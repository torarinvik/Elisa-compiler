...
| S09 | Complete bounds and safe pointer formation | In progress — stage1 view reads/stores now use count guards and known-length slices clamp start/count to the receiver extent; `slice_bounds_smoke.sh` passed 112 checks at `-O0` and `-O2` against stage0 and stage1. Runtime string-view operations now fail closed on negative or target-unrepresentable lengths before pointer use; the new Stage0/Stage1 O0/O2 smoke covers malformed and valid cases. This closes only the exercised native paths; positive forged pointer-length extents, other indexable forms, target widths, pointer arithmetic, and proof/backend coverage remain open. |
