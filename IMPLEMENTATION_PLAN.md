...
This is a local software-engineering project to build and verify Elisa as a correct, memory-safe programming language and compiler. Work changes this repository’s compiler/runtime and runs repository-owned test programs against local build products; it does not test or access third-party systems. Elisa-core compiler source is used only as an explicitly identified bootstrap dependency. References to memory safety describe language guarantees and compiler-correctness regressions.

**Inspection baseline before the current local C-string follow-up:** self-hosted compiler main was `8d13f2ba` (latest verified checked-index recovery handle initialization milestone; prior optional debug-handle milestone `27646dc3`; prior runtime-hook milestone `7cb1a319`; prior integrated WASM source milestone `0a1def1a`); its safety-relevant implementation baseline remains `f7edb529`, followed by commits expanding the WASM integer-width parity matrix (`9df7d12b`), adding signedness/width checks (`95dd8921`), gating the expensive parity runner behind bounded explicit reauthorization (`9b8ba316`), adding a portable sampled process-tree supervisor (`99624055`), preserving timestamps for unchanged Python sidecars (`1bc77526`), adding focused supervisor-limit/process-tree/signal tests (`123232ea`), and applying compare-before-write to the self-hosted driver plus a repeated-build timestamp regression (`0a1def1a`). Elisa-core main is `5793517d`, including the `AtomicSlot` work, fail-closed zero-representation walk, opaque-handle/sview classification correction in `c1cdd77f` with boundary tests in `26f1bf92`, and the nested-region and single-evaluation match fixes in `12e8eaa0` and `60267978`; Core commits `d6f9b77a` and `5793517d` add focused backend-IR and gated runtime regression coverage for generic sview region flow into darray growth. The compiler working tree contains an uncommitted two-line pattern-binding rename in `src/semantic/check_struct_field_construct_unproven.elisa` and unrelated untracked `.log` and `test/repro/field_ref_shadowed_param.elisa`; preserve these independently of roadmap edits. Current Stage1 product SHA-256 is `7431fb9fad3728f60f7f1222edb2c7350ca57241dd50ba9ff6935ca99cd90ba6`; `scripts/assert_stage1_fresh.sh` passes. Its generated runtime object at `build/runtime/elisacore_runtime.o` has SHA-256 `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. This product was seeded from the recorded Core Stage0 revision `a3f3ea3d` (binary SHA-256 `109571df4dbe8ec2a6a382b32b05bc12ab972d4a51428a6701b5a7f81387e752`) under an explicit 5.5 GiB cap; the product includes the committed Core-IR lowering, target-machine pointer-ABI, initialized metadata-slot and pattern-predicate corrections, valid inert runtime-hook handles through `7cb1a319`, optional debug handles through `27646dc3`, and checked-index recovery handle state through `8d13f2ba`; the gated repeated-build parity test remains unrun. The latest bounded strict seed used source-built Core main `5793517d` (binary SHA-256 `9e2d049ee8365c0ee97a681c60f7c1885467d67a2de50d4152e76cfd9d563d07`) and failed before publication with 193 initialization/read diagnostics (84 invalid zeroed initializers and 109 uninitialized reads), down from 194 (85 and 109). The `index_recovery_block` placeholder was the eliminated zeroed handle; `codegen_runtime_decl.elisa`, `codegen_dwarf.elisa`, `codegen_module.elisa`, and now `codegen_runtime_types.elisa` have no initialization/read diagnostics. See `/tmp/elisac-index-recovery-strict-20260925.log` for the current probe and `/tmp/elisac-debug-optionals-strict-20260925.log` for its predecessor. The earlier `5fffd6be` run labeled opaque LLVM handles as possible sviews; `c1cdd77f` corrects that classification while preserving safe-code rejection through the general invalid-zero-representation check. Keep the remaining initialization failures visible and do not weaken the checker to produce a seed. The Core checkout's existing `bin/elisac` SHA-256 is `db506d8dc72f05a1ccba75f9b7e0928be6fb35dcb9610a878ba02982b5da7f39`, but its embedded revision is `06bb1c0f` and its working-tree metadata says modified, so it is stale for Core main. A source build to `/tmp/elisac-core-5fffd6be` embeds exact revision `5fffd6beb7e36d4b77725f3e57f5c98ad4dfbb93` and has SHA-256 `246e8cc5ea6a87fa12d0f7971a4d020e4ab5c97c747c0550d8bfe586eef9c659`. Core semantic/backend package tests pass on current source. `TestRunCLIStoreSurfaceOverDArray` remains blocked before runtime execution by the four generic initialization diagnostics below, but emits no Store conformance error; Store borrow validity remains an open soundness question. The proof project currently has uncommitted `ELISA_COMPILER_REV=e61d5943` and `ELISA_STAGE0_REV=2fd6766d` pin updates, plus a dogfood edit and new replay fixtures; preserve this in-progress work. The clean compiler `4f3f7354`/`f7edb529` and Core `a3f3ea3d`/`ec851a72` worktrees had no unique commits or working-tree changes; their commits are already in compiler/Core main. The current proof pins resolve from the main checkouts, so the four redundant worktrees were removed after checking for open processes and path consumers. The clean `/private/tmp/elisac-engine-validation-v2` checkout had no unique commits or source changes; after its engine smoke exited and `lsof +D` found no users, it was removed. The last uncached `go test ./... -count=1` run completed red in `elisacore/src`, `test/backend`, and `test/semantic`; see the distinct runtime and fixture diagnostics below. No full memory-safety audit, uncached acceptance-gate pass, complete WASM parity run, or sanitizer run is claimed. Historical green counts are historical evidence only. Every implementation item below starts pending unless explicitly described as existing infrastructure; a named checker is not evidence that its entire problem class is solved.

**Follow-up to that historical inspection baseline:** the pattern-binding rename and `test/repro/field_ref_shadowed_param.elisa` have since been preserved, checked through the refinement-shadow smoke, and committed in `e16105ef`. The pre-existing empty `.log` remains untracked and was excluded from all commits. Current compiler and product identities are recorded in the current verification checkpoint below.

## Current gains review and revised priorities — 2026-09-26

**Review checkpoint before the current local C-string follow-up (2026-09-25):** compiler main was `8d13f2ba` and Core main was `5793517d`; the latest strict-seed probe used a source-built Core CLI at main revision `5793517d` (binary SHA-256 `9e2d049ee8365c0ee97a681c60f7c1885467d67a2de50d4152e76cfd9d563d07`) because the checkout's existing `bin/elisac` embeds stale revision `06bb1c0f`. The current Stage1 product, seeded from the exact proof-pinned a3 Stage0, passes freshness and has SHA-256 `7431fb9fad3728f60f7f1222edb2c7350ca57241dd50ba9ff6935ca99cd90ba6`; the runtime object is `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. Target-machine, pattern-predicate, inactive-runtime-hook, debug-handle, and checked-index recovery checks pass; strict Core-main seeding still fails on compiler-internal placeholders elsewhere, but the strict diagnostic count has fallen from 204 to 193 (84 invalid zeroed, 109 uninitialized reads). The target-machine, pattern-predicate, runtime declaration, DWARF, module, and runtime-type files are clear of initialization/read diagnostics after the checked-index recovery handle fix. The concrete Stage1 same-name atomic overload regression is committed and covered, while resolved intrinsic identity remains open in both backends. Empty-darray sview backing and checked C-string growth are committed and pass paired targeted checks. The added WASM facade width tests pass locally; the artifact-by-artifact compiler/Python comparison is explicitly gated and has not been run in this checkpoint. The parity runner has a committed cross-platform process-group supervisor with sampled RSS, elapsed-time, and output limits; focused tests cover cap validation, process-tree accounting, malformed snapshots, TERM/KILL escalation, status normalization, and reauthorization refusal. Python and self-hosted emitters now preserve timestamps for unchanged JSON/JS/type sidecars, and the gated parity smoke includes a repeat-build mtime assertion. The Python unit suite passes 43 tests, and the freshly seeded product rechecks generic invalid-reference rejection and the valid generic-scalar control at `-O0`/`-O2`. The self-hosted sidecar runtime behavior still needs the explicitly authorized bounded parity check. This review does not certify the complete compiler or runtime as memory safe. Earlier dated measurements retain their original source scope. No S01–S28 package is promoted to complete by this review.

**Current product snapshot after nullable-storage, container-mutation, and indirect-reference milestones (2026-09-25):** compiler main is `84c4b522`; Core main is `2fd6766d`. The fresh Stage1 product is SHA-256 `ef698e28768ab41ee2296ee74029c37cf575341165ddd1cc3ac6d932e9bfe2b4`; its runtime object remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`, and `scripts/assert_stage1_fresh.sh` passes. It was seeded from archived Core commit `a3f3ea3d` using binary SHA-256 `abcecb2a5c2ce069d8be42916110ff70d129fb38c830fffdfc60a36160d57827`. Seeding from current Core main did not complete: the compiler source still produces 193 invalid-zeroed/uninitialized diagnostics. A full `runtime_string_view_safety_smoke.sh` comparison is also blocked because current Core source rejects its runtime on one uninitialized `InlineVec` output and three generic `zeroed` pool slots; those are existing runtime-validity gaps, not passing parity evidence. This snapshot supersedes the prior product hash above for subsequent checks and does not promote any S01–S28 package to complete.

**Current product snapshot after qualified zeroed-alias containment (2026-09-25):** compiler main is `b490fcf6`; Core main remains `2fd6766d`. The fresh Stage1 product is SHA-256 `c2cd6c2205fabcc0c9f04791edb4f8b6b960e63b1719c760622fe05d6495a11f`; its runtime object remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`, and `scripts/assert_stage1_fresh.sh` passes. It was seeded from `/tmp/elisac-stage0-a3` (binary SHA-256 `56a4362179a3072d1b1d7f56c8fb8bbe125d40a070c93d4021df949c497bd89a`) under the 5.5 GiB seed cap. The Stage0/Stage1 zeroed-reference smoke, module-alias namespacing smoke, and all 410 diagnostics fixtures pass. The new module-qualified cases are Stage1-only because Stage0's corresponding qualified invalid-zero alias behavior is not yet qualified; they reject before LLVM output or accept and execute according to nullability at both `-O0` and `-O2`. This is a focused S04/S10/S15 containment milestone, not a full safety qualification or an S01–S28 completion claim.

**Current product snapshot after qualified zeroed-sview containment (2026-09-25):** compiler main is `4090c332`; Core main remains `2fd6766d`. The fresh Stage1 product is SHA-256 `52fdda17191c7adda1d38dd56cc0616e08b61936da3d379fd8afc2d74385ab71`; runtime object SHA-256 remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. It uses the same pinned Stage0 binary (`/tmp/elisac-stage0-a3`, SHA-256 `56a4362179a3072d1b1d7f56c8fb8bbe125d40a070c93d4021df949c497bd89a`) and the 5.5 GiB seed cap; Stage1 freshness passes. `zeroed_reference_safety_smoke.sh` and `sview_representation_safety_smoke.sh` pass at `-O0`/`-O2`, `module_type_alias_namespacing_smoke.sh` passes, and diagnostics remain 410/410. The qualified invalid-sview and nullable-sview alias cases are Stage1-only; the first rejects before LLVM output and the latter executes with exit 42. This resolves the reproduced direct module-declaration alias chain for borrowed views only; Stage0 qualification, import/`using` aliases, relative nested-module resolution, and other representation families remain open.

**Current product snapshot after shared zeroed-alias resolution and optional-zeroed lowering correction (2026-09-25; compiler commit `bae30846`, based on `4b82106a`):** the fresh Stage1 product SHA-256 is `d52b3e46dd8e426fbd6f65bba8d65e3c6ba703e2cac4a4905b167020fccc02a2`; `scripts/assert_stage1_fresh.sh` passes. Its runtime object remains SHA-256 `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. The seed used proof-pinned `/tmp/elisac-stage0-a3` (SHA-256 `56a4362179a3072d1b1d7f56c8fb8bbe125d40a070c93d4021df949c497bd89a`) under the existing 5.5 GiB cap. The zeroed safety smoke's Stage0 controls used the fresh Core `compiler/bin/elisac-stage0` at SHA-256 `c0743ffc386444a1da77036ada4ef90966c0a2a4af6313ebf7323392cbbfc205`.

- The zero-validity checker now routes alias expansion through one bounded resolver shared by its invalid-zero, borrowed-view, and non-null-reference classifiers. It strips storage qualifiers, parentheses, and refinements; follows local chains; resolves directly qualified declaration-tree module paths by exact module identity; retains optional/reference constructors for the caller's representation decision; and fails closed on ambiguous or over-depth/cyclic resolution. This is a shared alias-resolution mechanism, not yet the canonical semantic TypeId/representation service required by the plan. The direct module-qualified alias regression now also covers `id[Slot]`: returning a zeroed non-null handle through a two-link alias rejects with the uninitialized-variable diagnostic and leaves no LLVM artifact at `-O0`/`-O2`. A module-qualified nullable handle control remains valid.
- The fresh pre-fix product `52fdda17191c7adda1d38dd56cc0616e08b61936da3d379fd8afc2d74385ab71` lowered an optional-context `zeroed` as a present value with a zero payload (`Some(zeroed payload)`). A nullable handle comparison therefore returned 1 instead of 42; emitted IR showed the optional tag set true and handle payload zero. Stage0's established zeroed semantics produced the absent/all-zero optional and returned 42. Stage1 lowering now exempts `zeroed` from implicit optional payload wrapping so it reaches the literal emitter at the complete expected type. Qualified nullable reference, handle, and borrowed-view controls, plus `ReferenceHolder?` whose payload contains a non-null reference, compare equal to null and return 42 at both `-O0`/`-O2`; all four controls also return 42 on Stage0. This is specific evidence for tagged optional and niche-null layouts exercised here, not a general proof for every optional ABI or target.
- On the fresh product, `zeroed_reference_safety_smoke.sh` passes at `-O0`/`-O2` with invalid references, qualified handles, and qualified borrowed views rejected and with initialized, nullable, and scalar-generic controls executing successfully. `sview_representation_safety_smoke.sh` and `module_type_alias_namespacing_smoke.sh` pass. Freshness, `git diff --check`, and shell syntax checks pass. The attempted current-source `diagnostics_smoke.sh` run was stopped before fixture evaluation: its preparatory `parse_report.elisa` Stage1 O2 compilation remained active for over ten minutes and had produced no result. Do not carry forward a diagnostics pass claim for this product; earlier 410/410 results belong to the prior checkpoints.
- Update S04/S10/S15 follow-up work: module-qualified direct aliases for non-null references, `sview`, and `id[T]` now have focused Stage1 rejection evidence, optional zeroed values containing reference-bearing payloads have a parity/runtime control, and a narrow cstr local slice now checks direct returns and nested call-argument reads. Still open: import/`using` alias identity, nested-relative/reopened namespace resolution, aliases to aggregate fields and generic substitutions beyond tested forms, enums and other niche/tag layouts, arbitrary non-call read boundaries, and complete path-sensitive initialization. Keep these safety packages open. The next semantic task is still a canonical resolved representation query over compiler-resolved types; the shared syntax alias walker reduces drift but does not close that task.

**Current product snapshot after nested module-qualified zeroed-alias containment (2026-09-26; compiler commit `8d9f5f66`, based on `5c53ea59`):** fresh Stage1 SHA-256 is `8493bba4506f9c873678cd3d969c9fb2bc9901e01dbc1d121b06b2d40f3549d1`; `scripts/assert_stage1_fresh.sh` passes. The runtime object remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. Seed used proof-pinned `/tmp/elisac-stage0-a3` (SHA-256 `56a4362179a3072d1b1d7f56c8fb8bbe125d40a070c93d4021df949c497bd89a`) under the 5.5 GiB cap. The smoke's Stage0 runtime controls used Core `compiler/bin/elisac-stage0` SHA-256 `c0743ffc386444a1da77036ada4ef90966c0a2a4af6313ebf7323392cbbfc205`.

- A fresh pre-fix Stage1 accepted `Outer::Views::BorrowedText`, `Outer::References::Pointer`, and `Outer::Handles::SlotId` zeroed aliases inside sibling `Outer::Client` and emitted LLVM. The parser stores each nested module's full owner (`Outer::Views`) on its `Decl.Module` node. The checker now carries the file declaration tree for alias lookup, compares the complete AST module owner against the source path, and still passes the current module subtree to aggregate field traversal. This rejects the nested borrowed view with the valid-backing diagnostic, the reference with the non-null-reference diagnostic, and the handle on its first read; each fails before LLVM output at `-O0` and `-O2`. A nested nullable-sview alias control compares equal to null and returns 42 on Stage1 and Stage0 at both optimization levels.
- `zeroed_reference_safety_smoke.sh`, `sview_representation_safety_smoke.sh`, and `module_type_alias_namespacing_smoke.sh` pass on this product; freshness, whitespace, and shell syntax checks pass. Do not claim a current 410/410 diagnostic run: the most recent attempt stopped during the harness's `parse_report.elisa` preparation before fixture evaluation. Keep S04/S10/S15 open for shortened/enclosing-relative paths, `using`/import alias identity, reopened or ambiguous namespaces outside the tested exact owner paths, generic/enum and aggregate representations beyond current cases, remaining value boundaries, and complete CFG initialization. The canonical TypeId-based resolved representation service remains a separate planned task.

**Current product snapshot after enclosing-relative zeroed-alias containment (2026-09-26; compiler commit `45dacaf9`, based on `95fa8e2c`):** fresh Stage1 SHA-256 is `295be8ecd0590abb8f6feb0b4db4334725c70107ae2eac6e009181839cd8bed8`; `scripts/assert_stage1_fresh.sh` passes. The runtime object remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. The seed used `/tmp/elisac-stage0-a3` (SHA-256 `56a4362179a3072d1b1d7f56c8fb8bbe125d40a070c93d4021df949c497bd89a`) under a 5.5 GiB cap. The product and checks were built from the full working tree at that checkpoint; this implementation commit contains only the checker/API change, its regression fixtures, and its focused smoke additions.

- The fresh pre-fix product `8493bba4506f9c873678cd3d969c9fb2bc9901e01dbc1d121b06b2d40f3549d1` accepted language-valid `Views::BorrowedText` paths both inside `Outer` and inside sibling `Outer::Client`, then emitted LLVM for a zeroed `sview`. Alias lookup now tests the current module owner, each enclosing owner, then the root, comparing exact AST owner names and failing closed when a selected owner has ambiguous alias declarations. The safety classifier receives the current module through local/global initializer checks, returns, nested statements, and aggregate-recursion call paths. Regressions cover nested-relative and sibling-relative borrowed views, a nested-relative non-null reference, a sibling-relative `id[Slot]` handle, and a nullable sibling-relative view. Invalid values fail with their expected diagnostics and no LLVM artifact at `-O0`/`-O2`; the nullable-view program returns 42 at both levels.
- `zeroed_reference_safety_smoke.sh`, `sview_representation_safety_smoke.sh`, and `module_type_alias_namespacing_smoke.sh` pass; Stage1 freshness, shell syntax, and `git diff --check` pass. The relative-path cases remain Stage1-only because the pinned Stage0 does not implement this qualified invalid-zero alias behavior. The current diagnostics harness was not rerun; the latest attempt remains stopped during `parse_report.elisa` preparation before fixture evaluation.
- This closes the reproduced current/enclosing-relative declaration-tree alias bypass for the tested representation families. Keep S04/S10/S15 open for Stage0 parity; import/`using` aliases; reopened-module ambiguity beyond the exercised `extend` fragments; aliases nested in aggregate fields or generic substitutions; enum and other niche/tag layouts; non-call read and write boundaries; complete path-sensitive initialization; and the canonical TypeId-based resolved-representation service. Direct return and call-argument reads have focused cstr-local tests, but are still backed by a function-wide name set rather than binding-identity CFG facts. Closest-owner and enclosing-owner precedence now have focused shadowing controls; add resolver identity tests as the implementation moves away from this syntax-level walk.

**Current product snapshot after module-extension and alias-owner zeroed-alias containment (2026-09-26; compiler commits `90f36d44` and `cc0c44b0`, based on `71a798c4`):** fresh Stage1 SHA-256 is `dd7c07b7757fc07b5b805123aca4e92948670d822cbd7bdb38093e7b8b9ebb6c`; `scripts/assert_stage1_fresh.sh` passes. The runtime object remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. The seed used `/tmp/elisac-stage0-a3` (SHA-256 `56a4362179a3072d1b1d7f56c8fb8bbe125d40a070c93d4021df949c497bd89a`) under the 5.5 GiB cap. As in the previous checkpoint, the product and smokes were built from the full working tree; unrelated in-progress edits were not included in these commits.

- Pre-fix Stage1 product `295be8ecd0590abb8f6feb0b4db4334725c70107ae2eac6e009181839cd8bed8` accepted sview, non-null-reference, and `id[Slot]` aliases whose base aliases were in an original module fragment and whose public aliases were in `extend Outer::Types`; LLVM contained zeroed sview, null-reference, and zero-handle stores. A later intermediate product `be66e8f95a8e4e4ab2b508f5737fd4cf43fc1dc18edefbd9223b3f71ed171364` fixed cross-fragment chains but still emitted zeroed sview when an extension alias target used `Internal::NestedBorrowedBase`, because the target was resolved from its use site. The resolver now combines aliases across all fragments with the selected exact owner, rolls back alias rows from failed nearer-prefix candidates, and carries the declaring owner through qualified and lexically inherited alias chains. Relative paths inside alias targets are therefore checked from their declaration module.
- Regressions cover cross-fragment chains for all three invalid-zero families, a nested-relative target inside an extended module, an inherited bare alias whose nested target is shadowed by a same-named scalar module at the use site, and nearest-module precedence controls. Invalid cases reject with the expected sview/reference/uninitialized diagnostics and no LLVM at `-O0`/`-O2`. The nearest scalar shadow avoids an incorrect sview diagnostic; Stage1 currently declines that function in the backend, so the smoke accepts either that explicit decline or later successful lowering. Extension and relative-alias cases remain Stage1-only because pinned Stage0 reports their qualified names as unknown types.
- `zeroed_reference_safety_smoke.sh`, `module_type_alias_namespacing_smoke.sh`, and `sview_representation_safety_smoke.sh` pass on this product; Stage1 freshness, shell syntax, and `git diff --check` pass. Do not claim a current 410/410 diagnostics run; the latest attempt still stopped during `parse_report.elisa` preparation. Keep S04/S10/S15 open for Stage0 parity, import/`using` identity, complete visibility and ambiguity rules for reopened modules, owner identity through every aggregate/generic alias path, enums and other niche layouts, remaining value boundaries, full CFG initialization, and the canonical TypeId-based resolved-representation service.

**Current product snapshot after module-import zeroed-alias containment (2026-09-26; compiler commit `43dd29f5`, based on `3ff1ddb5`):** fresh Stage1 SHA-256 is `9a1abe8ab1d21ba9403066b54b67f9a64608e95eed9d5075bd30e2c73231ab09`; the freshness assertion passes. The seed used `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Validation used the full working tree; unrelated concurrent edits remain outside this milestone commit.

- Pre-fix product `dd7c07b7757fc07b5b805123aca4e92948670d822cbd7bdb38093e7b8b9ebb6c` accepted `using Views as V` followed by `text: V::BorrowedText = zeroed` and emitted an invalid zeroed sview carrier. The shared zero-validity resolver now expands module import aliases before finding the type alias. Parser metadata records the declaring module on both paired rows; the checker matches source line, offset, and owner, searches the nearest lexical module then ancestors/root, resolves relative import targets from their declaring owner, and bounds repeated expansion. Duplicate paired rows and cycles fail closed.
- `zeroed_reference_safety_smoke.sh` passes at `-O0`/`-O2` with new Stage1 regressions for root import aliases, nested module targets, and an enclosing-module alias inherited by a child. Borrowed views and non-null references reject at initialization; a returned zeroed handle reports an uninitialized read; failures leave no LLVM artifact. Nullable-view and scalar aliases compile and execute with exit 42. `module_type_alias_namespacing_smoke.sh` and `sview_representation_safety_smoke.sh` also pass, as do freshness, shell syntax, and whitespace checks.
- Keep S04/S10/S15 open. This is declaration-tree containment for the tested `using ... as ...` paths, not a complete import or visibility model. Follow up with selective/wildcard imports, cross-file identities, alias/module name collisions, duplicate and cyclic import fixtures, source-path normalization, and consistency with other semantic/backend import consumers. Continue the canonical TypeId-based resolved-representation service, complete aggregate/generic/enum layouts and value boundaries, Stage0 parity, and CFG definite initialization. No new full diagnostics, full gate, sanitizer, or complete memory-safety qualification is claimed.

**Current verified aggregate-alias milestone (2026-09-26; compiler commit `1daae595`, based on `ccd13ec7`):** the isolated Stage1 product SHA-256 is `b9b022b2b62473156218c9a17e89cac22f0c65c16f55546dff5c8864d58dd0f2`; the runtime object remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. A clean temporary checkout of `ccd13ec7` plus exactly this milestone's source and tests was seeded from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Freshness passed inside that checkout. The verified binary is preserved at `/tmp/elisac-stage1-aggregate-1daae595`; the temporary worktree is removed after its gains are committed. The main working tree contains concurrent backend/parser/checker edits; its product is not claimed fresh for this milestone. An initial full-tree build failed on an undefined `ctx` in an unrelated in-progress backend edit; isolated validation preserved that work.

- Pre-fix Stage1 accepted qualified aliases to named tuples containing sview and non-null references. Direct classifiers resolved aliases, but the aggregate walk matched the original qualified expression and skipped the payload. It now walks the shared resolver's resulting shape, carries the declaring owner through child-type checks, resolves bare aliases from that owner rather than a use-site alias array, and bounds shape recursion. New negative coverage includes tuples, fixed-array aliases, import-aliased tuples, and a tuple element alias shadowed by a scalar at the use site. Each tested invalid declaration has its expected diagnostic and no LLVM output at `-O0`/`-O2`.
- Validation exposed a separate representation error: backend alias resolution ran before tuple layouts existed, then fell back to the target span's last identifier. The pre-fix tuple alias emitted an `i64` allocation/store instead of a tuple; a valid nullable-tuple alias comparison declined. Tuple-bearing alias targets now register their nested shapes in the declaring owner during declaration-order alias resolution, before consumers size structs or emit values. An unresolved tuple shape cannot use the last-identifier fallback. Label-span accounting now follows optional/parenthesized nested tuple types. The scalar alias-chain control writes 11 and 31 to distinct tuple fields and reads back 42; its LLVM uses `{ i64, i64 }` and distinct field GEPs. Nullable and nested-nullable tuple controls both return 42 at `-O0`/`-O2`.
- Passed in the isolated checkout: `zeroed_reference_safety_smoke.sh`, `sview_representation_safety_smoke.sh`, `module_type_alias_namespacing_smoke.sh`, `tuple_label_return_smoke.sh`, `positional_tuple_type_smoke.sh`, and `emit_c_bind_parity_smoke.sh` (10 identical, 0 divergent). Freshness, shell syntax, and whitespace checks pass. Logs are `/tmp/elisa-aggregate-{smoke,sview,namespacing,tuple-return,positional,cbind}.log`; the seed log is `/tmp/elisa-aggregate-isolated-seed.log`.
- S04/S10/S15 and backend correctness remain open: direct qualified structs and complete nominal owner identity, generic argument provenance, import visibility/collisions, enums, every expression boundary, CFG initialization, and canonical TypeId representation facts still need implementation. Audit all remaining structural-alias head fallbacks and layout dependency ordering, including packed/payload enums and generic tuple aliases; this tuple-specific registration does not establish a complete dependency graph. No current full diagnostics, full acceptance gate, sanitizer, or complete memory-safety claim is made.

**Current verified nominal-struct safety milestone (2026-09-26; compiler commit `328453dc`, based on `4a61e2a2`):** isolated Stage1 SHA-256 is `0f49e607c056b0213245742a2d6846bb2ec1c878228cad12e6c3c7e5a2fa38fd`; runtime object SHA-256 remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. Validation used a clean checkout of the recorded base plus this milestone's source/tests, seeded from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Freshness passed there; the verified product is retained at `/tmp/elisac-stage1-nominal-328453dc`. The temporary validation worktree is removed after committing its gains. Concurrent source edits in main were preserved and are not covered by this isolated product's freshness claim.

- The previous verified product accepted a direct qualified struct containing a module-local sview alias, an imported alias for that struct, a qualified generic struct with a borrowed-view type argument, and nested same-named structs containing a borrowed view. The direct qualified struct emitted a zeroed `%Unsafe.Carrier`; the imported `U::Carrier` incorrectly selected `%Safe.Carrier` in lowering. The checker now resolves nominal structs by exact declaration owner, filters fields by both name and `struct_field_module`, carries owner identity through qualified/bare/inherited struct lookup and generic heads, and removes the basename-only visited shortcut that skipped same-named nested structs. Generic substitutions carry the argument-expression owner and occur before alias lookup, so a template's same-named scalar alias cannot hide a caller's borrowed-view argument. Builtin leaf types bypass unnecessary declaration-tree lookup.
- Backend simple qualified annotations now expand a module import alias before selecting the owner, including the multi-argument application path. The valid same-named struct control writes and reads a field through `S::Carrier`, emits `%Safe.Carrier = type { i64 }`, and returns 42 at `-O0`/`-O2`; the semantic qualified-generic scalar control also returns 42. These are tested paths, not complete generic-template or scoped-import identity support.
- Passed in the isolated checkout: `zeroed_reference_safety_smoke.sh` (new qualified/imported struct, non-null-reference field, generic argument-owner, nested same-name negatives, and scalar controls at both optimization levels), `sview_representation_safety_smoke.sh`, `module_type_alias_namespacing_smoke.sh`, `tuple_label_return_smoke.sh`, and `emit_c_bind_parity_smoke.sh` (10 identical, 0 divergent). Negative tests require the representation diagnostic and no LLVM artifact. Freshness, shell syntax, and whitespace checks pass. Seed log: `/tmp/elisa-nominal-seed.log`; suite logs: `/tmp/elisa-nominal-<suite-name>.log`.
- **Generic template ownership milestone (2026-09-26, `c8baf167`):** the previously measured `Wrong::Box`/`Right::Box` layout collision is corrected. Template lookup follows exact declaring owners and lexical ancestors; instances memoize by template row identity plus argument type identity, and recursion tracks exact instance rows. Field annotations resolve in the template owner while qualified argument expressions resolve in the caller. Qualified single/multiple arguments, imported module aliases, inference, concrete export lookup, and legacy instance lookup use that identity. Distinct module-qualified emitted names preserve distinct layouts. Incomplete recursive by-value/optional/array layouts decline; nullable recursive references remain valid.
- Validation used isolated base `f98443dc` plus the milestone source/tests, seeded from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Product SHA-256: `ff925a347ec04d976938d5db0d6a2069bb7964da9b140cad84b43fa0a70de9cf`; runtime object remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. The product is retained at `/tmp/elisac-stage1-template-c8baf167`. The new generic-struct module identity suite passes for Stage0 and Stage1 at O0/O2: reversed declaration order, simultaneous instances, exact LLVM field layouts, caller alias arguments (strengthened after this milestone to shadow template-local aliases), one/two type arguments, relative module paths, qualified inference, recursive references, and rejection without LLVM output of infinite optional layouts. Existing zeroed-reference safety, module alias namespacing, phantom generics, aggregate exports, and qualified generic call/argument scope suites pass. The qualified-call suite required the isolated runtime-object path. Concurrent main edits are outside this product's freshness claim.
- **Builtin phantom state identity milestone (2026-09-26, `220865d9`):** the saved template-identity product collapsed `Token[Held]` and `Token[Pending]` into `%Token__unknown` and a single `identity__Token_unknown` specialization. Backend annotations now retain distinct nominal tags for builtin `Local`, `Frozen`, `Joinable`, `Pending`, and `Held`. They remain `Unmodeled` for runtime representation; nonzero tag bits participate in existing identity comparisons and marker spellings participate in mangling. A generic binding still shadows the builtin lookup. This corrects direct builtin-marker instantiation identity without granting these tags a runtime layout.
- Validation used isolated base `2690f3a4` plus exactly the milestone source/tests, seeded from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Product SHA-256 is `88cd21f074f89f7160ebac5a994acca9bd11fa7d524d498da67ce7482c4454dd`, retained at `/tmp/elisac-stage1-state-220865d9`. Freshness passed in that checkout; concurrent main edits remain outside its qualification. The dedicated suite verifies all five distinct LLVM struct names and generic function definitions at O0, execution returning 42 at O0/O2, and failure without LLVM when attempting `zeroed` runtime storage of a state marker. Existing zeroed-reference safety, generic struct module identity, and phantom shadow suites pass. Current Core also executes the five-state control at O0. Aliased/user-declared phantom types, imported nominal identities, all state-effect dispatch paths, recursive structural argument mangling, and persistent canonical TypeIds still require implementation/verification; this milestone does not close S01/S04/S17 or full acceptance.
- **Zero-selected enum payload safety milestone (2026-09-26, `2b2857f7`):** the preceding saved Stage1 product accepted `Message = zeroed` when variant zero was `Text(value: sview)`, emitting a zero tagged-union aggregate with an invalid active backing pointer. The semantic representation walk now resolves enum declarations by exact lexical/qualified owner (including module import aliases) and recursively checks the first ordinary payload variant, which the backend selects with tag zero. Inactive variants do not impose live-payload validity. Nested enum/struct payloads use the same alias, owner, and generic substitution walk. Local, global, and direct-return initializers all use this classifier.
- Validation used isolated base `2ea9a823` plus the milestone checker and tests, seeded from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Product SHA-256 is `ed6d4747eeab49762e39b5f0c39aa974efe34f782a799de8fc40ccee95f6eb13`, retained at `/tmp/elisac-stage1-enum-2b2857f7`. The new representation suite passes with this Stage1 and source-built Core `/tmp/elisac-core-recursion-candidate`: six invalid initializer fixtures reject with a zeroed diagnostic and no LLVM/object output at O0/O2; inactive-view, active-scalar, and explicit-view-constructor controls execute and return 42 at both levels. Existing zeroed-reference safety, module payload namespacing, payload constructors (LLVM verifier included), and generic struct module identity suites pass against the isolated Stage1. Concurrent main source changes remain outside this product's qualification.
- **Prior measured enum-layout gap (closed by `0d8b0ad9` below):** `test/repro/zeroed_enum_nullable_payload.elisa` has a first variant containing `i64&?`. Current Core compiles it and returns 42 at O0/O2; both preceding and new Stage1 products decline its variable declaration because ordinary payload registration excludes reference kinds. The following reference-payload milestone implements and verifies this path; the nullable control has been restored to the combined representation suite. Enum inheritance, function-local state enum metadata, packed-store validity, nonzero scalar-enum discriminants, all expression initialization boundaries, canonical validity facts, CFG initialization, self-host generations and full acceptance/sanitizer gates remain open. Aliases directly to `Held`/`Pending` currently reject as unknown types; define and verify their intended language semantics before treating them as equivalent phantom arguments.
- **Reference payload layout milestone (2026-09-26, `0d8b0ad9`):** ordinary payload registration now accepts reference and optional-reference kinds. Blob sizing obtains reference byte size and alignment from the target pointer ABI; optional-reference sizing retains existing niche handling. Existing constructor and match paths store/load at the actual payload LLVM type. This also makes structs containing nullable references sizeable when used as enum payloads, without treating a non-null reference zero bit pattern as valid.
- Validation used isolated base `e5e3fe30` plus exactly this source/tests, seeded with `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Product SHA-256 is `669352c536d66c86c2c0b9139d870a934a50c48c69ac1b5bde0c02706d0cd7d3`, retained at `/tmp/elisac-stage1-ref-enum-0d8b0ad9`. Both this Stage1 and source-built Core `/tmp/elisac-core-recursion-candidate` execute null, direct, multi-field named arguments, module-owned alias annotations, nested struct payloads, extraction and live backing mutation controls at O0/O2, returning 42. The single-pointer blob is one i64 word; the mixed i64/pointer payload is two words. Native LLVM verifies. Both compilers also emit verified wasm32 LLVM and compile it to wasm objects at O0/O2 with a 32-bit pointer data layout; wasm execution is not established by these object checks. Restored nullable zeroed controls and the complete active-payload rejection suite pass. Existing payload constructor, module payload namespacing, and zeroed-reference safety suites pass. The isolated product does not qualify concurrent main edits or full acceptance/self-host/sanitizer gates.
- **Prior measured module-alias gap (fixed by `7c277162`/Core `fb225fe8` below):** constructing and matching `A::Message.With`/`A::Message.Empty` after `using Aliased as A` fails in both compilers: current Core reports unknown identifier `A.Message` in constructor lowering, and the isolated Stage1 declines the match and call. The reference roundtrip deliberately uses the declaring module for constructors/patterns while retaining the import alias in the type annotation; it therefore does not prove constructor/pattern alias support. Resolve these through declaration owner identity and retain direct/alias equivalence runtime tests before module identity can close. Packed reference payloads, scalar-enum invalid zero discriminants, hierarchy/state metadata, complete CFG validity and all-target execution remain open.
- **Enum module-alias identity milestone (2026-09-26, Stage1 `7c277162`, Core `fb225fe8`):** Stage1 constructor heads resolve alias roots while comparing the complete owner path; flattened payload-pattern paths normalize alias roots and separator spelling without allocating or dropping declaration ownership. The shared const/packed head matcher also retains exact owner ranges instead of matching only the final owner component. Core now carries its analyzed module-alias map in `semantic.Result` and uses canonical alias targets between lexical and explicitly written candidates during backend lookup. It no longer reconstructs alias ownership from an unresolved source qualifier.
- Validation used isolated Stage1 base `152f4b07` plus the four changed backend sources/tests, seeded from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Stage1 product SHA-256 is `acfbf0161ef2a8614fa0e20cf84ed11a0d117c80df1765bc4546138738ea4396`, retained at `/tmp/elisac-stage1-enum-alias-7c277162`. Core candidate `/tmp/elisac-core-enum-alias-candidate` SHA-256 is `575f1a3b0181d73e19e797de8b764ac8ba37485e3647d49b906f3419617c4411`; it was built from the milestone source before the commit and is not claimed to embed `fb225fe8`. Core semantic/backend packages and the new alias constructor/pattern IR regression pass. Both compilers execute root aliases, aliases to nested modules, same-named enums with differing arities/tag order, direct/alias constructors, empty variants, nullable-reference constructors/extraction and backing mutation at O0/O2; LLVM verifies. A wrong-declaring-module return rejects with no LLVM/object at O0/O2: Core reports the type mismatch, while Stage1 declines safely in backend return lowering. Related module payload namespacing, reference payloads (including wasm IR/object checks), qualified generic call/argument scope, module sibling/generic precedence, and packed enum namespacing suites pass. Concurrent main changes and full acceptance remain outside the isolated product's qualification.
- **Explicit nested enum constructor milestone (2026-09-26, `722aad8e`):** the preserved `enum_module_nested_direct.elisa` gap is fixed. Constructor leaf extraction now accepts arbitrary scoped heads; the subsequent head-aware registry lookup still validates the complete declaring-module path. `enum_module_alias_smoke.sh` includes explicit two- and three-level paths alongside alias controls and a nested wrong-owner negative. Both Stage1 and the retained Core alias candidate execute the positive cases with result 42 and verify LLVM at O0/O2; wrong-owner returns reject without LLVM/object artifacts at both optimization levels. Related module payload namespacing, packed enum namespacing, payload constructor and reference payload suites pass; the reference suite also verifies wasm32 LLVM and emits wasm objects, without claiming wasm runtime execution. Validation used isolated base `3a98aab5` plus the exact changed helper and fixtures, seeded from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Stage1 SHA-256 `8472a91699fa9ae7c4d33f80ba7a8435758d6a4279c854343a92d73982d89d95` is retained at `/tmp/elisac-stage1-nested-enum-722aad8e`. Concurrent main edits are outside this product qualification. General lexical alias visibility, ancestor-relative enum paths, chained aliases, packed flattened pattern paths, persistent canonical identities and complete initialization analysis remain open.
- **Chained module alias milestone (2026-09-26, Core `bcbcffff`, Stage1 `ca466bb5`):** Core canonicalizes a previously registered alias root when registering another alias, preserving any qualified suffix. The new backend regression follows three aliases to `Outer.Right`, asserts each analyzed target is canonical and lowers constructors/patterns. Full Core semantic/backend packages pass. Candidate `/tmp/elisac-core-chained-candidate` SHA-256 `aeaf68229f5ac354ec398a56eb60c1cf42629830f1a730da0368e0df0bf7a2f8` was built before the commit; it executes the chained enum fixture with result 42 at O0/O2 and verifies LLVM. Stage1 now follows recorded alias chains with traversal bounded by the alias row count; exhausted cyclic lookup returns no owner. Isolated base `515099f4` plus the exact helper and regressions seeded successfully from `/tmp/elisac-stage0-a3` under the 5.5 GiB cap. Product SHA-256 `d85e681bd2c2ff419e7537cecfa345ff8bcd3c09545e8186608c99e30cb44291` is retained at `/tmp/elisac-stage1-chained-enum-ca466bb5`. The expanded module-alias suite executes three-alias constructors/patterns at O0/O2 and rejects chained wrong-owner returns without LLVM/object artifacts in both compilers. Module payload namespacing, packed enum namespacing and reference payload suites pass, including reference wasm LLVM/object checks. These checks do not establish qualified-suffix chains, lexical alias scope or general cyclic-alias diagnostics. Ancestor-relative paths remain measured failures: `Right::Message` from `Outer::Worker` fails semantic/type resolution in both products. Core module-path recognition already walks namespace ancestors, whereas visible name candidates currently add only the immediate namespace; align semantic and backend precedence before claiming this requirement complete.
- **Remaining S01/backend obligations:** row IDs are local compilation identities, not persistent canonical TypeIds across imports/caches. The globally unique imported-template fallback remains; explicit visibility, general phantom argument nominal identity beyond the direct builtin markers, and generic-function declaration-owner coverage require further work. The initially reported module-local duplicate-alias prerequisite was a test error: `Local` is a builtin state-marker type name, so its redeclaration correctly rejects. Renaming that alias to `Padding` restores the intended caller/template alias-shadowing test. The strengthened fixture uses template-local `Padding = i32` and `Item = sview`, with root `Padding = i64` and `Item = i64`; both pinned Stage0 and the saved Stage1 product return 42 at O0/O2. Stage1 O0 LLVM independently shows `{ i32, i64 }` and `{ i64, i32 }`; O2 folds main to `ret i64 42`, so representation assertions apply at O0. This is fixture/plan correction with no compiler source change. Recursive optional rejection remains frontend recursion-limit evidence. Additional fixed-array and mutual by-value fixtures now reach Stage1 backend decline at O0/O2 and leave no LLVM artifact; these directly exercise pending layout guards. A mutually recursive nullable-reference control compiles and returns 42 on both pinned compilers at O0/O2. Pinned Stage0 rejects the fixed-array cycle with bounded frontend diagnostics but crashes with a Go stack overflow for the mutual by-value case; the dedicated smoke runs that negative on Stage1 only. Core follow-up `385721e2` (based on `a6ec0b15`) bounds callback-carrying type specialization at 256 and reports a semantic error instead of exhausting the host stack. Generic field substitution creates fresh instance objects, so a pointer-only visited set would not establish termination. The focused hardening test and source-level mutual-generic value/reference tests pass; the full Core semantic package passed before adding those source regressions, which then passed separately. A candidate built from exactly the changed Core source (`/tmp/elisac-core-recursion-candidate`, SHA-256 `d8993f6ba3fef043719c0cf91982a7637cf766d765f7a282e555a65a66d1d4e6`) rejects the formerly crashing fixture with the callback-specialization depth diagnostic and no LLVM at O0/O2; both valid reference controls return 42. The candidate was built before the commit and is not claimed to embed the new revision. The compiler smoke can include the fixed Stage0 mutual-layout negative with `ELISA_STAGE0_RECURSIVE_LAYOUT=1`; the legacy pinned seed retains its known defect. This is a bounded robustness fix, not complete recursive-type validity/canonical graph analysis or full cross-generation acceptance. Keep S04/S10/S15 open for all representation families, enum/generic layouts, expression boundaries, imported visibility, canonical facts, Stage0 parity, and CFG initialization. Full acceptance, self-host generations, differential corpus, and sanitizer qualification remain unproven.

### Latest verified continuation — 2026-09-25

- **Checked-index recovery now has explicit optional handle state:** compiler commit `8d13f2ba` changes `Runtime.index_recovery_block` from `zeroed LLVMBasicBlockRef` to `LLVMBasicBlockRef? = null`. The checked-index guard unwraps the handle before creating the recovery edge; if internal state says recovery is active but the block is absent, the out-of-bounds edge follows the ordinary trap path, avoiding an invalid LLVM operand or unchecked element read. The fresh Stage1 product from the proof-pinned a3 Stage0 has SHA-256 `1b56eafe1aae917e6c49bf0ba67b41aea1f0a6d274fd365f9580ec30d26829eb`; the runtime object remains `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`, and `scripts/assert_stage1_fresh.sh` passes. `get_index_else_recovery.elisa` agrees between Stage0 and Stage1 at `-O0`; a native Stage1 run at both `-O0` and `-O2` returns success. A bounded strict-source seed using Core main `5793517d` still fails before publication on 193 diagnostics (84 invalid zeroed values, 109 uninitialized reads), but reports none in `codegen_runtime_types.elisa`; the previous total was 194. This closes one invalid-placeholder site and one tested checked-index lowering state only, not all LLVM handle initialization or bounds-check correctness.
- **C-string scanning now keeps its termination contract through nullable APIs (compiler commit `0924ffd9`):** the runtime `strlen`/`strcmp` externs take `cstr`, `runtime_strlen`, `string_index`, and slice-comparison helpers accept only `cstr?`, and the latter reuse bounded `sview` operations. The Stage1 argument checker now follows `Optional` payloads to recognize `cstr?` parameters; before this, a nullable raw `u8&?` could pass into `runtime_strlen` because the checker saw only the wrapper. Reference compatibility no longer treats every `u8&` as a C string; a directly written string literal retains its compiler-proven terminator path, while a raw reference write through `mutable cstr&` is rejected. The fresh Stage1 seeded from the recorded Core Stage0 `a3f3ea3d` has SHA-256 `7431fb9fad3728f60f7f1222edb2c7350ca57241dd50ba9ff6935ca99cd90ba6`; its runtime object is `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`, and the freshness assertion passes. `sview_representation_safety_smoke.sh` passes at `-O0`/`-O2`, rejecting nullable raw pointers at the runtime scanner and other C-string scan boundaries and rejecting a raw write through `cstr&`; positive controls preserve C-string scanning, null handling, literal writes through `cstr&` (exit 98), and C-string-returning `u8&` implementation results (`cstr_ref_return_smoke.sh`). `runtime_string_view_safety_smoke.sh` also passes with the Stage1-only raw-pointer and region-lifetime requirements separated from the proof-pinned Stage0 parity set. `diagnostics_smoke.sh` also passes all 410 fixtures. That pinned Stage0 still accepts raw nullable bytes at the C-string boundary and predates destroyed-region checking; this is recorded as a compiler-generation gap, not a Stage1 parity success. S09 remains open for foreign pointer extent/provenance, external C-string validity, borrowed result lifetimes, target-width coverage, and backend/FFI proof.
- **Nullable C-string storage no longer hides raw references in common aggregate boundaries (compiler commit `e257717b`):** `check_string_storage_value` now carries the expected structural TypeId through optional values, arrays, dynamic arrays, sets, dictionaries, tuples, comprehensions, conditional/block tails, nested struct construction, and record updates. It is applied to typed local initialization, rebind, return, array/darray element writes, and struct-field writes. The `darray.push` intrinsic is checked against its receiver's element TypeId, and assignment wording preserves the nullable `?`. The regression rejects raw `u8&?` in darray/fixed-array/dict/set/tuple literals, a darray comprehension, field/index writes, `push`, nested struct/container fields, a record-update field, and one branch of a nullable-string conditional; all cases fail before LLVM output at `-O0` and `-O2`. Safe nullable C-string field, darray literal/push, and fixed-array storage compile and run at both optimization levels. `cstr_ref_return_smoke.sh` passes and `diagnostics_smoke.sh` remains 410/410; Stage1 freshness passes at SHA-256 `4b5ba14811aef6c4cf9eee4e2bbe0b916350dc3954bdd0704162865ece8bc9b7`. Some aggregate-positive forms are intentionally negative-only here because the backend declines their unrelated aggregate lowering; current Core Stage0 also cannot run the broad runtime-view comparison because of the four existing runtime initialization diagnostics recorded above. This is a local AST/type-table containment pass, not a complete storage API audit: match/catch result tails, enumerated built-in mutation calls, and one mutable dictionary-reference write have focused regressions, while generic/imported nominal identity, other mutation/reference-producing APIs, calls routed through user helpers or indirect calls, canonical provenance, and safe FFI termination/extent remain open. Keep S04/S09/S15 open.
- **Nullable C-string storage now checks match and catch result arms (implementation commit `1deb2767`, regression commit `b0d37089`):** the storage walker applies the receiving structural TypeId to each terminal expression or returned expression in `match` and `catch` arms, while leaving guards and preceding effect statements outside that result context. Negative repros reject a raw nullable `u8&?` on one `cstr?` match branch and one catch error branch before LLVM emission at `-O0` and `-O2`; the positive storage fixture selects a valid nullable C string or `null` through both constructs and compiles/runs at both levels. `sview_representation_safety_smoke.sh` and `cstr_ref_return_smoke.sh` pass; the surrounding diagnostics run passed 410/410 fixtures; shell syntax, whitespace, and Stage1 freshness pass. Current Stage1 SHA-256 is `47dec3260521235478bbf7d78f853b25bf9bb41fd66d7e9934e40fbc787602dd`; runtime object SHA-256 remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. This closes the tested AST result-arm shapes only, not calls, arbitrary mutation methods, nominal identity, FFI provenance, or full memory safety.
- **C-string guards now cover built-in container mutation APIs (compiler commit `282368c0`):** the earlier `dict.put("key", raw)` probe accepted a raw `u8&?` for a `cstr?` value and emitted LLVM. The semantic pass now checks concrete receiver types for `darray.push`, `set.add`/`insert`, `dict.put`/`get_or_insert`, and `dict.entry(...).insert`/`get_or_insert`; it unwraps reference layers so borrowed container parameters receive the same check. Seven negative cases (the six local forms plus a borrowed dictionary receiver) reject before LLVM output at `-O0` and `-O2`. Safe nullable values pass through all four dictionary write APIs, and the executable confirms all four keys are present at both optimization levels. `sview_representation_safety_smoke.sh`, `cstr_ref_return_smoke.sh`, `diagnostics_smoke.sh` (410/410), shell syntax, whitespace, and Stage1 freshness pass. Current Stage1 SHA-256 is `89457ece5efc362ce1badfab8c4dedcf6313b4993b014eefabf8712151b2c30a`; runtime object SHA-256 remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. Remaining coverage includes unmodeled mutation APIs, user helper/indirect calls, canonical type identity, and FFI extent/termination; this targeted boundary check does not establish general container or memory safety.
- **Writes through dictionary lookup references preserve C-string validation (compiler commit `84c4b522`):** a second probe found that `slot: mutable cstr?&? = values.get("key"); if slot != null: slot as & <- raw` could store raw `u8&?` through the returned mutable element reference. The analyzer now recovers the stored value TypeId through optional-reference wrappers and checks the pointee assignment. The raw write rejects before LLVM output and the corresponding valid `cstr?` write compiles and executes at `-O0` and `-O2`. The complete representation-safety smoke, C-string return control, diagnostics matrix (410/410), shell syntax, whitespace check, and Stage1 freshness pass. Stage1 SHA-256 is `ef698e28768ab41ee2296ee74029c37cf575341165ddd1cc3ac6d932e9bfe2b4`; runtime object SHA-256 remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. The check covers the tested locally typed optional-reference alias; different reference-producing APIs, raw/foreign aliases, and pointer casts need a canonical place/provenance analysis.
- **Qualified aliases can no longer hide non-null references from `zeroed` validation (compiler commit `b490fcf6`):** the fresh pre-fix Stage1 (`ef698e28768ab41ee2296ee74029c37cf575341165ddd1cc3ac6d932e9bfe2b4`) accepted `mutable RefTypes::IntegerReferenceAlias = zeroed` and emitted LLVM, where the module alias chain ended at `mutable i64&`. The checker now walks `Module::Alias` declaration paths by exact module identity, follows module-local alias chains, and strips type-transparent storage/refinement wrappers before classifying the representation. The negative fixture combines a mutable-qualified non-null alias chain with same-named nullable aliases in another module; it rejects with the non-null-reference diagnostic and no LLVM artifact at `-O0`/`-O2`. The corresponding same-named nullable alias compiles and executes with exit 42 at both levels. `zeroed_reference_safety_smoke.sh`, `module_type_alias_namespacing_smoke.sh`, and `diagnostics_smoke.sh` (410/410) pass; shell syntax, whitespace, and Stage1 freshness pass. Product SHA-256 is `c2cd6c2205fabcc0c9f04791edb4f8b6b960e63b1719c760622fe05d6495a11f`; runtime object SHA-256 remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. This resolves the reproduced direct declaration-tree module-alias bypass. `using`/import aliases, qualified lookup relative to nested enclosing modules, reopened/ambiguous namespaces, qualified aliases for other invalid-zero families (`sview`, `id[T]`, aggregates), and universal canonical type identity remain open; the new cases are intentionally Stage1-only until Stage0 behavior is checked.
- **Qualified aliases can no longer hide invalid zeroed borrowed views (compiler commit `4090c332`):** the fresh pre-fix Stage1 (`c2cd6c2205fabcc0c9f04791edb4f8b6b960e63b1719c760622fe05d6495a11f`) accepted a module-qualified alias chain ending in `sview` and emitted LLVM, although a zeroed view has no valid backing. The checker now applies the module-aware alias path to sview classification, follows local alias chains, respects nullable wrappers as valid absent values, and gives local shadowing precedence when resolving same-spelled aliases. The negative qualified view alias rejects with the valid-backing diagnostic and no LLVM at `-O0`/`-O2`; a nullable alias twin compiles and executes with exit 42 at both levels. The expanded zeroed-reference smoke, the full sview representation smoke, module alias namespacing, diagnostics (410/410), shell syntax, whitespace, and Stage1 freshness all pass. Product SHA-256 is `52fdda17191c7adda1d38dd56cc0616e08b61936da3d379fd8afc2d74385ab71`; runtime object SHA-256 remains `b00c85c0df1454efdd4fbb5f2ea7cbcdb679b663ae5f71b72753806972205fc6`. This closes the tested direct module-alias `sview` shape, not imported/relative aliases or other invalid-zero families. Code review of the two focused checkers also makes the next work item clearer: consolidate them behind one resolved representation classifier so alias identity, optionality, cycles, and module context cannot drift between references, views, handles, and aggregate fields.
- **Worktree gains reconciled:** the compiler snapshots at `4f3f7354` and `f7edb529` and Core snapshots at `a3f3ea3d` and `ec851a72` were clean and had no commits absent from their respective main branches (compiler `9c0df597`, Core `2fd6766d`). The proof project’s current uncommitted pins name `e61d5943` and `2fd6766d`, both available from main. After confirming there were no open processes or active source-path references, the four redundant worktrees were removed; the proof project’s other uncommitted edits were left untouched.
- **Core zero-representation analysis now fails closed for unknown semantic types:** commit `5fffd6be` replaces the previous implicit “no known invalid leaves means valid” default with explicit classification of supported scalar, aggregate-header, enum, constant-enum, and error-union representations. Nil, opaque, typed-nil, and unclassified forms are rejected as zero-validity proofs and treated conservatively by the sview walk. Regression tests cover an unknown custom `Type`, an unknown builtin, a typed-nil `*DArrayType`, and valid empty view/darray controls. `go test ./src/semantic -count=1` and `go test ./src/backend -count=1` pass; `git diff --check` passed at the source milestone. A current-source CLI build embeds revision `5fffd6be`. The targeted `TestRunCLIStoreSurfaceOverDArray` still fails before runtime execution on four existing standard-runtime initialization diagnostics (`InlineVec[T,N]` at `collections.elisa:326`, and generic pool slots at `heap.elisa:189`, `:244`, `:288`); it reports no Store conformance mismatch. This is stronger unknown-layout containment, not a complete representation proof, CFG initialization analysis, or runtime migration.
- **Generic zeroed-reference substitution requalified on the fresh Stage1:** after the `0a1def1a` seed, the generic-field bypass repro fails at both `-O0` and `-O2`, with the specific `pointer` field diagnostic and no LLVM file. The nearby `zeroed_generic_scalar_field.elisa` positive control compiles and returns 42 at both optimization levels under the 2 GiB Stage1 RSS cap. This supersedes the older `cc2f7a...` checkpoint's statement that this exact `RefBox[T]` instantiation was still accepted; it qualifies only this struct-field substitution boundary, not enums, all generic aliases, imported identity, other value boundaries, or CFG initialization. S04/S10/S15 remain open.
- **WASM facade width coverage and safer parity execution:** commits `9df7d12b` and `95dd8921` add edge-width parity coverage and signedness/conversion checks for i8/u8/i16/u16/i32/u32/i64/u64. Commit `9b8ba316` makes the compiler-vs-Python artifact test require `ELISASCRIPT_VALIDATION_REAUTHORIZED=1`, an explicit Stage1 RSS cap no greater than 2 GiB, and this checkout's fresh Stage1 binary. Commit `99624055` replaces the platform-specific timeout executable with a Python supervisor that launches each command in a fresh session, samples combined process-tree RSS, bounds each command to 300 seconds and captured output to 64 MiB, and terminates the process group on limit or signal. Commit `1bc77526` writes generated JSON/JS/type sidecars only when exact UTF-8 bytes differ, preserving unchanged timestamps and deterministic line endings. Commit `123232ea` adds unit coverage for the supervisor's cap validation, process-tree RSS accounting, malformed snapshots, graceful and forced process-group termination, status normalization, and fail-closed reauthorization; it also fixes malformed long RSS input classification. Commit `0a1def1a` ports compare-before-write into the self-hosted Wasm emitter and adds a second-build timestamp assertion to the bounded parity smoke. `python3 test/parity/wasm_build_unit.py` passes all 43 tests. `bash -n`, AST parsing, both no-reauthorization exit-125 guards, and current Stage1 freshness pass. The newly seeded product requalifies the generic invalid-reference negative and safe scalar positive controls at `-O0`/`-O2`; the self-hosted sidecar timestamp behavior still lacks runtime evidence. The parity runner compares four fixtures, including the new width-edge case, across Wasm and all sidecars. The full artifact comparison remains pending because its explicit execution gate was not opened; do not count unit tests or the supervisor's guard as artifact parity evidence.
- **Core-IR LLVM lowering now constructs only valid handles:** compiler commit `89fb9b27` moves local-slot construction into the `has_local` path and selects the arithmetic result through an exhaustive `I64BinaryOperator` match, eliminating `zeroed` LLVM handles and reads the analyzer could not prove initialized. A fresh a3-based Stage1 seed produced product SHA-256 `53ea75a33843029b02af128dcd8898361eee7ad246b23758e0f839a38fdf904c`; the runtime object remains `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`, and `assert_stage1_fresh.sh` passes. Ten native compile/link/run probes cover local-plus-add and literal add/subtract/multiply/divide at `-O0` and `-O2`; each exits 42. A strict-Core seed recheck removes all five diagnostics previously reported in `codegen_core_ir.elisa` and reduces the global error list from 220 to 215 (99 opaque-handle zeroing, 116 uninitialized reads). This is a narrow internal-lowering correction, not bootstrap completion or general CFG initialization proof.
- **LLVM target-machine pointer ABI is now checked end to end:** compiler commit `facd341d` models LLVM target and machine handles as raw `void&` pointers inside the backend, with nullable C returns/out cells represented as one-word `void&?` storage and presence/status checked before use. Passing `&LLVMTargetRef?` or returning `LLVMTargetMachineRef?` was not ABI-safe because those opaque optionals are tagged; a first attempt reproduced success-with-an-absent-handle, and an intermediate typed-slot adaptation reached LLVM with an invalid target-machine pointer. Follow-up commit `c91b199c` also seeds both metadata operand arrays with valid `LLVMValueRef` handles instead of `zeroed`; commit `4d394959` gives the enum, sview, and cstr OR-pattern accumulators a valid LLVM false constant instead of `zeroed`. The final source passed a fresh seed from proof-pinned a3 Stage0, rebuilt the runtime object, and passes `scripts/assert_stage1_fresh.sh`. `target_machine_ffi_smoke.sh` passes default-native O0 compile/link/run (42), explicit `x86_64-unknown-linux-gnu` O2 object emission, and the checked invalid-triple error path. `pattern_predicate_initialization_smoke.sh` returns 42 at O0/O2 across enum, sview, and cstr OR-pattern paths. Product SHA-256 is `4d57ba628fde95e5f5f7df365d7a57516a71178251234070f9f483dab90dd182`; runtime object SHA-256 remains `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`. The latest strict Core probe has no initialization/read diagnostics in `codegen_target_machine.elisa` or `codegen_pattern_tests.elisa` (the current overall count is 204: 95 invalid zeroed opaque handles and 109 uninitialized reads). This proves only these target-machine calls and tested targets; raw `void&` handle erasure, optional layouts across other FFI declarations, target-machine disposal/lifetime, and broader C ABI contracts remain open.
- **Opaque-handle zeroing classification corrected:** Core `c1cdd77f` now treats `OpaqueType` as unable to contain an inline `sview`, while `zeroedInvalidRepresentationCount` still treats a non-null opaque handle as invalid in safe code. `26f1bf92` adds tests for both the accurate diagnostic and the trusted-boundary exception. `go test ./src/semantic -count=1`, `go test ./src/backend -count=1`, and the focused `go test ./src/semantic -run 'TestOpaqueHandleZero|TestZeroed' -count=1` pass. The source-built CLI at `c1cdd77f` confirms that opaque placeholders must migrate to explicit nullable/validity state; at that checkpoint the 116 remaining uninitialized-read sites required separate proof of all-path initialization. This changes classification and error quality, not the safe-code zeroing rule or the compiler bootstrap outcome.
- **Inactive LLVM runtime-hook slots now contain valid handles:** compiler commit `7cb1a319` replaces `zeroed` fallbacks for disabled trace functions, the inactive AoS record helper, and the inactive packed-prefix helper with a valid LLVM null-pointer constant handle. These entries remain in the fixed `Runtime` layout, and mode guards prevent calls through the inert entries. New `runtime_inactive_handle_smoke.sh` compiles and runs the trace probe, retained-read packed-index fixture, and packed AoS fixture at `-O0` and `-O2`; all six return 42. `target_machine_ffi_smoke.sh` and `pattern_predicate_initialization_smoke.sh` also pass on the current fresh product, as do `bash -n`, `git diff --check`, and `scripts/assert_stage1_fresh.sh`. A strict seed using source-built Core `c1cdd77f` still stops before publication, but its initialization/read diagnostic total drops from 204 (95 invalid zeroed, 109 uninitialized) to 197 (88 invalid zeroed, 109 uninitialized); the seven eliminated zeroed handles were in this hook cluster. At this checkpoint, the only two remaining diagnostics in this file were `LLVMDIBuilderRef` and `LLVMMetadataRef` debug placeholders; commit `27646dc3` supersedes that state with explicit optionals.
- **Debug builder and file now have explicit absence state:** compiler commit `27646dc3` makes the runtime fields `LLVMDIBuilderRef?` and `LLVMMetadataRef?`, initializes them to `null`, and unwraps both before calling the C API or finalizing the builder. The debug-variable handle is now initialized directly from one of its two LLVM constructor calls instead of `zeroed`. `debug_handle_initialization_smoke.sh` emits `-g` objects at `-O0` and `-O2`; `llvm-dwarfdump` confirms compile-unit and subprogram DIEs at both levels and parameter/local DIEs at `-O0`. The fresh product seeded from proof-pinned a3 Stage0 has SHA-256 `089b4b2701960591ae86e237f8706b20462599d8fbe196224872f58c71c1f949`; the runtime object remains `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`. Debug, inactive-hook, target-machine, and pattern-initialization smokes pass. The strict Core-source probe drops to 194 initialization/read diagnostics (85 invalid zeroed, 109 uninitialized); none remain in runtime declarations, DWARF, or module finalization. One separate `LLVMBasicBlockRef` placeholder and the rest of the compiler-wide findings remain open; this does not certify every debug C API handle or the complete backend.
- **Other Core candidate gains integrated and redundant checkout removed:** the unregistered `worktrees/elisa-core-stage0-or-pattern` clone had five commits beyond its remote tip. Its recursive nested-container walk and struct visibility parsing are already superseded by the current Core source (which supports nested containers and actual private/public field sections). The missing nested-reference region inheritance, call-expression region cache update, and analyzer-state save/restore are committed as `12e8eaa0`; the scalar/string or-pattern single-scrutinee lowering is committed as `60267978`. Semantic, backend, and parser package tests pass uncached, with focused IR tests checking concrete hidden arena lowering and one call per side-effecting match scrutinee. The candidate's older end-to-end runtime test was not retained because this checkout's standard runtime still trips the known generic initialization diagnostics; backend IR coverage exercises the changed lowering. After comparing candidate files and confirming no open users, the redundant clone and stale two-file `worktrees/elisa-compiler-nw` snapshot were removed. Proof-pinned toolchain checkouts remain active dependencies.
- **Generic sview region provenance through growable storage now has layered regression coverage:** Core commit `d6f9b77a` adds an LLVM-IR test for a `sview @r` stored in a `StringHandle[@r]`, returned through a generic accessor, and pushed into `darray[sview] @r`; it asserts the accessor, first allocation, and reallocation all receive the same hidden arena pointer. Commit `5793517d` retains the end-to-end runtime scenario as a focused test that skips only while the four known standard-runtime initialization diagnostics block compilation: the uninitialized `InlineVec` output at `collections.elisa:326` and invalid generic `zeroed` pool slots at `heap.elisa:189`, `:244`, and `:288`. The targeted LLVM-IR regression and full Core backend package pass; the runtime regression currently reports that explicit prerequisite skip. This verifies allocator routing in emitted IR and preserves a runnable runtime gate, but does not yet prove execution of the scenario. Resume the runtime check after the `MaybeUninit`/validity-state migration removes those blockers.
- **Prior stricter-Core bootstrap probe (compiler main `4d394959`):** a bounded seed attempt using source-built Core `c1cdd77f` (binary SHA-256 `3017801981299ecdb09ca864e5070b40a8b8526e3aa7b0dfea9582777c2dc88e`) failed before product publication. The log `/tmp/elisac-core-pattern-predicates-strict-20260925.log` (SHA-256 `f1f1cd5748caaae4115b69c8c289bc1c22d1da4c8d1584b5188728b12cac467a`) has 204 initialization/read diagnostics: 95 invalid zeroed opaque-handle initializers and 109 uninitialized reads, eleven fewer than the previous Core-IR checkpoint (215: 99 and 116). The target out-handle zeroing/read issues, both metadata-operand zeroed handles, and all seven OR-pattern accumulator findings are gone; the strict probe reports no initialization/read diagnostics in either `codegen_target_machine.elisa` or `codegen_pattern_tests.elisa`. Safe non-null opaque zeroing remains rejected, and the focused Core tests confirm that only an explicit trusted unsafe boundary admits raw opaque zeroing. The safe temporary bootstrap still uses proof-pinned Core Stage0 `a3f3ea3d` under the 5.5 GiB ceiling and produces the recorded Stage1 product. The narrow Stage1 correction uses raw `void&`/`void&?` at the LLVM C target-handle boundary because opaque optional values are not proven ABI-compatible with C pointer slots or returns. Replace this type-erased TCB boundary with a specified and tested foreign-handle/out-parameter representation, then analyze remaining read sites individually. Do not globally initialize pointers, add broad unsafe grants, or weaken fail-closed rules to obtain a seed.
- **Earlier worktree integration and retention snapshot (superseded by the latest reconciliation above):** compiler main contains the committed gains from compiler candidates; proof-pinned compiler checkouts `f7edb529` and `4f3f7354` remain because the proof project and historical audit still depend on them. The proof-pinned Core `a3f3ea3d` worktree also remains active; current Core main `5793517d` is seven commits ahead of that pin. The standalone Core candidate clone, stale `worktrees/elisa-compiler-nw` snapshot, and linked `stage0-region-sview-darray-fix` checkout were audited, their useful regression coverage integrated or confirmed superseded, and the candidate directories/branch removed after no-open-user checks. The linked candidate's only additional source diff was an uncommitted environment-triggered debug print in darray growth-owner selection; it changed no semantics and was not retained. `/private/tmp/elisac-engine-validation-v2` was previously removed after its process/open-file checks. At that earlier checkpoint, the clean proof checkouts were retained while the pin migration was still pending.

- **Field-refinement and zeroed safety gates on the latest product:** `struct_field_refinement_shadow_smoke.sh` rejects construction from a same-named loop shadow and invalid stores/updates/calls/returns while accepting proven refined values; `zeroed_reference_safety_smoke.sh` passes its invalid reference, aggregate, and safe-positive cases at `-O0` and `-O2`. The tests pass on the latest Stage1 product after the checker binder rename. They establish the covered cases only; stable binding identity and path-sensitive transfer/join rules remain required for S04/S15.
- **Empty darray sviews and checked C-string growth:** Core commit `a3f3ea3d` and compiler commit `9042aae1` now choose a process-lifetime NUL byte when `darray[u8].as_sview()` sees count zero, so the non-null `sview.data` contract holds even when an empty darray has a null items pointer. `sview_darray_backing_smoke.sh` reads that byte and returns 42 on Stage0 and Stage1. Review of Stage1 string-view lowering also found that `as_cstr()` computed `count + 1` with wrapping arithmetic; commit `c64de2cc` added a checked-add regression, and `f7edb529` routes the overflow branch through `emit_trap_unreachable` to call `llvm.trap` before `unreachable`, matching Core's checked trap. `darray_cstr_checked_growth_smoke.sh` requires the overflow intrinsic, the overflow block, and an explicit trap in both generated modules. `go test ./src/semantic ./src/backend -count=1`, Stage0/Stage1 freshness checks, both new string-view smokes, `atomic_invalid_order_smoke.sh`, `sview_region_tie_smoke.sh`, and `sview_relocation_smoke.sh` pass. These checks cover the empty sentinel and generated overflow branch; they do not replace a full darray representation audit, prove safety against headers corrupted by unsafe or foreign code, or close general alias, mutation, and concurrency rules.
- **Stage1 atomic lowering:** with the then-fresh Stage1 product, direct emission of `atomic_runtime_operations.elisa` and the runtime-atomics codegen fixture produced LLVM `load atomic`, `store atomic`, integer and pointer `atomicrmw`, bool/float atomic accesses, pointer `cmpxchg`, and `fence seq_cst`. The real four-thread CAS increment executable returned success at O0 and O2. `atomic_invalid_order_smoke.sh` passed at both levels, including the negative `AtomicSlot` raw-access case, invalid-order cases, a different-signature `i32` overload control, and a concrete user-defined `load(AtomicSlot[i64]&, MemoryOrder)` that returns 42. The smoke now runs the user function without forcing the runtime-stdlib exemption. This validates the exercised direct overload only.
- **Closed concrete atomic miscompile, generalized identity still open:** `test/repro/atomic_same_named_slot_user_function.elisa` defines a top-level user `AtomicSlot[i64]` and ordinary `load` returning 42. A prior product emitted an atomic load from the field and returned 0 at O0/O2. The Stage1 backend now yields to a concrete exact-signature user overload before intrinsic lowering; the fresh product compiles and returns 42 at both optimization levels. This is a tested containment fix, not full identity resolution: atomic slots and intrinsic functions are still recognized partly by source names/signatures, so generic overloads, namespace shadowing, imported same-name declarations, and the Go backend need canonical resolved identities before S01/S17 can close.
- **Atomic milestone commits:** Core commit `06bb1c0f` recognizes `AtomicSlot` in its semantic payload checks and LLVM operation lowering and adds focused semantic and IR tests; `go test ./src/semantic ./src/backend -count=1` passes. The compiler-side `atomic_invalid_order_smoke.sh` passes at O0/O2 on current Stage1 product `96568fa4…` without a freshness bypass, including raw-slot rejection and the exact ordinary-function control. The Go backend still chooses atomics partly from call spelling and slot type name, and the same-name ordinary-function control has not yet been carried over; keep resolved intrinsic identity as an open blocker even after the compiler-side containment commit.
- **Historical worktree reconciliation:** `/private/tmp/elisac-engine-sync-validation` contained no gain absent from compiler main: its AtomicSlot and region-return edits match committed main changes, and its only extra method-name whitelist entry (`as_str`) has no corresponding API. After confirming no process or open file used that checkout, it was removed. The proof toolchain at `build/toolchain/elisa-compiler-4f3f7354` remains because `elisa-proof/AUDIT.md` names that exact pinned compiler/runtime path; deleting it would remove a live audit dependency, not a redundant candidate checkout. Current registered checkout status is recorded in the active continuation above.
- **Core focused/full Go tests:** commit `88cea3f5` adds the resolved zero-validity walk and whole-binding invalid/uninitialized state. The latest `go test ./src/semantic -count=1` and `go test ./src/backend -count=1` pass, including a fail-closed regression for unresolved generic layouts. `go test ./... -count=1` completed red in `elisacore/src`, `elisacore/test/backend`, and `elisacore/test/semantic`. Hundreds of CLI/integration failures share four default-runtime diagnostics: `collections.elisa:326` returns an `InlineVec[T,N]` whose fixed `T[N]` slots are inactive but may have invalid representations; `heap.elisa:189`, `:244`, and `:288` zero arbitrary `T` pool slots. These expose the need for a language/runtime uninitialized-storage abstraction with explicit initialization state; broadening unsafe exemptions or declaring arbitrary `T` zero-valid would conceal a real representation hazard. Separate semantic/backend fixture failures also exercise direct `StringView` construction, `cstr[shape]`/`u8&` bridge mismatches, and generic `T = zeroed`; each needs a safe constructor/ABI/API correction or an updated intentional-negative expectation. The Stage0/Stage1 safety smoke and runtime-object build are useful narrow evidence, not full runtime acceptance.
- **Verified generic zeroed analysis:** before the latest checker fixes, a reused cycle-visited set skipped ordinary aggregate fields, owner filtering let unrelated generic fields taint an instance, and a failed generic-name lookup substituted every nonmatching field identifier with the last type argument. The checker now resets per-walk cycle state, recurses only through fields owned by the selected struct, and uses an explicit no-match sentinel. `zeroed_reference_safety_smoke.sh` passes against the latest Stage1 product at `-O0`/`-O2`, covering direct and aliased references, globals, returns, generic/ordinary/nested fields, tuples, fixed arrays, `sview`, and positive initialized/nullable/generic-scalar controls. A nested phantom-generic scalar fixture specifically prevents the former false-positive regression. These checks remain syntax/declaration-oriented containment, not universal representation analysis or complete path-sensitive definite assignment.
- **Committed self-hosted containment milestone:** compiler commit `34c52dde` extends the source checker to return values, generic substitutions, nested by-value aggregates, arrays, tuples, and `sview`, adds safe positive twins, and migrates compiler-runtime placeholders that would otherwise be invalid under the new checks. The paired smoke passes on fresh Stage0 and Stage1 at `-O0`/`-O2`; compiler Stage1 freshness and runtime-object generation pass. The commit is containment only: its checker is still syntax/declaration-oriented and the Go full suite exposes uninitialized runtime storage that needs a first-class validity abstraction.
- **Definite-assignment gap and containment:** the Core candidate previously accepted an omitted `mutable i64&` local read and could treat address escape or a projected/write-through store as initialization. The candidate now seeds omitted locals and invalid `zeroed` locals, checks reads and writes through an invalid reference, and clears the state only on whole-binding assignment; address-taking, unknown callees, and projected writes do not prove a complete value. New tests also retain the safe case where a zeroed scalar reference is rebound to a valid reference before use. `go test ./src/semantic -count=1` and `go test ./src/backend -count=1` pass. This is still a coarse whole-binding state, not field-sensitive CFG dataflow or a proof of external callee writes; S04 remains open.
- **Bootstrap boundary:** an up-to-date Core Stage0 now seeds the current Stage1, and the Stage1-built runtime object completes after the compiler’s generic zeroed-walker fixes. This closes the immediate bootstrap blocker for this tested source pair only. `trusted Unsafe.PointerCast` still permits inactive generic payload storage, so an explicit validity/initialization abstraction and state-guarded reads remain required; the runtime wrappers are not a soundness closure by themselves. The uncached Core Go suite has now been measured and is red on the runtime and fixture families listed above; overall acceptance gates remain open.
- **Store conformance versus Store borrow safety:** an earlier seed error reported a region-signature mismatch after an explicit `@r` annotation experiment; those annotations were reverted. The latest current-source seed succeeds without that mismatch, and the focused Core `TestRunCLIStoreSurfaceOverDArray` currently emits only the known standard-runtime initialization diagnostics, not a Store signature diagnostic. Store is therefore not the current bootstrap blocker. Keep the actual borrow contract open: `store_get` returns an element reference, Deque stores backing through an independent nullable reference, and darray growth may relocate storage. Before calling the protocol safe, add owner/loan tracking for the actual backing allocation and prove that a returned element borrow cannot outlive growth, replacement, destruction, or region reset. Add paired positive/negative tests for darray and Deque; do not treat signature conformance or a successful seed as lifetime proof.
- **Unknown generic layouts fail closed:** review of the just-added Core validity walker found that a `GenericInstanceType` without a resolved struct layout fell through as zero-valid. The walker now treats an unknown/non-struct generic layout as unsafe for zero construction and as potentially containing an `sview`. `TestZeroedUnknownGenericInstanceFailsClosed` covers unresolved and opaque generic layouts; latest focused semantic and backend packages pass. Instantiated resolved structs still recurse through substituted fields. This avoids using missing layout information as a safety proof; all generic enums/aliases, imported layouts, and instantiated representation assumptions still need complete coverage.
- **Runtime migration remains the next S04 gate:** use safe optional storage only where it preserves the element ABI; otherwise introduce a first-class `MaybeUninit[T]`/validity-state abstraction whose raw storage cannot be read, borrowed, dropped, or returned as `T` until initialization is proven. InlineVec’s `T[N]` backing is exposed through contiguous `view[T]`, so replacing slots with tagged `T?` is not layout-preserving and must not be attempted as a superficial fix. The pool create/free paths must stop returning arbitrary zeroed storage as `T`; redesign them around an initialization token or constrained zero-valid type. Any trusted raw storage remains an explicit narrow TCB boundary with audited state transitions.
- **Atomic identity remains open:** `atomic_invalid_order_smoke.sh` now passes the exact same-signature user `load` reproducer (returns 42 at O0/O2), raw-slot rejection, invalid-order controls, and ordinary overload controls. The self-host backend’s concrete-overload guard is syntactic and the Go backend still dispatches using function/slot names; carry the ordinary-function control into Core and replace name/signature matching with semantic intrinsic and nominal type identities before closing S01/S17.

### Gains to retain and build on

| Area | Integrated gains and existing evidence | What the gains do not yet establish |
|---|---|---|
| Identity and control flow (S01–S03) | Declaration/binding IDs, lexical region type identities, Core CFG terminators and malformed-graph rejection, plus AST statement-disposition coverage. | A shared typed place/loan model, complete qualifier substitution, persistent identities, or full expression/cleanup semantics. |
| Ownership and lifetimes (S04–S08) | Generic affine-owner classification, reference reborrow checks, region scope/reset/rewind tracking, helper/callback invalidation, backing aliases, and aggregate/closure-held view dependencies. | Complete alias-sensitive fixed-point analysis across branches, loops, indirect calls, and separate compilation. Conservative name-based passes remain migration targets. |
| Runtime and representations (S09–S13) | Checked arena/fixed-buffer/string arithmetic, immutable valid string-view boundaries, packed-store row guards, and serialization of shared arena-cache metadata. | Whole-runtime failure atomicity, generation exhaustion, complete cleanup edges, same-arena thread safety, or allocator-object sanitizer coverage. |
| Authority and invariants (S14–S16) | Scoped unsafe grants, cast/reference capability checks, private-constructor protection, nullable flow checks, and interval-based field refinements across zeroed construction, stores, returns, Match/Catch, and selected generic/direct calls. Module/type shadowing and fallible field assignment fixes also have focused Stage1/Gen2 evidence. | Universal zero-validity, resolved call/alias coverage, general proof entailment, or preservation through every invariant-bearing boundary. Stage0 acceptance is not a reason to restore a demonstrated hole. |
| Atomics (S17) | Native lowering, nominal MemoryOrder checks, invalid literal-order rejection, pointer/bool/integer coverage, threaded CAS tests, and a committed Stage1 fix for the concrete exact-signature same-name user overload. | Resolved runtime type and function identity across generic overloads, modules, imports, and Core; faithful dynamic-order behavior; all pointer result bindings; race-free safe APIs; or supported-target qualification. Runtime experiments that rename `atomic[T]` to `AtomicSlot[T]` must update semantic fencing and backend intrinsic identity together. |
| Error sets and native ABI (S02/S18/S22) | Composite selection/tag metadata and catch lowering, restricted expression-catch coverage, direct-register C status adaptation, and typed hidden-sret calls. The latest recorded native suite passed 521/521; real C stubs passed LLVM/object O0/O2 cases on arm64. | All indirect calls, generic/module layouts, byval/contract interactions, C union layout agreement, other architectures, or current-head bootstrap fixpoint. |
| Artifact integrity and verification (S25–S28) | Mandatory-gate and partial-output fixes, targeted safety regressions, recorded source/product hashes, and several native Gen2 validation milestones. | A current uncached full pass, sanitizer qualification, or a complete reproducible build manifest. The failed full run in M.5 remains historical evidence requiring remeasurement. |

### Earlier follow-up audit — 2026-09-25 (historical snapshot, superseded above)

At the time this snapshot was first written, the newer compiler product was stale and the generic aggregate/return extensions had not yet been exercised. The latest continuation above supersedes that status: fresh paired Stage0/Stage1 products now pass the expanded zeroed-reference smoke at `-O0`/`-O2`, and the current compiler product is recorded in the baseline. These source-oriented checks still do not replace the typed CFG initialization and universal representation analysis required by S04/S10/S15.

The canonical Elisa-core checkout has committed the companion zeroed-validity checker and whole-binding definite-assignment state in `88cea3f5`, covering resolved references, aggregate fields, generic instances, arrays, tuples, enums, and type parameters. Focused semantic/backend Go packages and the current self-host/bootstrap smoke pass. The complete Go suite remains red on the concrete runtime and fixture families listed in the latest checkpoint. This is a committed containment milestone; it does not close runtime migration or acceptance gates.

The compiler and Core runtime copies carry the `AtomicSlot[T]` wrapper: `atomic_cell[T]` constructs initialized slots and cells. Follow-up edits include `AtomicSlot` in the self-hosted raw-operation fence, payload check, reference binding classification, and LLVM lowering, with corresponding Core type-safety, raw-operation, and backend handling. Stage1 atomic IR/runtime/order/raw-access checks and focused Core tests pass for their covered cases, but `atomic_same_named_slot_user_function.elisa` still demonstrates name-based intrinsic mislowering at both O0 and O2. Nominal primitive identity remains an open blocking S01/S17 requirement; same-spelled user types/functions must not acquire intrinsic lowering accidentally.

The expanded zeroed-reference smoke passes on paired fresh Stage0 and Stage1 products for the previously covered direct/alias/global/return boundaries, nested generic and ordinary fields, tuples, fixed arrays, `sview`, and positive initialized, nullable, and scalar-generic cases. Stage1-only extensions now cover module-qualified non-null reference and borrowed-view alias chains, same-spelled aliases with distinct module identities, storage wrappers, and nullable twins. The focused checkers still duplicate alias-chain traversal and do not establish universal checks for every assignment RHS, call argument, aggregate/enum constructor, imported or `using`-aliased module boundary, relative nested-module lookup, resolved shadowing case, or AST form. Consolidate this into a canonical resolved representation query shared by all invalid-zero families before claiming broad alias coverage.

The detached proof-toolchain checkout remains clean at `4f3f7354`, an ancestor of compiler main, and is still referenced by the active `elisa-proof` pin/build. It has no unique compiler source gains and cannot yet be removed without first advancing and validating that proof dependency.

### Revised execution order and closure criteria

1. **Contain confirmed invalid-value access first (S04/S10/S15).** A historical Stage1 product accepted the following source at O0 and emitted `store ptr null` followed by `store i64 42` through that loaded pointer:
   ...
   Product SHA-256: `efcc69e059302f678c126c9457ac8ede55c0c67195ff8a9542e1814ee9b04c83`. The absolute external product path was outside the scope of `assert_stage1_fresh.sh`, so this is historical IR evidence only, not a claimed runtime or Stage0 test; the exact source now rejects in the fresh strict-seeded product recorded below. `check_uninitialized_zeroed.elisa` explicitly excludes non-null references and treats assignment/address-taking anywhere as initialization for its limited return check. Continue enforcing invalid representations at reads, writes through references, calls, returns, globals, aggregate fields, aliases, and generic substitutions. If staged initialization is supported, require initialization on every incoming path; taking an address alone proves nothing about a callee initializing it. Include conditional/loop initialization, shadowing, omitted initializers, nested aggregates, valid nullable-null and valid initialized-reference twins. Audit/migrate `arena_take_free_block`'s temporary reference and the scoped reference-rebind fixture without simply granting blanket unsafe authority. Rejection of invalid construction is an acceptable interim containment; it must not be described as complete definite-assignment analysis.

   **Current containment status:** the original local repro is rejected before LLVM output. Direct and top-level alias-chain locals, direct globals, omitted local initialization, direct/generic reference returns, nested generic and ordinary struct fields, named tuples, fixed arrays, `sview`, and the new direct `Module::Alias` non-null-reference and borrowed-view paths are covered. Qualified tests exercise module-local chains, same-spelled nullable/non-null aliases in separate namespaces, storage wrappers, and nullable twins. Positive controls also include initialized references/reference-bearing aggregates and nested/phantom generic scalars. `arena_take_free_block` no longer seeds a non-null temporary reference from `zeroed`; its split header reference is constructed from the already checked slot offset inside its existing narrow unsafe helper. The current checker also rejects unresolved/non-struct generic layouts rather than assuming they are zero-valid. This closes only the measured representations. Require one canonical, module-aware alias/type identity and shared representation classifier for all invalid-zero checks; do not fall back to basename matching. Keep S04/S10/S15 open for imports and `using` aliases, relative nested-module paths, generic enum/alias and every invalid representation, all expression boundaries and AST forms, complete uninitialized read/drop/call checks, and path-sensitive initialization.

2. **Close ABI adapter interactions before widening its support claim (S18/S22).** Preserve the native call instruction separately from the normalized status value: `emit_error_status_call` can return a load, while `emit_direct_call_indexed` later applies byval call-site attributes to `call_result`. Add real-C regressions combining direct/sret error status with indirect aggregate parameters. Separately audit checked extern postconditions: the direct-call path loads the success-output slot before an explicit success branch, but a failing foreign callee need not initialize that slot. Define success/error contract semantics and only inspect initialized values on the applicable edge. These are source-inspected hazards, not reproduced runtime failures in this review. Test failure with an untouched/poisoned success slot, success with a valid result, catch/try/try-else, void results, split views, and function-valued calls; require LLVM verification and O0/O2 runtime evidence. Specify a versioned C status layout rather than assuming Elisa's concatenated variant fields match an arbitrary C tagged union. Also preserve exact pointer ABI for LLVM/C opaque handles: the target-machine smoke covers one corrected out-parameter/nullable-return path, but does not certify optional opaque handles or other extern declarations globally.

3. **Complete atomic identity and dynamic-order semantics (S01/S17).** Bind lowering to the resolved runtime primitive, including same-name/same-signature user-function controls. If an internal slot type is renamed, update builtin/type classification, raw-operation fencing, payload restrictions, native lowering, runtime signatures, and tests in both Stage0/Core and Stage1 in one integration slice; require IR evidence that every operation still emits the intended atomic instruction. Define nonliteral order dispatch and invalid runtime-order handling; a blanket SeqCst fallback is not evidence of implementing the requested order semantics. Test the success/failure CAS order matrix, consume old pointer values at runtime, and distinguish native execution from IR-only target evidence.
...
- [ ] Replace timestamp-only build evidence with a content manifest covering compiler inputs, standard library/runtime, target, flags, compiler generations, and relevant tool versions. `assert_stage1_fresh.sh` currently skips missing binaries, relative paths, and products outside the absolute repository `bin/` prefix; a success from that helper alone cannot authenticate Gen2 or another checkout. Verify binary existence and manifests independently, and normalize intended product paths. A seed resource-guard stop is a failed build attempt, not semantic evidence; an external seed's output needs independent identity verification before reuse.
- [ ] Add explicit ABI/atomic coverage to the relevant fast feedback profiles, or document exact focused commands required alongside them. `run_all.sh` already discovers `test/parity/*_smoke.sh` for the full profile, including the new sret/composite/atomic/refinement scripts; do not add duplicate registrations. Its current backend profile patterns omit the `extern_*`, `composite_*`, and `atomic_*` smoke families. Inspect the selected list and report executed/skipped/cached results before claiming a profile covers these gains.
- [ ] Record an integrated source/product manifest for the next full acceptance gate. A strict-bootstrap source/product manifest is now recorded below, but it does not establish full-profile, sanitizer, or cross-backend coverage; prior Gen2 records also predate the newest ABI/atomic changes.
...
**Observed unsafe lowering:** a value-position payload-enum `match` could reach `emit_match_into_slot` directly from declaration/return lowering and bypass the family check at `emit_expression`. A pattern for a same-named variant from another module could therefore request a wider payload load than the scrutinee's declared enum layout. The wrong-owner expression fixture emitted a `{ i64, i64 }` load from storage reserved for `{ i32, [1 x i64] }`; this is a concrete out-of-bounds read in generated code, not merely a diagnostic mismatch.

**Containment implemented and verified:** payload-pattern ownership is checked at the shared value-match lowering boundary as well as statement dispatch. Explicit qualified pattern paths and `is` constructor targets must resolve to the exact enum family; unresolved qualified ownership fails closed instead of falling back to a same-spelled row. Const-enum expression types carry a declaration-local enum identity, and coercion/return lowering now declines when the source identity is absent or differs from the destination. The match and `is` guards use a valid `Expr.Binary` destructure followed by scalar `TokenKind.Is` equality. Existing alias, ancestor, nearest-scope, mixed type-family, deep nesting, packed-layout and const-value positives remain accepted.

**Focused evidence:** `test/parity/enum_module_alias_smoke.sh` passes on the fresh Stage1 product at `-O0` and `-O2`. Thirteen positive programs compile to objects, link and return 42, and their emitted LLVM verifies. Twelve negative programs cover wrong module owners, wrong payload arity in statement/value/nested matches, foreign `is` targets, wrong const-enum owners/widths, and raw-literal/cast enum impostors; both LLVM and object modes reject every case without leaving an artifact. Shell syntax and whitespace checks pass. Product SHA-256: `c1474092a1969008793b5f50d9dc70e4d3ebd98006e60da004c00feddfe17a00`; matching runtime object SHA-256: `dc01d02e488b3095e40d8ee6fda29c4419555eb62aadb7447132278ffaf52284`; seed compiler `/tmp/elisac-stage0-a3` SHA-256: `56a4362179a3072d1b1d7f56c8fb8bbe125d40a070c93d4021df949c497bd89a`. The product includes concurrent working-tree edits; only the focused enum cases above are qualified by this run. Preserve the local row-ID caveat: these identities are compilation-local and do not replace canonical semantic `TypeId`s across imports, caches, or separate compilation.

This closes the reproduced wrong-family payload read and nominal const-enum substitution cases in the exercised Stage1 paths. It does not close enum/state validity, arbitrary pattern resolution, all alternate match lowerings, full type identity, initialization analysis, another backend, or the overall memory-safety goal. Keep S01/S04/S10/S15 and full qualification gates open.

### Verified strict Core bootstrap and compiler-safety follow-up — 2026-09-26

**Committed source snapshot:** compiler HEAD is `e16105ef5e43ec69733c1a903567c0565e0e2ecd` (tree `c4de5edf80b133f2e99399887ce8fa559d84cbbd`). The gains landed in four reviewable commits: `e7c43a4b` replaces invalid zero-bit LLVM handle sentinels with valid typed values or optionals and fails closed before consuming absent results; `eba00ac7` recursively registers nested named-tuple types before lowering enclosing generic containers; `0296daf7` preserves nullable string payload identities in safety checking; and `e16105ef` records same-name refined-parameter shadowing as a regression. The strict-seeded product was built from this exact compiler source content before the documentation-only plan update. No Elisa source or interface file was newer than that product at capture time.

**Bootstrap manifest:** Elisa-core is clean at `45ea9245a5e3e5143fea58fa1ae59d7589249d4d`; the source-built Core CLI `/tmp/elisac-core-current-45ea9245` has SHA-256 `b4970c9595689f140628fbde21cefaa6017816edabbff2a2a470e0bc22ec56de`. The Stage1 product `/tmp/elisac-stage1-current-core-20260926c` has SHA-256 `c79c01a2b7f4424813a4a8e315ee803cd039d29b9ee173788a9d9aa3a5d42c2c`; its generated runtime object `/tmp/elisac-runtime-current-core-20260926c.o` has SHA-256 `dc01d02e488b3095e40d8ee6fda29c4419555eb62aadb7447132278ffaf52284`. It was seeded with `ELISA_STAGE1_SEED_MAX_RSS_KB=5767168` using `ELISACORE_BIN=/tmp/elisac-core-current-45ea9245 bash scripts/elisac_stage1.sh --seed` on Darwin arm64, macOS SDK 27.0, Homebrew clang 23.1.1. Strict Core seeding now completes on the entire current Stage1 source tree; the prior 193 invalid-zeroed/uninitialized diagnostics no longer block publication. The linker emitted an SDK `libSystem.B.tbd` warning, but the seed exited successfully and wrote both products. This is a bootstrap/compiler-internal initialization milestone, not a proof that all user-program representations or paths are safe.

**Focused verification:** with this product and its matching runtime object, `zeroed_reference_safety_smoke.sh`, `sview_representation_safety_smoke.sh`, `named_tuple_darray_smoke.sh`, `enum_module_alias_smoke.sh`, and `struct_field_refinement_shadow_smoke.sh` all pass. The suites exercise O0/O2 acceptance and rejection paths where applicable; the enum suite prints only the existing internal-Arena fixture warnings. `git diff --check` and `bash -n` pass for modified smoke scripts. The named-tuple program returns its expected status at O0/O2; the safe nullable C-string forwarding case compiles; unsafe zeroed references, forged views, wrong enum families, and unproven shadowed field construction reject. These focused checks do not qualify ABI adapters, atomics, other backends, sanitizers, the uncached full acceptance profile, or all CFG initialization cases.

**Plan impact:** the strict Core bootstrap blocker for invalid LLVM handle placeholders is closed for this source snapshot, and the focused representation/test gains are now in main. Continue S04/S10/S15 work on canonical resolved type identity, every constructor/call/assignment boundary, and path-sensitive definite initialization. Continue S01–S08/S15/S23 shared CFG/type/place analysis, S18/S22 ABI interactions, S17 atomic identity and ordering, and S25–S28 full integrated qualification. No safety package is marked complete.

### Verified Stage1 invalid-zero call-argument read containment — 2026-09-26

**Finding and correction:** a fresh strict-seeded Stage1 product accepted `text: cstr = zeroed; return consume_cstr(forward_cstr(text))` and emitted a null pointer argument, while Elisa-Core rejected the uninitialized read. The Stage1 checker previously diagnosed only a direct `return name`. Commit `20e82549` added recursive expression traversal for candidate reads in nested call arguments and returned expressions, including calls nested under other syntax. The follow-up `ad3ff91c` made the limited initialization evidence stricter: only assignment to the whole identifier can clear a candidate; a projected write or `&text` does not establish a valid value. The checker treats `dstr`'s all-zero header as a valid empty dynamic string, matching Core's representation classification and preventing the Stage1 runtime build from reporting false uninitialized reads for normal empty outputs.

**Focused evidence:** a clean in-repository Stage1 product (`bin/elisac-stage1`, SHA-256 `5011bf00047689e704ec4abeaa2d068110f8900b88b662ffea745af92c2410e7`) and matching runtime object (`build/runtime/elisacore_runtime.o`, SHA-256 `dc01d02e488b3095e40d8ee6fda29c4419555eb62aadb7447132278ffaf52284`) were produced from compiler commit `ad3ff91c4edafc4bdbf028a4c32d6a549ffde92f`, tree `5efa5ceb97521a3f0f52daafa0e2c4785b4d4fa4`, with no `.elisa`/`.elisai` input newer than the product. The seed used Core `45ea9245a5e3e5143fea58fa1ae59d7589249d4d`, source-built CLI SHA-256 `b4970c9595689f140628fbde21cefaa6017816edabbff2a2a470e0bc22ec56de`, under the 5.5 GiB cap. `zeroed_reference_safety_smoke.sh` passes against Core and Stage1 at `-O0`/`-O2`: nested cstr forwarding and address-only initialization followed by a read reject before LLVM output; literal initialization and whole-binding assignment controls return 42 on both compilers. Existing reference/alias/nullable controls in the same suite pass. `sview_representation_safety_smoke.sh`, `zeroed_enum_representation_smoke.sh`, and `enum_reference_payload_smoke.sh` also pass on this in-repo product, including the latter's LLVM verification and wasm32 object-emission cases. `bash -n`, `git diff --check`, the wrapper freshness check, and the in-repo Stage1 freshness guard pass.

**Remaining safety obligations after the follow-up below:** the checker now uses resolver BindingIds and ordered facts, but it is still a targeted definite-initialization pass rather than the shared, complete CFG/representation analysis required by this plan. Statement `if` joins are modeled. Statement `match` joins now recognize only an unguarded wildcard or bare binding as a catch-all; incomplete and guarded matches retain the incoming state, while typed enum coverage and expression-level joins remain conservative. Loop bodies do not export writes because a loop may execute zero times. Lambda bodies are inspected against the capture-time facts rather than a complete closure execution/capture model. Opaque callees, including routines that initialize `&value`, have no write summaries, so the checker intentionally does not treat address passing as proof of initialization. Whole-identifier `<-` writes are processed after their RHS reads; compound writes read the old target and cannot initialize it; direct `zeroed` reassignment clears the fact. More complex invalid-value provenance through conditional expressions, casts, nullable/raw operations, imported or generic representation substitutions, and unchecked external contracts remains open. The tracked representation families and declaration-time reference checks are still a subset of the language. `dstr`'s valid zero header does not make its element storage initialized after growth. Full CFG integration, interprocedural summaries, complete representation coverage, the full acceptance gate, sanitizers, and cross-target qualification remain required before any complete memory-safety claim.

### Verified Stage1 invalid-zero source-order and branch-flow follow-up — 2026-09-26

**Finding and correction:** after the call-argument fix above, the checker still scanned an entire function for any assignment and treated that as initialization. A read before a later write and a read after a one-arm-only write were therefore accepted; name-only matching also allowed a same-named shadow to interfere. Commit `5fd40c3f` replaces that shortcut with source-ordered facts for resolver `BindingId`s. Candidate declarations and reads are mapped using the resolver's function owner, source line, name, and local-reference kind. The checker evaluates assignment RHS reads before changing facts, recognizes only direct whole-identifier `<-` writes as initialization, treats compound mutation as a read of the old value, and clears the fact when the binding is assigned direct `zeroed` again. Address-taking and projected writes do not prove a whole value valid. Each statement `if` gets independent branch facts; the join retains only facts true on every path that continues, while a returning arm does not erase facts from the only continuing arm. Loops analyze their bodies against a copy and preserve incoming facts afterward. At this checkpoint, match arms and nested expression blocks use conservative copies rather than exporting unproven writes; the statement-match follow-up below adds a narrow catch-all join.

**Focused evidence:** a fresh Stage1 product from the shared checkout passed `scripts/assert_stage1_fresh.sh`; SHA-256 is `c22d6df1330a8c0de037c0d6eb576bc31f458b1d8e9bc381ff97ca84c22d9fa0`. The runtime object SHA-256 is `dc01d02e488b3095e40d8ee6fda29c4419555eb62aadb7447132278ffaf52284`. `test/parity/zeroed_reference_safety_smoke.sh` passes on Stage1 and Elisa-Core at `-O0` and `-O2`: reads before later assignment, one-branch assignment, assignment of `zeroed` after a valid write, and same-named shadow writes that leave the outer binding invalid are rejected before LLVM output. A direct assignment followed by a read remains accepted, and assignments in both `if` branches compile and return 42 on both compiler generations. Existing invalid-zero reference, borrowed-view, handle, alias, aggregate, and nullable controls in the smoke continue to pass. `bash -n`, `git diff --check`, and the Stage1 freshness guard pass. The product was built from the shared checkout while unrelated parser/backend work remained uncommitted; its hash is a reproducible product snapshot, not a claim that those concurrent edits are part of commit `5fd40c3f`.

**Scope boundary:** this closes only the reproduced source-order and `if`-join misses for the tracked invalid-zero local families. It does not establish complete definite assignment, general reference/borrow safety, sound invalid-value provenance, callee effects, or overall memory safety. Keep the remaining obligations above open and extend the canonical CFG/type-representation work before relying on this subset for a broader language guarantee.


### Verified Stage1 zeroed-read statement-match follow-up — 2026-09-26

**Finding and correction:** after ordered `if`-flow handling, the checker still treated every statement `match` arm as an independent copy and exported no initialization facts. It now snapshots the incoming `BindingId` facts, analyzes each reachable arm from that same snapshot, and intersects the facts from arms that continue past the match. Only an unguarded wildcard or bare binding proves a catch-all. Without one, the incoming state participates in the join to represent unmatched values or failed guards; arms after a catch-all are skipped, and returning arms do not weaken facts from paths that continue. Enum-family coverage, other structured patterns, guarded catch-alls, and expression-level matches remain conservative until resolved coverage and shared CFG facts are available.

**Focused evidence:** commit `df0b1b63` adds two negative fixtures (a write in only one arm and a guarded catch-all whose guard may fail), plus positive fixtures where both arms initialize and where a returning arm leaves only an initializing continuation. The complete `test/parity/zeroed_reference_safety_smoke.sh` passes on the fresh Stage1 product and Core controls at `-O0`/`-O2`; the positive fixtures return 42. The Stage1 product SHA-256 is `b07efc3138488b18b16744fc826300a43eece04bd135a95c60e7df397295659d`, and the runtime object SHA-256 is `dc01d02e488b3095e40d8ee6fda29c4419555eb62aadb7447132278ffaf52284`. `scripts/assert_stage1_fresh.sh`, `bash -n test/parity/zeroed_reference_safety_smoke.sh`, and `git diff --check` pass.

**Scope boundary:** this is a narrow statement-match join for the tracked zeroed-local candidates, not complete pattern exhaustiveness, expression-level flow, initialization analysis, or memory safety. Keep loop/closure/callee summaries, generic and imported representation identity, all read/write boundaries, canonical type facts, full gates, sanitizers, and cross-target qualification open.
...
| B17 | Stage1 `optional_is_niche` does not classify every opaque C handle as a pointer niche; LLVM target-machine APIs use pointer typedefs for opaque handles, while the first `LLVMTargetRef?` out-slot experiment behaved as a tagged optional and did not match `LLVMTargetRef*`. The Stage0 full-source IR also represented a `cstr?` error-out cell as a tagged optional, not `char**`. `test/parity/target_machine_ffi_smoke.sh` now covers the corrected raw-pointer path, while later strict-seed evidence removed the two output-array placeholders in the same file. | Never infer an extern ABI from source-level optional spelling. Specify nullable-handle and out-parameter layout, initialization, nullability, ownership, and failure behavior per C declaration; test return and out-slot representation against compiled C/LLVM behavior before broadening support.
...
**Progress (2026-09-24, compiler backend metadata):** Extern fallible signatures now populate the line-keyed error-family rows consumed by backend ABI/catch lookup; previously their error annotation made the function fallible but the backend's family lookup remained blank. `extern_error_family_catch_smoke.sh` compiles the fieldless extern catch with both Stage0 and a fresh Stage1 (product SHA-256 `4d028f365e7b3043fe8aacd6e88bb14220c1ffefe33b0bb8967c994abc04066c`); `catch_result_type_smoke.sh` also passes on that Stage1 and executes inline plus stored payload catches with result 84. This is one metadata coverage fix, not complete error-set modeling: at that checkpoint, composite `error[Family{...}, Other{...}]` signatures still collapsed to a first-family name, and the Stage1 combined-set repros declined. This was the 2026-09-24 state. Subsequent composite/tag/catch and C ABI slices are recorded in the 2026-09-25 review and ledger; complete canonical identity, propagation, and layout/call coverage remain required S02/backend work.

**Follow-up validation (2026-09-24):** `extern_error_catch_smoke.sh` now exercises statement-position catch dispatch for an extern `error[FileError]` function against both compilers, linking a C ABI stub and checking the success result plus `NotFound` arm at runtime. It passes with the committed parser metadata fix and the existing statement-catch emitter, so the candidate worktree's separate statement-emitter rewrite is not required for this plain-family case. Composite and restricted sets remain outside this evidence.
...
- [ ] Treat declarations with omitted initializers as uninitialized whenever their representation requires a valid non-null reference/handle. A direct whole-binding assignment may establish the value; write-through, field/index writes, address escape, and unknown callees do not establish every invalid leaf without a field-sensitive proof or checked write summary.
- [ ] Inventory bundled-runtime `zeroed` placeholders for generic `T` and aggregate fields. Migrate them to valid optional/empty representations or initialized constructors, or add sound per-member initialization facts; never turn a possible-reference aggregate back into an accepted zero value merely to preserve current runtime idioms. A scalar placeholder is acceptable only when the same definite-assignment analysis proves every read follows a write on every path and the write itself does not dereference the uninitialized value.
- [ ] Track linear OS resources alongside memory owners; distinguish resource leakage from temporal memory safety.

**Tests:** conditional moves followed by join use; moves on loop backedges; move through tuple/enum/error/closure; double release; partial initialization failure at each field; source/destination alias; custom destructor observing fields; reinitialization after move. Exercise representative bundled-runtime generic collection/heap/debug placeholders after migration, and pair every invalid-placeholder rejection with a valid optional or explicitly initialized twin. Positive twins include branch-complete initialization and legal ownership transfer.
...
**Follow-up verification (2026-09-24, fresh Stage30):** The Stage23 attempt was an intermediate result: its new positive closure control exposed that conservative indirect-call invalidation ran before checking the call itself. The checker now checks a local callback against the pre-call state, applies its unknown effects for following operations, and analyzes directly bound/assigned lambda bodies in a fresh deferred-execution state so reset-then-read inside the body is rejected. It also diagnoses stale reads in statement-level if/while/for/match expressions, traverses nested scalar-producing block/match/catch/recovery/comprehension/effect expressions, and carries view origins out of value blocks/match arms (including the `sview(pointer, ...)` constructor). Inline callbacks are walked at their call site. `destroyed_view_lifetime_smoke.sh` passes at `-O0`/`-O2`, including closure alias/assignment/returned closure, closure-internal reset, inline callback, if/while/for/match read positions, view-producing and scalar-reading blocks/matches, a live closure after freeing an unrelated arena (returns 65), and the tuple-capture backend refusal. Fresh Stage30 `build/arena_closure_stage30/elisac-stage1-gen2` passes freshness; `region_scope_smoke.sh` passes all 21 cases against the freshly rebuilt Stage0 oracle; `cross_module_fallible_return_smoke.sh`, self-host fixture, shell syntax, and whitespace checks pass. Stage30 Gen2 SHA-256: `2b9b818a7fc0a91d922647cb491a77a5f7f56579b0c7ce3f6ba88159d311e732`. This remains an AST/name/line-based conservative checker rather than typed CFG provenance: unknown returned closures can be rejected after unrelated invalidation, aggregate callback storage remains unsupported by the backend, and exhaustive origin/effect summaries, shadowing-safe nested scopes, and interprocedural closure escape analysis remain open. A separate audit earlier confirmed that safe `StringView{data, len}` construction could overstate pointer extent and `sview(u8&?, ...)` could reach `strlen` with a non-terminated byte. Those exact repros, `test/repro/sview_extent_overrun.elisa` and `test/repro/sview_unterminated_input.elisa`, were subsequently addressed by the S09 bounded source-view milestone recorded below (Stage1 commit `ae85f1b7`): the current representation-safety smoke requires the forged carrier and raw-reference conversions to reject while a C-string literal view remains valid. This historical trigger is no longer the next S06 milestone. Token-origin extents, external C-string termination, foreign lengths, pointer arithmetic, other indexable representations, and proof/backend coverage remain open under S09. S06 remains open and no memory-safety qualification is claimed.

**Verified S06 narrow fix (2026-09-24, closure over relocatable darray storage):** The old fresh Stage1 product (`c3a8d04d…`) accepted `test/repro/sview_closure_after_growth.elisa`: a closure captured `bytes_view(values)`, `values` grew, and the closure was then called. The storage checker now walks lambda bodies with lexical binding/scope tracking, collects only free-variable dependencies, resolves captured aliases to their backing owners, and marks a function-valued binding that carries captured storage as a borrow alias so owner growth invalidates it. Same-spelled lambda parameters, sequential locals, condition binders, loop variables, match/catch patterns, comprehensions, nested lambdas, and explicit `do`-block captures are handled; an unrecognized binding pattern conservatively includes all known storage aliases. Fresh Stage1 product `6dc1cb403c24ae4661fd1a80252f83c60e9043f4eaeddadc62ecd7a7e3d9ee1f` passes `assert_stage1_fresh.sh`. `sview_relocation_smoke.sh` requires the stale closure and a closure that rebinds a captured mutable view with `<-` to fail at `-O0` and `-O2`; unrelated darray growth, scalar-only captures, and parameter shadows compile, and the body-local-shadow control must not produce a stale-storage diagnostic. `=` remains a lexical binder in this pass; `<-` assignment targets are scanned as references to existing captures. Representation-safety, destroyed-view lifetime, region-pool-release, and region-tied-return smokes also pass against this product. This resolves those minimized paths only: the body-local-shadow multi-statement closure is declined by the LLVM backend, so that control proves only that the analyzer avoids a false stale-view report, not executable closure behavior. Capture and alias facts remain name-based; complete typed storage identities, branch/CFG joins, interprocedural closure escape/capture summaries, aggregate closure support, and alias-safe handling of every binding form remain open. Keep S06 open and do not claim a memory-safety qualification.

**Open S06/S07 follow-up (2026-09-24, residual closure provenance):** The new lambda walker is scoped to current AST forms and tracked local aliases. It is not a typed free-variable or escape analysis: unresolved external/global storage, closure copies through aggregates or opaque calls, branches that merge different capture sets, shadowed owners beyond tracked syntax, and deferred or separately compiled callable effects can still lose or over-approximate provenance. Extend the compiler's typed provenance/effect model and backend support, then add positive runtime and negative use-after-relocation controls for those paths before treating closure safety as covered.

**Verified S06 opaque-pointer callback forwarding (2026-09-24, commit `e523a7fc`):** the source-level region-tied return pass now recognizes `void` pointees through parenthesized, mutable, and optional type wrappers and excludes those opaque foreign-function pointers from inferred typed-reference return roots. The regression requires `mutable void&?` callback forwarding to compile on Stage0 and Stage1, keeps explicit `sview` lifetime erasure and mismatch cases rejected on both, and checks that a typed field-reference return remains rejected by Stage1. `sview_region_tie_smoke.sh` passes against a fresh Stage1 product (SHA-256 `c3a8d04dd350c6a9912cf85792027709dfa9b01e93f33dac2d3f72d6e7f0bba0`) with the freshness guard. This removes a false positive for untyped ABI pointers; it does not prove the pointee's actual allocation lifetime or close region-return provenance.
...
**Verified S09 NUL-terminated versus counted-byte view boundary (2026-09-25; compiler source based on `7a5f5aad`, Elisa-core main `ec851a72`):** `sview` and its postfix conversion now take `cstr?`, so the safe string-scanning path requires an explicit NUL-terminated representation; nullable raw byte references are rejected by both the postfix-cast and direct-constructor regressions. Darray-backed views in EASM name expansion and test output use `bytes_view_range_ref`, which checks the requested range against live count/capacity and carries the container borrow. Foreign LLVM names and metadata remain counted bytes: `LLVMGetValueName2` is declared as `u8&` plus its output length, and consumers use `unsafe_sview_bounded_bytes` rather than strlen-based construction. That helper checks that the length fits the sview representation and makes its trust preconditions explicit through `Unsafe.PointerCast`; callers still must prove the pointer extent and keep storage alive for each view use. The stable Python-helper suffix uses that bounded path only after copying into permanent storage. A fresh Stage1 seed from proof-pinned Core Stage0 `a3f3ea3d` passed and rebuilt the runtime object (Stage1 SHA-256 `4774cbc5ad7f0f15a85d79eccf4d485cfcd736a9a2135d38c48a0d588a27af8a`; runtime object SHA-256 `3a982fe82c18e651d5727efe3e2f6d5d3e8df0e64e81130d97684c787d3133b6`). `sview_representation_safety_smoke.sh` passes on fresh Stage1 at O0/O2, including a non-terminated three-byte buffer viewed with its explicit length; focused Stage1 checks reject both raw optional-pointer forms, accept nullable-cstr postfix and direct-constructor paths for present and null values with exit 10, fail closed on malformed sview lengths at O0/O2, reject a view used after region destruction, and accept a last-use-before-destroy control. The combined runtime smoke remains incomplete on Stage0: the freshly built Core CLI `ec851a72` fails while compiling its default runtime before reaching these fixtures, with the already-tracked uninitialized `InlineVec` output and three generic zeroed-pool-slot diagnostics (`collections.elisa:326`, `heap.elisa:189,244,288`). This is an API-safety boundary improvement, not full FFI extent/lifetime validation or S09 completion; external C-string contracts, foreign pointer extents/provenance, other indexable representations, target widths, and proof/backend coverage remain open.
...
**Verified fresh-Stage1 field-refinement slice (2026-09-25):** Unproven construction, record-update, mutable-field store, and compound mutation obligations now emit the error-level `StructFieldRefinementUnproven` diagnostic and block object emission. Struct receiver types resolve through `mutable` and reference wrappers, including record-update bases and inferred nested expressions. The visitor walks the current value-bearing expression forms (calls and static-effect captures, field/index/slice, arithmetic, aggregate literals, constructors and record updates, if/match/catch, comprehensions, lambdas, blocks, and GetElse recovery) and statement forms (contracts, declarations, stores, returns, branches, loops, blocks, and matches); pattern/local/loop/lambda binders shadow name-based facts. It seeds facts from parameter `where` predicates and runtime `assert`/`requires`, and only an interval that fits a recognized field interval can discharge an obligation. Unknown field predicates use an unprovable sentinel. To avoid stale proofs without alias-aware places, all calls and assignments invalidate accumulated facts, control-flow joins discard them, and compound writes to refined fields fail closed. `struct_field_refinement_shadow_smoke.sh` passes with a fresh Stage1: it accepts matching refined construction and mutable-reference stores plus a valid record update; it rejects unproven construction, direct and compound stores, bad record updates, a loop-shadowed binding, a mismatched parameter refinement, and stale assertions after assignment, calls, or writes through an alias. Each rejection checks the diagnostic and that no object was emitted. Stage0 accepts the minimized record-update, compound-store, and alias-write bypasses, so those are explicit Stage1 safety regressions rather than parity expectations. `scripts/assert_stage1_fresh.sh` and `git diff --check` pass. This is an interval-only, source-name-based slice, not a general proof engine: BindingIds, alias-aware mutation tracking, complete call/effect summaries, expression-condition narrowing, arbitrary refinement/law entailment, and every invariant-bearing boundary remain open. S15 and the full soundness acceptance remain open.

**Related self-host name-collision fix (2026-09-25, commit `6c216085`):** the first Gen2 attempt exposed four name-based `StorageDependencyInvalidated` reports where distinct branch-local cleanup buffers and LLVM row-type buffers reused source names. Those locals now have unique names. A Gen2 product built at that time with the already-seeded Gen1 (semantic checker unchanged, so `ELISA_ALLOW_STALE_STAGE1=1` was used for the renamed driver inputs) passed its runtime fixture; its SHA-256 was `3545a5380e0901c305bd36e860582528c9c160301f1b9e2efcffaf313594e7c4`. The current semantic checker has since been included in a fresh Stage1 seed; current-product evidence follows.

**Previously verified Gen2 field-refinement build (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement` passed its runtime fixture using Stage1 SHA-256 `c680fd566bb40791c5d5768c2c5d4db4adc3a6ba187f1ef76961d5520459447c` and produced Gen2 SHA-256 `7d01d59f65cd1927fb749a89b88d6a7107356dee6d13572f1be7d19363455e23`. The then-current `struct_field_refinement_shadow_smoke.sh` passed against that Gen2 binary. This confirms the earlier checker version rebuilt and preserved the then-tested Stage1 behavior at Gen2; it predates the zeroed-value checks below and is not evidence for those new checks.

**Verified Gen2 zeroed-field refinement follow-up (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement_zeroed` passes its runtime fixture using fresh Stage1 SHA-256 `d6df0e36c370daa5cb859c16072edb2cbdf39037da34f5620326c087c874b287` and produces `build/self_host_gen2_field_refinement_zeroed/elisac-stage1-gen2` with SHA-256 `ee9fab4ece28e5dcb5d1bf64167080341a09b6d63abe8859c7e9c8e3da4cbd84`. The expanded `struct_field_refinement_shadow_smoke.sh` passes against this Gen2 binary, including all five zeroed-value negatives and the compatible-zero positive. This establishes only the tested native self-host path; unsupported targets, broader S15 proof obligations, and all remaining plan gates stay open.

**Verified Gen2 zeroed-expression wrapper follow-up (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement_conditional_zeroed` passes its runtime fixture using fresh Stage1 SHA-256 `5d53bf5c8130319a49a400237fca94391783c4337a1703283f081330e54b09fc` and produces `build/self_host_gen2_field_refinement_conditional_zeroed/elisac-stage1-gen2` with SHA-256 `66c883a49dbffda4ad0993c13df5a7678182ad1e2dfba24de74ded0680e56ef2`. The expanded field-refinement smoke passes against this Gen2, including rejection of `zeroed` in a ternary branch and acceptance of the zero-compatible conditional. This confirms the wrapper check on the tested native self-host path only; recursive embedded-field checking and other unmodeled AST result forms remain open.

**Verified fresh-Stage1 zeroed-field invariant slice (2026-09-25):** extended `sfcu_check_zeroed_struct` so contextual `zeroed` values are checked against every directly declared field `where` interval, resolving struct aliases with a bounded cycle guard. The checker rejects zeroing a positive-only refined struct through a local initializer, a type alias, a nested field initializer, whole-struct assignment, and a global mutable initializer; the smoke verifies each error and that no object is emitted. A zero-compatible nonnegative interval remains accepted, and the existing refined-construction, record-update, store, and stale-proof cases continue to pass. Stage0 accepts the direct, alias, nested, assignment, and global minimized repros, establishing these as safety regressions. The seed source fingerprint remained `bd318425a91e5f017c032e6202974820b079319e182a268ea83f16fd17e93a71` throughout compilation; `scripts/assert_stage1_fresh.sh` passes for Stage1 SHA-256 `d6df0e36c370daa5cb859c16072edb2cbdf39037da34f5620326c087c874b287`. `struct_field_refinement_shadow_smoke.sh`, shell syntax, and `git diff --check` pass; runtime object SHA-256 is `838c3bc6f82379d13fb78f28996296c0d0cf05e9205f4dfaec33a2600db1333c`. Fresh Gen2 validation is recorded above. This checks the exercised statically known struct destinations and direct field intervals only. Zeroed values carried through arbitrary/ascribed expressions, nested generic or optional/reference carriers, non-by-value nested wrappers, alias/module ambiguity, and broader soundness remain open.

**Verified fresh-Stage1 zeroed-expression wrapper follow-up (2026-09-25):** a minimized input `Positive = zeroed if true else zeroed` was accepted by both the pre-fix Stage0 and Stage1 products, whereas the direct zeroed initializer was already rejected. `sfcu_is_zeroed` now recognizes values that may yield zeroed through parenthesized/move expressions, ternary branches, and block-expression tails; an invariant-bearing destination fails closed if any such value path yields zero. `struct_field_refinement_zeroed_conditional.elisa` verifies the Stage0 acceptance and Stage1 diagnostic/no-object rejection. The nonnegative-refinement positive fixture also uses a conditional zeroed expression and remains accepted. The source fingerprint was `2dc1ea6d3745cd1c5bafc50055e14a8cda11887d76e9ee7adaeedfae50d4e99c`; freshness and focused smoke pass on Stage1 SHA-256 `5d53bf5c8130319a49a400237fca94391783c4337a1703283f081330e54b09fc`; runtime object SHA-256 remains `838c3bc6f82379d13fb78f28996296c0d0cf05e9205f4dfaec33a2600db1333c`. This wrapper slice passes Stage1 and Gen2; recursive by-value nested propagation is Stage1-validated and awaits Gen2, while other result-producing AST forms (notably Match/Catch and conversions), generic carriers, non-by-value nested wrappers, and remaining aggregate propagation stay open.

**Verified fresh-Stage1 nested zeroed aggregate follow-up (2026-09-25):** Stage0 and the prior Stage1 accepted `Holder = zeroed` even when `Holder.inner` was an alias of `Positive`, whose value field requires `> 0`. The checker now recursively validates known by-value struct fields and resolves direct type aliases, using a visited-type set to stop cycles and collecting diagnostics before applying them to the symbol table. The expanded smoke rejects `struct_field_refinement_zeroed_nested_aggregate.elisa` with the required diagnostic and no object, while its `NonNegativeHolder = zeroed` control remains accepted. Seed source fingerprint `687d3e869d273439f3880403b58548a0cf23c1625714360df7b52a7eb3a07b3f` remained stable through compilation; Stage1 SHA-256 is `1fb613b5b49da8c0cdac72455f38d7a2a15029574deaf5df5e9e9dea2560b56c`, and freshness plus the full focused smoke pass. Fresh self-hosted Gen2 validation is recorded immediately below; generic substitutions, optional/reference carriers, parameterized aggregates, cross-module duplicate type names, and all other S15 obligations remain open.

**Verified fresh-Stage1 Match/Catch zeroed-arm follow-up (2026-09-25):** the existing detector missed `zeroed` yielded by expression-valued Match/Catch arms. Stage0 and pre-fix Stage1 accepted the Catch case where an error arm initializes a `Positive` with `zeroed`; Stage0 rejected the Match case as arm-type-incompatible, but pre-fix Stage1 still emitted an object for it. `sfcu_is_zeroed` now inspects expression tails in each Match/Catch arm and recursively checks their result wrappers. The expanded smoke requires the field-refinement diagnostic and no object for both `struct_field_refinement_zeroed_match.elisa` and `struct_field_refinement_zeroed_catch.elisa`; the valid controls still pass. Source fingerprint `bf6cd155be8acd114e00802680ce4e26b5b8ede1c585acb5493f5dfe6a740b2f` remained stable through the seed; freshness and the smoke pass on Stage1 SHA-256 `95c5c239dd3b970946ed0e5c6f29f4d24eb98c1b7e3259ad2d85d718e37ccf82`. Fresh Gen2 validation is recorded immediately below. Arm bodies ending in non-expression control statements, conversions, generic result carriers, and the broader S15 model remain open.

**Verified Gen2 Match/Catch zeroed-arm follow-up (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement_match_catch_zeroed` passes its runtime fixture using fresh Stage1 SHA-256 `95c5c239dd3b970946ed0e5c6f29f4d24eb98c1b7e3259ad2d85d718e37ccf82` and produces Gen2 SHA-256 `e50a1c792c7736d4fcaad475177e203da0eb40b3281c6dfa8a5559ca64b2fd60`. The expanded `struct_field_refinement_shadow_smoke.sh` passes against Gen2, including the Match and Catch arm negatives and existing positive controls. This confirms the exercised native self-host chain only; unsupported targets, other arm-body shapes, conversion/call result carriers, generic aggregation, and remaining S15 obligations stay open.

**Verified Gen2 nested zeroed aggregate follow-up (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement_nested_zeroed` passes its runtime fixture using fresh Stage1 SHA-256 `1fb613b5b49da8c0cdac72455f38d7a2a15029574deaf5df5e9e9dea2560b56c` and produces Gen2 SHA-256 `f55ea7af0e156f07a67d482363b88861ae5aab7c75f49a8b96acf72f8677e591`. The expanded `struct_field_refinement_shadow_smoke.sh` passes against Gen2, including the by-value `Holder.inner` alias regression and zero-compatible nested control. This validates only the current native self-host chain; generic substitutions, optional/reference carriers, parameterized aggregate recursion, cross-module duplicate type names, and remaining S15 proof obligations stay open.

**Verified fresh-Stage1 direct-call zeroed-argument follow-up (2026-09-25):** Stage0 accepted `identity(zeroed)` when `identity` takes and returns a struct whose field requires `> 0`; the pre-existing Stage1 checker also emitted the object. The expression walker now resolves the formal row for each positional or named argument of a uniquely declared direct function and applies the recursive zeroed-struct invariant check when that argument may yield `zeroed`. The smoke rejects the positional `struct_field_refinement_zeroed_identity_call.elisa` and named `struct_field_refinement_zeroed_named_call.elisa` cases, including checking the diagnostic and absence of an object, while `accept_nonnegative(zeroed)` remains accepted in the positive fixture. The seed source fingerprint stayed `5bdf2c2ff9da2e642124a9e73cb8239e9c1f6f8b67e037a2f9a71c2d58a98017`; `scripts/assert_stage1_fresh.sh` and the full focused smoke pass on Stage1 SHA-256 `231aed47c3a3305ac2d583ca90104f1cf1ea4d979fc95814851f8bd4e8ebfb3c`. Fresh Gen2 validation is recorded immediately below. This closes only the unique direct-call formal-argument boundary: overload resolution, method/function-value/closure calls, generic substitution, call-result carriers, conversion operators, and the broader S15 obligations remain open.

**Verified Gen2 direct-call zeroed-argument follow-up (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement_zeroed_call` passes its runtime fixture using Stage1 SHA-256 `231aed47c3a3305ac2d583ca90104f1cf1ea4d979fc95814851f8bd4e8ebfb3c` and produces Gen2 SHA-256 `02246800bcbbf1a02f1f9c5dd42a58b4e0dc2da5ffb35617d01ec512bb98aaf7`. `struct_field_refinement_shadow_smoke.sh` passes against that Gen2 binary, including rejection of the positional and named positive-only identity calls and acceptance of the zero-compatible call and existing valid controls; Stage1 freshness passes as well. This verifies the exercised native self-host chain only; overloads, indirect/method calls, generic substitution and all other open invariant carriers remain unverified.

**Verified fresh-Stage1 explicit-generic zeroed-argument follow-up (2026-09-25):** the direct-call boundary now recognizes `Expr.Index`/`Expr.IndexN` generic callees and substitutes an explicitly supplied type argument when a uniquely declared direct function parameter is the matching bare generic formal. `struct_field_refinement_zeroed_generic_call.elisa` exercises `identity[Positive](zeroed)`; Stage0 accepted this minimized repro, and the fresh Stage1 field-refinement smoke now rejects it with the refinement diagnostic and verifies no object is emitted. The expanded `struct_field_refinement_shadow_smoke.sh` passes on current Stage1 SHA-256 `a40d7a5b77cf0a244b3fa5248659f1339ad77501f8ecd9ce1582631bd562c724`, with freshness asserted. A later combined Gen2 validation below passes this smoke. This covers only explicit generic arguments on uniquely declared direct calls where the selected formal is a bare generic name; inferred generics, nested generic formal shapes, ambiguous overloads, methods, function values, indirect calls, and generic call-result/return substitution remain open, alongside broader S15 obligations.

**Verified Gen2 explicit-generic zeroed-argument follow-up (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement_generic_alias` passes its runtime fixture using Stage1 SHA-256 `a40d7a5b77cf0a244b3fa5248659f1339ad77501f8ecd9ce1582631bd562c724` and produces Gen2 SHA-256 `1e1d0f14afa9422d3edf9a0743a388b7e0b295710a0a7842b41bae7ebbdbe016`. The full `struct_field_refinement_shadow_smoke.sh` passes against this Gen2, including rejection of `identity[Positive](zeroed)` and all existing zeroed/refinement cases; `scope_binding_smoke.sh` also passes 83/83 against Gen2. This validates the exercised native self-host chain only; inferred and nested generic substitutions, indirect/result carriers, unsupported targets, and the broader S15 obligations remain open.

**Verified fresh-Stage1 zeroed-return boundary follow-up (2026-09-25):** a function declared to return `Positive` could return bare `zeroed`; Stage0 rejected the expression for lacking an expected destination, but pre-fix Stage1 and Gen2 both emitted an object. The structural-refinement walker now carries the enclosing function's declared result type through nested expression and statement traversal (resetting that context inside lambdas) and recursively checks any returned expression that may yield `zeroed`. The smoke rejects `struct_field_refinement_zeroed_return.elisa` with the field-refinement diagnostic and no object, and accepts the Stage0/Stage1 positive `struct_field_refinement_valid_return.elisa` identity-return fixture alongside the existing valid construction, store, update, alias, zero-compatible call, Match/Catch, and identity-call controls. The per-file source manifest matched exactly at seed completion with aggregate fingerprint `d2f47fcbefe0accaeda21059c6e8769286382ae4535965223fd6a24aab3fed9b`; `scripts/assert_stage1_fresh.sh` and the focused smoke pass on Stage1 SHA-256 `f5b891c0c498e65740124ac960b0a4b567f6f542328e59e8fcc59c690b270876`. Fresh Gen2 validation is recorded immediately below. This covers returns recognized by the conservative zeroed-expression detector; arbitrary/indirect call results, generic result substitution, unannotated result inference, and other invariant-bearing boundaries remain open under S15.

**Verified Gen2 zeroed-return boundary follow-up (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_field_refinement_zeroed_return` completed its runtime fixture from Stage1 SHA-256 `f5b891c0c498e65740124ac960b0a4b567f6f542328e59e8fcc59c690b270876` and produced Gen2 SHA-256 `0df45d82914d8d980c85533469876545c90afa08b54d9d375519dc21578ef1a0`. The expanded `struct_field_refinement_shadow_smoke.sh` passes against Gen2, rejecting the invalid return and accepting the valid identity-return control; the full field-refinement smoke and Stage1 freshness check pass. This validates the tested native self-host path only. All indirect/generic return carriers, unsupported targets, and broader S15 work remain open.
...
**Verified Stage1 module-alias ownership lookup (2026-09-25):** a module-local `Store.Handle = id[Slot]` alias shadowed an unrelated top-level `struct Handle` in the backend's bare annotation lookup; the pre-fix Gen2 declined both the function return annotation and record constructor, while Stage0 compiled and returned `42`. Annotation lowering now resolves aliases owned by the current lexical module, then top-level declarations, before considering the legacy globally unique alias fallback. `module_alias_does_not_shadow_top_level_struct` is added to the differential scope smoke; Stage0 and fresh Stage1 both compile and return `42`. The exact source manifest fingerprint is `af8af5c995cd9cdd589f67a2f959b2ae688e708a4eb70d70442ef075770e86e3`, and Stage1 SHA-256 is `74d6864637f4c65de804091b478337e70cbdf5b4e1689db6892ba0ddc4994168`. The later combined Gen2 validation below passes this smoke; other alias-resolution paths, qualified/generic annotation ownership, and the remaining S16 obligations are still open.

**Verified latest Stage1 alias-collision regressions (2026-09-25):** after the same source changes were reseeded, `scope_binding_smoke.sh` passes 83/83 differential cases, including the module-local `Store.Handle` versus top-level `Handle` collision. The imported-stdlib affine `Handle` collision fixture also passes on Stage0 and Stage1; Stage1 emits LLVM with no declined functions and LLVM's verifier accepts the IR. The current Stage1 SHA-256 is `a40d7a5b77cf0a244b3fa5248659f1339ad77501f8ecd9ce1582631bd562c724`; `assert_stage1_fresh.sh` passes. Its source-file-list fingerprint, computed from the sorted SHA-256 manifest over Elisa files under `src/` and `elisacore_std/`, is `2e3094fffc219b86294afbe3fdfb85fd929acebe00ff21cb5ca046b2e60f2e29`. A later combined Gen2 validation below passes this smoke. These cases cover two bare-name annotation collisions only; other owner/alias combinations and the remaining S16 obligations remain open.

**Verified Gen2 alias-collision follow-up (2026-09-25):** the Gen2 product recorded above passes `scope_binding_smoke.sh` 83/83, including the module-local alias/top-level struct differential, and passes `affine_borrow_std_alias_collision_smoke.sh`; the latter's Gen2 LLVM has no declined functions and passes LLVM verification. Gen2 SHA-256 is `1e1d0f14afa9422d3edf9a0743a388b7e0b295710a0a7842b41bae7ebbdbe016`. This confirms these two exercised native backend name-collision cases only; qualified annotations, other owner-resolution cases, cross-target behavior, and the remaining S16 obligations stay open.

**Verified Stage1 fallible record-field assignment (2026-09-25):** the backend already lowered `slot <- try fallible()` for a local slot, but a field destination fell through to ordinary value-call lowering and declined a named fallible call's hidden status/out ABI. Record-field assignment now resolves the named error callee and owner, propagates failure before the store, and stores only the success payload after confirming or converting its type to the declared field type. `try_field_assignment.elisa` checks both paths: a successful decode updates the field to `41`; a failed decode is caught by the caller and leaves the previous `41` intact, producing exit code `123`. Stage0 and fresh Stage1 both execute the fixture with that result; `module_struct_field_type_collision_smoke.sh` also passes on this fresh Stage1. Stage1 SHA-256 is `13eb781a8e3167f2c63be0c6af3268658928b95f7f2631a1503ebc0b2cb58657`, and `assert_stage1_fresh.sh` passes. A later combined Gen2 validation below passes this smoke. The implemented destination is a named direct error callee assigning a scalar-compatible value to an existing field; qualified/generic/indirect error callees, non-scalar aggregate destinations, and all other expression-assignment forms remain separate coverage obligations under S16.

**Verified qualified enum-valued field propagation case (2026-09-25):** `try_qualified_enum_field_assignment.elisa` exercises a module-qualified decoder returning a const enum into a record field, then checks successful storage and that a later decode failure leaves the successful value intact. The expanded `try_field_assignment_smoke.sh` executes both this case and the scalar case on Stage0 and Stage1; the qualified enum fixture also returned `42` on the earlier Gen2 SHA-256 `bfd93d8d5c5086e088b553bcbfaccbac45f910d3eedadc0288ab629c81b30b65`. An earlier `backend_native_smoke.sh` run reached 520/521 because Stage1 declined `packed_recursive_new_is_accepted`; a rerun against the current source passes all 521/521 checks, so that mismatch does not reproduce on the current broad gate. The cause of the earlier divergence is not isolated, and the broader backend completeness matrix remains open.

**Verified Stage1 value-position struct-pattern lowering (2026-09-25):** value-producing matches whose scrutinee resolves to a struct now use the existing slot/destructuring matcher even when their arms yield expressions. `struct_pattern_value_smoke.sh` passes on Stage0 and fresh Stage1: a valid `P{a: x}` destructure emits Stage1 LLVM with no declined functions and passes LLVM verification, while a `Q{b: x}` arm against a `P` scrutinee is rejected by both stages with no object left behind. Stage1 SHA-256 is `d76ca46cef8306b1a541333b0a5c72c287187b0f61b079af8b128ad09e17c392`; the sorted per-file Elisa source manifest fingerprint is `cbfe28e51b3f3dae6a806ad271181f8755f55fc4d12de0bca00495e05aaa3716`, and Stage1 freshness passes. A later combined Gen2 validation below passes this smoke. This covers value-position `Expr.Match` over struct scrutinees; nested/qualified binding forms and all other type/pattern consistency obligations remain open under S16.

**Verified Stage1 generic phantom-binding shadow (2026-09-25):** `bound_type_of` can return `Unmodeled` for a real generic binding, so using the returned type's kind to detect binding presence allowed resolution to fall through to a same-named user declaration. Type resolution now checks the active generic binding stack by name independently of whether the bound argument has a modeled representation, preserving the generic parameter's shadowing rule. `generic_phantom_shadow_smoke.sh` includes the runtime and the `darray[S]` fixture; Stage0 compiles it, and fresh Stage1 emits complete LLVM accepted by `opt -passes=verify`. The source manifest fingerprint is `a76c07ec5ef9634f116b3d57c7cbd09964e01d9ade8c7eff9754b4525be30e2d`; Stage1 SHA-256 is `30eeea2e772571de3bd30a155a830103cd6793e54f143db48199e4dc3dfc3913`, and freshness passes. A later combined Gen2 validation below passes this smoke. This only closes the active binding-presence ambiguity in these two annotation/type-resolution paths; other generic shadowing, substitutions and unknown-type uses remain open.

**Latest combined fresh-Stage1 regression pass (2026-09-25):** Stage1 SHA-256 `6b2de6993e466f89dd239cbb1c7f9eb07e815884e76fc2c2b6fd0448804601d7` is fresh for sorted Elisa-source manifest fingerprint `8399cfba8f817b2409ba7aa40a46e786409b541e17f9999ddbd2ec51eb9d7320`; the manifest remained stable through the seed and `assert_stage1_fresh.sh` passes. On that product, `scope_binding_smoke.sh` passes 83/83; the generic phantom-shadow, value-position struct-pattern, scalar and qualified-enum fallible field-assignment, module type-name collision, and tuple-`when` binding smokes pass; the affine alias-collision, module struct-field collision, and field-refinement smokes also pass. The tuple fixture emits verifier-clean LLVM on both stages and runs to 52; the module-local duplicate-type fixture runs to 42 on both stages. `backend_native_smoke.sh` passes all 521/521 native compile-and-run checks on the current Stage1.

**Verified combined self-hosted Gen2 regression pass (2026-09-25):** `scripts/self_host_gen2.sh build/self_host_gen2_combined_worktree_gains` builds Gen2 SHA-256 `359ff9d20499e86a6ca60dad960892509d39bc6d0593d122d232269c8535c81c` from the Stage1 product and source manifest recorded above. On that Gen2, the generic phantom-shadow, module type-name collision, tuple-`when` binding, scalar and qualified-enum fallible field-assignment, scope-binding 83/83, affine alias-collision, module struct-field collision, value-position struct-pattern, and expanded field-refinement smokes pass. The module struct-field check used the installed Stage0 binary explicitly because its default path is specific to another developer environment. These checks validate only the exercised native self-host chain; they do not close the remaining S15/S16 obligations or establish cross-target safety.

**Verified Stage1 module-local concrete type precedence (2026-09-25):** bare concrete type lookup now checks the lexical module and file scope before cross-module per-kind fallback, so a `RenderGraph::Resource` struct and `Schedule::Resource` enum no longer redirect one another's field annotations. Enum-array backing lookup follows the same owner-first rule and declines cross-module fallback when a local concrete declaration already shadows that name. `module_type_name_collision_smoke.sh` exercises `Graph.resources: mutable Resource[4]` alongside the unrelated enum; Stage0 and fresh Stage1 compile, link, and both execute with result 42. This complements the narrower module struct-field collision test. The later combined Gen2 validation below passes this smoke; qualified/imported collision cases and namespace rules beyond the exercised owner/file lookup remain open.

**Verified Stage1 tuple-`when` binding scope (2026-09-25):** tuple `when` rows now treat `Pattern.Binding` columns as hits, store the corresponding tuple value in an entry alloca, and declare the name only for that row body, restoring the local scope afterward. `tuple_when_binders_smoke.sh` checks literal, binding, and wildcard columns: Stage0 and Stage1 produce LLVM accepted by `opt -passes=verify`, and both execute with result 52. This evidence covers the tested scalar column bindings and row-local visibility; aggregate binding layouts, duplicate binder diagnostics, nested patterns, and exhaustive-match correctness remain separate S16 obligations. A later combined Gen2 validation below passes this smoke.
...
The entries below identify tests to add unless an existing fixture is named explicitly. `test/repro/catch_multi_family_payload.elisa` now has focused coverage in `multi_family_payload_catch_smoke.sh`; the dated composite and ABI records define its measured scope. Place remaining cases under the current fixture/parity conventions or a new `test/safety/` harness integrated into `run_all.sh`.
...
| `extern_error_family_catch` | An extern is recognized as fallible, but its error-family row is absent when lowering ABI/catch dispatch. | Preserve family metadata and compile fieldless and payload catch paths on Stage0/Stage1; fieldless and selected payload/composite cases now have focused coverage, including arm64 direct-register and sret C stubs. Indirect calls, contract/byval interactions, broader layouts and target execution remain open. |
| `catch_combined_error_set_tags`; existing `catch_multi_family_payload.elisa` | Composite error sets lose family identity or use family-local ordinals/layout when dispatching a combined result. | Extract only top-level error-set families (commas inside `Family{Tag, ...}` separate tags), then match Stage0 canonicalization exactly: order families by canonical family name, preserve declaration order within each family, apply each family's selection independently before assigning codes, and size payload storage from the selected variants only. Verify raise/propagation, direct and extern ABIs, and statement/expression payload catches at `-O0`/`-O2`; include cross-compiler callers so self-consistent but ABI-incompatible tags cannot pass. |
...
| `zeroed_invalid_reference` | Current Stage1 emits a write through a zeroed non-null reference (reproduced in the current gains review). | Reject invalid construction or enforce definite initialization before every use; cover nullable and initialized-reference positive twins and aggregate/call carriers. |
...
| S04 | Initialization, moves, and ownership obligations | In progress — the Stage1 affine-copy checker now extracts nominal type heads from generic instantiations and mutable/optional wrappers instead of treating `Owner[T]` as an unrelated bare type. Multi-argument `IndexN` applications retain their owner head and first type argument, covering region-generic forms such as `GenericOwner[T, @r]`. Affinity side tables record declaration modules in row-aligned metadata; imported module owners remain affine, while a hidden same-named module declaration no longer contaminates a top-level type. `affine_owner_smoke.sh` verifies ordinary generic copies remain accepted, direct and imported one-argument owners and imported region-generic owners require explicit moves, the hidden-name collision is accepted, and the existing owner/container/global cases remain enforced. Core commit `5fffd6be` additionally makes its zero-representation and sview classification fail closed for nil, opaque, typed-nil, and unclassified semantic types; the unknown-Type/builtin/typed-nil regressions and valid empty-header controls pass in `go test ./src/semantic -count=1` and `go test ./src/backend -count=1`. This closes a false-proof default only. The Store/InlineVec and generic-pool initialization diagnostics still block the corresponding runtime CLI test before execution. This remains a narrow classification fix: binding-identity-based moves, complete field/element/destructuring flow, partial initialization, destructor/drop obligations, control-flow joins, and interprocedural ownership summaries remain open. |
...
| S12 | Packed-store and pool handle correctness | In progress — verified row-bounds checks now cover packed dense handle/index reads and packed variant-sparse reads, using per-allocation extents before pointer arithmetic. Stage0 main commit `8c1f8a19` also replaces hardcoded packed-state byte offsets for handles, tags, and prefix columns with ABI-derived field offsets; the backend package tests pass on that commit. Stage0 and fresh Stage1 regressions trap on invalid reads under optimized and unoptimized builds. The current `RegionPool[T]`/`Pooled[T]` types omit the backing region. Stage1 rejects direct and copied-pointer use after destroy for locally visible pool creation/acquisition, propagates facts through recognized carrier-returning helpers, and now fails closed when a pooled interior-reference alias may be rebound across an if/loop/match control-flow body; the new branch-rebind regression and pool-release smoke pass at `-O0`/`-O2`. This is not a type-level guarantee and does not close aggregate/opaque flow or reset/reuse generations. Keep this API unqualified as safe until region-indexed handle signatures and backend lowering are complete, or remove it from the safe surface. Store identity, generation, reset/reuse invalidation, pool ownership, transactional metadata, all other layouts, and 32/64-bit behavior remain open. |
| S13 | Unified cleanup semantics and lowering | Planned; existing cleanup implementation retained until validated replacement |
| S14 | Operation-specific unsafe authority | In progress — lexical grants remain operation-specific for aliasing, unchecked indexing, mutable globals, pointer arithmetic, raw extern calls, pointer casts, buffer reinterpretation, and stale-view reads; direct calls and typed callback aliases check concrete Unsafe rows against active grants. Generic pointer casts that change the pointee type or add write capability now require an explicit `Unsafe.PointerCast` grant even when a generic type parameter makes ordinary assignability permissive; dropping mutability for the same pointee remains a safe coercion. Recursive enum types are guarded during type-parameter walks so a cast over a recursive enum cannot overflow the compiler. The generic pointer-erasure visitor now recursively covers current expression and statement variants, including nested value branches, Match/Catch arms, Contracts, loops, blocks, assignment places, static-effect candidates/captures, aggregate fields, and GetElse recovery; current leaf variants are explicit so adding an AST case causes a compile-time coverage failure. `generic_cast_smoke.py` passes 12 strict rejections, 2 safe same-base cases, 2 explicit-grant cases, 8 conservative NUL-flow join cases, and the default warning case. NUL facts are joined conservatively: facts established only in branches/loops/matches are not promoted, including the all-arms case, because the current name/snapshot representation does not safely support that promotion. The unsafe-grant visitor now continues traversing inside a granted call so a grant on the closure's creation site cannot hide aliasing calls that execute later; `unsafe_grant_scope_smoke.sh` passes 96 focused cases, including pointer-arithmetic parameter/local aliases, compound offsets, calls, typed struct/indexed elements, range/reference for and comprehension binders, tuple/dictionary destructuring, enum payload bindings and guards in statement/value matches, nested enum and tuple payload destructuring, named struct-field match destructuring through both values and references, unique module-qualified direct calls, value-block tails, exact/unrelated grants, shadowing, and accepted positive twins. Pointer arithmetic now has one checker for both missing and unrelated grants, with scoped structural types for parameters and local annotations. The separate visitors remain partial; the complete operation/context matrix, BindingId-based facts, imported/generated identity, method/UFCS resolution, and all grant paths remain open. |
| S15 | Sound proof/refinement/assertion handling | In progress — nullable-reference analysis covers field reads and assignment targets, scalar arithmetic, callable nullable values, single/multi-index and slices, nested static-effect expressions, branches, Match/Catch, short-circuit conditions, loops, lexical blocks, lambdas, contracts, and GetElse recovery. Its binding-local proofs are checked inside each branch and intersected at joins; loops account for zero iterations, and only runtime `assert` contracts add facts. The 581-case semantic acceptance diff reports 579 agreements and 2 alias-flow differences; both remain visible because copied-alias refinement and source-rebind invalidation do not yet have a shared BindingId/alias model. The fresh-Stage1 field-refinement slice rejects unproven constructors, record updates, direct/compound field stores, assertions invalidated by calls or writes, and contextual zeroed values for refined fields. Zeroed-value checks cover local/global initialization, type aliases, nested field initialization, and whole-struct assignment, with a zero-compatible positive control; Stage0 accepts the minimized bypasses. The implementation traverses current value-bearing expression and statement forms and handles lexical/pattern shadows, but uses source-name facts, invalidates all facts after calls/writes, recognizes only interval entailment, and does not prove every value boundary; direct, conditional, and recursive nested zeroed cases pass the focused smoke on fresh Stage1 and Gen2. Match/Catch expression-arm zeroed paths pass the focused smoke on Stage1 and self-hosted Gen2; the dated record separately identifies Stage0 acceptance/rejection differences. A shared CFG/fixed-point engine, BindingIds, alias/effect summaries, general law/refinement entailment, and the complete acceptance matrix remain open (verified slices immediately above). |
...
**Verified S16 selective private-import index slice (2026-09-25):** integrated the PMA worktree's indexed private-member and selective-import lookup. Hashes select collision chains only; exact member and owner equality remains authoritative. The source/member relation is projected once from the parser's line-paired `__from_src`/`__from_member` annotations, replacing a repeated nested annotation scan at each candidate access. The expanded `private_visibility_smoke.sh` checks selective imports of private members against public-name collisions and passes Stage0 plus a fresh Stage1 both in the candidate worktree and after integration into main. The checker still relies on source-name and line-pair metadata rather than canonical declaration/binding identity, and this narrow import case does not close effect/type/module/global correctness or S16.

| S17 | Race-free concurrency and scoped task lifetimes | In progress — the shared arena region-cache list, bounded retention metadata, and reclaimed-region hint are serialized by a runtime atomic lock on POSIX and Windows; the independent-arena concurrency smoke passes at Stage0/Stage1 O0 and O2, alongside allocator lifecycle and overflow regressions. The Stage1 LLVM backend now lowers canonical `load`, `store`, `exchange`, `compare_exchange`, integer `fetch_*`, and `fence` calls to atomic instructions. Bool storage is widened to i8 at the LLVM atomic boundary; load/store/exchange/CAS and integer RMW orderings are mapped to LLVM orderings, and pointer exchange/CAS plus pointer/floating load-store shapes have IR coverage. `atomic_runtime_smoke.sh` verifies integer/bool/pointer/floating IR and four-thread CAS increments at `-O0`/`-O2` on Stage0/Stage1; `backend_native_smoke.sh` passes 521/521 native checks, including canonical MemoryOrder operation signatures and an unrelated `fence(i64)` overload. Core hook shape checks and Stage1 arity/representation checks keep unrelated overloads on ordinary resolution. This closes only exercised native LLVM lowering and the noted arena-cache globals: pointer-valued exchange result binding remains unverified; Atomic dispatch now requires nominal `MemoryOrder` identity for typed variables and the resolved enum row for direct literals. `atomic_typed_order_parameter.elisa` confirms a typed parameter still takes the atomic path; `atomic_same_named_i32_load.elisa` confirms a user-defined same-named `i32` overload returns 42 at both optimization levels instead of being replaced by an atomic load. Known-invalid literal orders (`Release` load, `Acquire` store, `Relaxed` fence, and `Release`/`Acquire` compare-exchange) now decline before atomic instruction formation, and Stage1 refuses object emission at `-O0` and `-O2`; `atomic_invalid_order_smoke.sh` passes. `atomic_runtime_smoke.sh`, `atomic_payload_smoke.sh`, `region_threaded_ownership_smoke.sh`, and `backend_native_smoke.sh` also pass, the latter at 521/521 native checks. Valid non-literal typed `MemoryOrder` expressions still use the conservative `SeqCst` fallback until dynamic-order dispatch is implemented; same-signature user functions can still be confused with the runtime atomic target, and semantic diagnostic precision remains open; exact call-target identity, unsupported target/backend behavior, same-arena ownership, remaining concurrency globals/APIs, scoped task cleanup, race instrumentation, and deterministic scheduler checks remain open. |
| S18 | Native FFI/ABI contracts and adapters | In progress — LLVM lowers typed `va_arg[T]` reads and `va_copy` with the builder/intrinsic APIs rather than treating them as unresolved external calls; C variadic default-promotion and wide-value runtime tests pass. The shared target C return classifier now handles both direct-register and hidden-`sret` returns of payload-bearing error sets at native extern boundaries. Direct C values are reconstructed through a target-aligned transfer slot; sret calls use a typed leading pointer and an aligned internal error-set slot. Named direct calls, `try`, `catch`, and error-union-value paths use the adapter, and ordinary named calls account for the success-output and sret parameters. C-stub regressions pass at `-O0` and `-O2`; `backend_native_smoke.sh` passes 521/521. Runtime evidence covers arm64 direct `{i32, i32}` and a 32-byte sret layout. External error function values/indirect calls, agreement for C layouts with multiple payload variants, x86-64/Wasm execution, pointer ownership, string/buffer extents, callback lifetime, varargs type safety, and full platform ABI coverage remain open. |
...
**Verified S14 pointer-arithmetic enum-match follow-up (2026-09-24):** strict pointer arithmetic through a reference payload bound by `Enum.Variant(pointer, step)` was invisible in both statement-match and value-match arms because the grant visitor installed pattern names with Unknown types. Enum payload metadata now retains structural TypeIds and field positions, and the grant pass recovers payload types for positional variant bindings plus structurally known tuple/as/type-bind patterns. Match-expression guards are visited after installing the arm bindings, matching their lexical scope. The smoke covers missing-grant and exact-grant statement/value match bodies and guards, including nested enum and tuple payload destructuring (88 cases total). Unknown, ambiguous, generic, and unsupported pattern shapes remain conservative Unknowns; nested/struct-pattern completeness and the whole operation/context matrix remain open. S14 remains in progress.

**Verified S14 struct-match pointer-grant follow-up (2026-09-24):** a match arm using `Packet{pointer, step}` had the same Unknown-binding hole as enum payload patterns. The pointer-grant walk now looks up each named struct field's structural TypeId, including explicit `field: binder` patterns, and refuses conflicting same-named module rows. The expanded smoke passes 90 cases, including missing-grant and exact-grant twins for named struct-field patterns; a fresh Stage1 seed, freshness guard, Gen2 self-host, shell syntax, and whitespace checks pass. Structs whose field metadata is incomplete (including anonymous-field layouts), ambiguous/missing types, and other unmodeled pattern forms still resolve to Unknown, so this does not close all match binding or S14 coverage.

**Verified S14 module-qualified pointer-operand follow-up (2026-09-24):** strict pointer arithmetic using `Namespace::ref_returning_call() + Namespace::int_returning_call()` was missed because the structural pointer-grant channel only resolved bare-identifier calls. The visitor now resolves a scoped direct call by its declaration name through `function_return_type_id`, which returns Unknown for overloaded or generic schemes; reference and integer operand classification share that path. `unsafe_grant_scope_smoke.sh` passes 96 cases with missing-grant and exact-grant module-call twins. Fresh Stage1 seed, freshness guard, smoke, Gen2 self-host, shell syntax, and whitespace checks pass. Method/UFCS calls, imported name collisions, overload/generic instantiation, and the larger operation/context matrix remain open.

**Verified S14 reference-wrapped aggregate-pattern follow-up (2026-09-24):** aggregate field/element selection now unwraps structural Ref TypeIds before resolving enum, struct, or tuple pattern contents, while a whole-value binding retains the scrutinee's original reference TypeId. The smoke passes 94 focused cases, including missing-grant and exact-grant twins for enum and struct patterns over reference parameters. A fresh Stage1 seed, `assert_stage1_fresh.sh`, 94-case smoke, Gen2 self-host, shell syntax, and whitespace checks pass. Generic substitutions, unresolved/ambiguous type identities, incomplete struct field metadata, and other aggregate/pattern forms remain open and must be included in the broader operation/context matrix.
...
**S14/S07 invalidated-view grant boundary (2026-09-25):** the unsafe-grant matrix previously expected an exact `Unsafe.StaleRef` grant to make a `view` usable after the backing `darray` moved. A direct probe shows both Stage0 and Stage1 reject that use with `StorageDependencyInvalidated`; authority to perform an unsafe operation does not establish that the backing storage is still live. `unsafe_grant_scope_smoke.sh` now asserts this fail-closed diagnostic and passes all 96 cases on a fresh Stage1 product. Any future explicit unsafe escape for stale views needs a separately represented contract and must not treat an invalidated safe `view` as live. This settles the current regression expectation only; complete stale-reference capability semantics and storage-provenance coverage remain open.
...
**Verified S09 static-literal binding slice (2026-09-24):** the string-binding checker now models `cstr` as a rejecting target for a propagated unannotated literal binding, while a direct literal remains contextually convertible. The paired `static_string_binding_cstr_argument` diagnostics fixtures require `take_c(s)` to report `static u8&` versus `cstr` and keep `take_c("zero")` silent; the full diagnostics matrix passes 410/410 fixtures. This covers the measured return/declaration/call/field/container sites in the checker only; it does not establish C-string termination for arbitrary pointers, view extent, or complete string-family compatibility.
...
**Verified S07 copied-container and borrowed-place invalidation slice (2026-09-24):** the storage dependency pass now keeps a copied darray's backing source when assigning the container alias, then carries that source through view construction and the exercised field/aggregate/value wrappers. It separately marks view/reference locals as borrow aliases, so container copies are recorded without making ordinary owner values themselves stale-view diagnostics. `sview_relocation_smoke.sh` passes chained and optional views, a cross-field shared-backing case, and a copied-darray alias case that force a growth-side observable use; stale Stage1 aliases are rejected at `-O0` and `-O2`, while the unrelated-growth live control remains accepted. `sview_representation_safety_smoke.sh` passes at both optimization levels, and the complete `diagnostics_smoke.sh` matrix passes 410/410, including the existing darray-interior-reference invalidation fixture. The first broad classifier also rejected many existing compiler sources; narrowing it to copied darrays plus explicit borrowed/view types restored the complete diagnostics gate before qualification. This remains local, source-name-based invalidation with no complete projection identity, arbitrary helper/callback mutation summaries, or lending API, so S07 remains in progress.

**Verified S07 independent-literal identity and diagnostic provenance (2026-09-24, Elisa-core commit `0e0e431d`):** the Stage0 storage-view pass now treats a list literal as a fresh container identity instead of recursively classifying its elements as a shallow darray alias. A regression grows a second independently initialized empty darray while a view into the first remains live; the checker must accept it, while existing copied-container and shared-backing tests still reject stale aliases. Storage invalidation diagnostics now report backing sources, container aliases, and a deterministic matched mutation source. Elisa-core `go test ./src/semantic -count=1` passes, and the source-built Stage0 compiles the complete compiler driver to an object without storage-view diagnostics. The fresh Stage1 relocation, representation-safety, destroyed-view, region-pool-release, and region-tie smokes pass, including `-O0`/`-O2` controls. Stage1 SHA-256 is `c3a8d04dd350c6a9912cf85792027709dfa9b01e93f33dac2d3f72d6e7f0bba0`; `assert_stage1_fresh.sh` passes. This is more precise source-name aliasing, not complete storage identity or callable-capture provenance; in particular, the closure-after-growth repro remains an open S06/S07 gap.

**Integrated compiler worktree gain (2026-09-24, commit `e5007c11`, source `/private/tmp/elisa-compiler-f50ecf8`):** dictionary checked indexing with `get values[key] else fallback` now lowers through the dictionary's optional-reference `get` method, branches on presence, loads the hit value, evaluates the fallback only on a miss, and merges through a result slot. The regression exercises hit `42` plus miss fallback `7`, and Stage0 and fresh Stage1 compiled executables both return the expected sum `49`; `dict_index_fallback_smoke.sh` and freshness checks pass. This restores a checked-index expression form that the Stage1 backend previously declined for `DynDict`. It is a targeted lowering/parity gain and does not establish broader dictionary bounds, mutation, or lifetime safety; those contracts remain part of S07/S08 validation.
...
**Verified S12 RegionPool release invalidation slice (2026-09-24):** `check_interior_ref_uaf` now extracts the call leaf from bare, module-qualified, method, and parenthesized callee forms, so `pool.release(move handle)` records the same affine-owner consumption as `release(move handle)`. Before this fix, a copied `mutable heap T&` from `handle.ptr` remained usable after a method-form pool release and could observe a slot that had been recycled for another object. Field-origin tracing now follows nested member and index projections back to the affine owner; `region_pool_nested_field_after_release.elisa` covers a reference derived from `handle.ptr.value`. Owner facts also flow through array, tuple, set, dict, struct-construction, and record-update expressions; when an aggregate combines distinct or unresolved pooled owners, it records an unknown owner and fails closed after a later handle consumption. `region_pool_struct_after_release.elisa` verifies a pointer stored as `Holder.ptr` is rejected after release. `region_pool_primitive_after_release.elisa` now reproduces that case; `region_pool_primitive_before_release.elisa` is the live-use control. The checker now leaves explicit scalar value locals (integer, float, bool, char, and void types) outside the interior-reference alias set; otherwise a copied `i64` read before release was spuriously treated as a pointer. A simple local `as & <-` rebind now updates the tracked owner fact; unresolved rebind sources become conservatively dependent on any later-consumed pooled owner. `region_pool_alias_rebind_after_release.elisa` is rejected after rebinding to a subsequently released handle, while `region_pool_alias_rebind_live.elisa` confirms that rebinding away from a released handle to a live handle remains accepted. The focused Stage1 smoke requires rejection with an `InteriorRefConsumed` diagnostic and no LLVM artifact after release, while accepting scalar use completed before release and valid live rebinds, at `-O0` and `-O2`. The fresh Stage1 smoke and the existing destroyed-view lifetime suite both pass at both optimization levels. The checker also records explicit affine moves inside calls nested in common member/index/slice/unary/binary/array/tuple/set expressions, and checks reads and consumption in if/while conditions, for iterables, match scrutinees/guards, assignment targets, and contract expressions; the condition regression wraps the consuming call in `or false` to ensure expression recursion is exercised. This is still the existing syntax-ordered local alias checker: branch joins, shadowed/reused binding identity, alias paths and aggregate/expression forms outside these visitors, indirect/dynamic consuming calls, parameter/return loans, and release facts across helper boundaries remain open. It does not make the unqualified `RegionPool[T]`/`Pooled[T]` types or raw pool references memory-safe; keep S12 in progress and retain the type-level region, generation, store-identity, raw-API, and failure-atomicity requirements.

**Follow-up — pooled affine-parameter owner slice (2026-09-24):** a helper parameter typed `Pooled[T]` is now inventoried alongside local pooled owners before collecting interior-reference aliases. This closes a concrete hole where `ptr = handle.ptr; pool.release(move handle); return ptr[0]` inside a helper was accepted because only local declarations contributed owner names. `test/repro/region_pool_parameter_after_release.elisa` is rejected with `InteriorRefConsumed` before LLVM emission, while `test/parity/fixtures/region_pool_parameter_before_release.elisa` proves that a scalar copied from the pointer before release remains usable. The fresh Stage1 product passes `region_pool_release_lifetime_smoke.sh` at `-O0` and `-O2`, and `destroyed_view_lifetime_smoke.sh` passes at both levels; Stage1 freshness, shell syntax, and whitespace checks pass. This remains a syntax/name-based, source-ordered local pass: parameter BindingIds, shadowing, arbitrary heap-reference parameter aliases, helper-mediated release summaries, branches/loops, and complete loan/liveness semantics are open. A conservative raw-reference-parameter attempt did not catch its isolated alias repro and was removed; it is not counted as closed. This makes no claim that `RegionPool[T]` or `Pooled[T]` is memory-safe.

**S12 follow-up — iterative pooled-owner expression walk (2026-09-24):** the pooled interior-reference checker now walks nested field/index/call/aggregate expressions with an explicit pending-node worklist, preserving unknown-owner joins while avoiding recursively nested helper calls in the self-hosted analyzer. Affine `Pooled[T]` parameters are included with local owners. The rebuilt Stage1 passes the 410-fixture diagnostics matrix and `region_pool_release_lifetime_smoke.sh` at `-O0`/`-O2`; the changed nested-field repro takes the address of the actual field, and pool fixtures use an explicit authority cast where required. This is still a source-ordered name-based subset: branch/loop joins, shadowing-safe binding identities, arbitrary reference parameters, helper-mediated release summaries, and the pool's type-level region/generation/store identity remain open.

**Verified S12 conditional pooled-alias owner join (2026-09-24):** `test/repro/region_pool_branch_rebind_after_release.elisa` reproduced a missed path: a local reference initially borrows `first.ptr`, one `if` arm rebinds it to `second.ptr`, then `first` is released and the alias is read. The source-ordered pass previously kept only the rebinding arm's owner and accepted the stale read on the path where the arm was skipped. The checker now detects when a tracked interior-reference local may be rebound inside an if/while/for/match body, marks each affected alias owner unknown before scanning a possible body and again after source-ordered traversal, then joins conservatively; the post-body fallback is necessary to retain the incoming owner when a visited rebind arm may not run. The nested statement-shape detector uses an explicit pending-node worklist, avoiding recursive statement descent in the self-hosted analyzer. `region_pool_release_lifetime_smoke.sh` now requires the specific `InteriorRefConsumed` diagnostic and no LLVM output for this case at `-O0` and `-O2`; it also requires the fresh Stage0 oracle to reject the same conditional owner path with the same diagnostic and no LLVM output at `-O0`. The complete Stage0/Stage1 pool-release smoke and `destroyed_view_lifetime_smoke.sh` pass at both Stage1 levels, and freshness checks pass. `diagnostics_smoke.sh` also passes 410/410 on the combined current source. The name-based pass still is not a CFG/fixed-point analysis: branch-local diagnostics may conservatively reject safe paths, loop iterations and abrupt exits are not modeled completely, and canonical BindingIds, helper summaries, type-level region/generation/store identity, raw pool API restriction, other targets, and full qualification remain open.
...
**Verified S26 C-archive completeness slice (2026-09-24):** `emit_c_archive` now checks backend decline metadata before producing the archive object or any ABI sidecars. A unit with a supported `main` and a declined required body fails with a nonzero status and leaves no `.a`, `.h`, `.unsafe.txt`, or `.elisa-abi.json`; `no_partial_backend_output_smoke.sh` covers that partial-body path alongside LLVM, object, bitcode, and executable refusal. The valid archive path passes `backend_c_archive_smoke.sh` and `emit_c_archive_bundle_smoke.sh`: the generated C consumer links and prints `42`, and the three Stage0/Stage1 bundle outputs are identical. This verifies partial-body refusal and the exercised valid bundle only; all-declined/no-function cases, every backend decline phase, stale pre-existing sidecars, and atomic publication across the other multi-file modes remain open under S26.
...
**Verified S06 movable-view backing invalidation slice (2026-09-24):** the region-storage checker tracks dependencies from an explicit set of built-in borrowed-view constructors and invalidates aliases when their backing dynamic array may relocate; nested call-callee expressions are checked as well, so reads such as `sview_at(view, ...).i64()` cannot hide the stale receiver. The iterative place-root walk also prevents the self-hosted backend from declining `storage_view_place_root` while compiling the diagnostic report. `sview_relocation_smoke.sh` passes the chained-growth and optional-view stale cases plus the unrelated-container positive control at both stages; `destroyed_view_lifetime_smoke.sh` passes at `-O0`/`-O2`, and diagnostics passes 410/410. The explicit contract list avoids treating every argument to an arbitrary `sview`-returning helper as backing storage. User-defined borrow summaries, complete alias paths, CFG joins, shadowing-safe identities, mutation through unknown callees, and non-native targets remain open; S06 is still in progress.

**S06 follow-up — copied darray backing aliases (2026-09-24):** the Stage1 storage checker now tracks a local alias only when its declared type is a dynamic array, carries the shared backing root through copies, and expands a growth target through those roots before invalidating dependent views. `sview_cross_field_alias_stale.elisa` covers two struct fields sharing one darray; `sview_darray_copy_alias_stale.elisa` covers a view of `source` followed by growth through `alias`. Both Stage1 cases reject with `StorageDependencyInvalidated`, leave no LLVM output, and pass at `-O0`/`-O2`; the unrelated-container positive control remains accepted. Stage0 currently accepts both new repros, so this is additional Stage1 protection and an explicit Stage0 parity gap. The combined fresh-product checks pass `sview_relocation_smoke.sh`, diagnostics 410/410, `region_pool_release_lifetime_smoke.sh`, and `destroyed_view_lifetime_smoke.sh`. This remains name-based and does not cover arbitrary aggregate copies, imported helpers, complete alias identity, path-sensitive joins, or all mutation forms.

**S06 follow-up — aggregate-held view backing invalidation (2026-09-24):** the Stage1 storage checker now carries borrowed-storage status into typed aggregate aliases when their initializer contains a recognized borrowed-view constructor, reference, or already-known view alias. Its iterative expression walk traverses record constructors/updates, tuple/array/set/dict elements, branches, field/index projections, and common call arguments without introducing recursive visitor depth. `test/repro/sview_aggregate_field_after_growth.elisa` constructs `ViewHolder{content: bytes_view(values)}`, grows `values`, and must fail with `StorageDependencyInvalidated` before LLVM is written; `test/repro/sview_aggregate_copy_after_growth.elisa` wraps that holder in `ViewBundle`, copies the nested aggregate, and must also fail after growth; the positive `test/parity/fixtures/sview_aggregate_field_unrelated_growth.elisa` copies the same nested holder while growing a distinct array and remains accepted. The fresh Stage1 passes `sview_relocation_smoke.sh`, `sview_representation_safety_smoke.sh`, `destroyed_view_lifetime_smoke.sh`, `region_pool_release_lifetime_smoke.sh`, and diagnostics 410/410 at the exercised optimization levels; the tested Stage1 SHA-256 is `60b777fdd530e18dfaa63c55003deede414c164b05c18884f3b0a29f73fbaed1`. This is still Stage1-only name-based evidence: it does not establish Stage0 parity or a complete field-sensitive borrow graph, and untested aggregate field/update/return shapes, user-defined borrow summaries, canonical binding identities, complete mutation coverage, other targets, and the full memory-safety gate remain open.
...
**Verified S06 Arena& alias rebind and control-flow fallback (2026-09-24):** `test/repro/arena_alias_rebind_reset_leak.elisa` exposed a stale-view read that the initializer-only alias summary missed: `alias` was initialized from `first`, rebound with `alias <- second`, then reset; the pre-fix Stage1 executable returned replacement value `7` instead of the preserved view's `65`. A second probe, `test/repro/arena_alias_conditional_rebind_reset_leak.elisa`, showed that sequential branch visitation also missed the runtime path where a conditional assignment does not execute; the pre-fix executable again returned `7` after resetting the actual `first` owner. The destroyed-region transfer now replaces a local alias's summary root on straight-line `<-` assignment, so subsequent reset/free invalidation follows the current owner. In `if`, `while`, `for`, and `match` control-flow bodies, rebinds retain an unknown owner summary rather than pretending one visited path is unconditional; resetting through that alias fails closed across tracked regions. A structurally inconsistent parallel alias table also fails closed. This conditional fallback can reject unrelated views after a reset, so precise CFG owner-set joins remain required for feature completion. The negative smoke cases require the expected region diagnostic and no LLVM artifact at `-O0` and `-O2`; `test/parity/fixtures/arena_alias_rebind_reset_live.elisa` confirms that a straight-line rebind to `second` followed by resetting `second` preserves the `first` view and returns `65`. `destroyed_view_lifetime_smoke.sh` passes against both the freshly seeded Stage1 and fresh self-hosted Gen2; `region_scope_smoke.sh` passes its 21 carrier-less cases and lexical regressions against Gen2; `cross_module_fallible_return_smoke.sh` passes against Gen2. `scripts/self_host_gen2.sh`, shell syntax, `git diff --check`, and `assert_stage1_fresh.sh build/self_host_gen2/elisac-stage1-gen2` pass. The final tested Stage1 SHA-256 is `924fd00f618992c5bb9b547d58a020524737efd70c0ba6ba390d2856c4ddf770`; fresh self-host Gen2 is `build/self_host_gen2/elisac-stage1-gen2`, SHA-256 `9f66721422a2c2b52525a405bab75ae795fff0ea972e176a3b1e2fb7eee19810`; source-built Stage0 is SHA-256 `2482b18cf6584a00115d256471fe50cd0da4d99a37087349cb6ec987091745bf`. This is still a name/line-side-table transfer for simple local aliases with conservative control-flow fallback, not a complete canonical owner identity, CFG provenance analysis, aggregate-field alias analysis, allocation epoch model, cross-target result, or full-gate result.


**Verified S06 branch owner-root set slice (2026-09-24):** the prior branch fallback safely marked all tracked regions invalid after a conditional Arena& rebind, but rejected unrelated views. The owner-alias side table now permits multiple rows for one binder: control-flow assignments add each possible root, and a later reset invalidates each root plus aliases that designate it. A straight-line rebind replaces the prior candidate rows with the currently resolved root. Copying a multi-root alias is conservatively summarized as unknown, and owner-table length mismatches fail closed. `test/repro/arena_alias_conditional_rebind_reset_leak.elisa` requires rejection when the branch is false at runtime and `alias` still designates the stale `first` arena; `test/parity/fixtures/arena_alias_conditional_rebind_unrelated_live.elisa` proves that resetting a possibly `first`/`second` alias preserves an independent third-arena view and returns `67`. The straight-line rebind live control continues to return `65`. `destroyed_view_lifetime_smoke.sh` passes against both the freshly seeded Stage1 and fresh self-host Gen2 at `-O0`/`-O2`; `region_scope_smoke.sh` passes all 21 carrier-less cases and lexical regressions; `cross_module_fallible_return_smoke.sh` passes. `scripts/self_host_gen2.sh`, shell syntax, `git diff --check`, and Stage1 freshness pass. The tested Stage1 SHA-256 is `3b36dc34239dcd9f25b1616ec0c015d87f4d49d6298ee12aff84f3759d8d32ee`; fresh Gen2 `build/self_host_gen2/elisac-stage1-gen2` is `b484ae4d3466723aa502763ea9d9d7586d2a552af7e7e50295ee67908ac9c55e`; source-built Stage0 is `2482b18cf6584a00115d256471fe50cd0da4d99a37087349cb6ec987091745bf`. This is a conservative root-set approximation, not a CFG fixed point: it may retain roots from branches that overwrite the alias on every path, loops can grow the union, and copying a multi-root alias falls back to global invalidation. Canonical BindingIds, exact joins, aggregate/field aliases, reset epochs, other targets, and the full acceptance gate remain open.

**Verified S06 multi-root alias-copy slice (2026-09-24):** after branch rebinds began retaining a set of possible roots, copying such an alias into another local collapsed that set to the unknown/global sentinel. The analysis now collects all roots associated with a source alias and carries each through local Arena& initialization and `<-` rebind; reset/free through the copy invalidates every candidate root and all aliases tied to them. Missing/misaligned side-table rows remain fail-closed. `test/repro/arena_alias_copy_conditional_rebind_reset_leak.elisa` conditionally rebinds `alias`, copies it, resets the copy, and requires the possible stale `first` view to be rejected with no LLVM output. The positive fixture `arena_alias_conditional_rebind_unrelated_live.elisa` resets through the copied alias while preserving an unrelated third-arena view, which returns `67`; the straight-line control still returns `65`. The full `destroyed_view_lifetime_smoke.sh` passes against the seeded Stage1 and self-hosted Gen2 at `-O0`/`-O2`; `region_scope_smoke.sh` passes the 21 carrier-less and lexical cases; cross-module fallible-return parity passes. `scripts/self_host_gen2.sh`, shell syntax, `git diff --check`, and Gen2 freshness pass. Final tested Stage1 SHA-256: `8d6f596b66c787ef040d77e8a41be1eded4f1b331857645157040712a47efa47`; fresh Gen2 `build/self_host_gen2/elisac-stage1-gen2`: `6af0a9ca552d4233782c0873ad56a35142345f4fac2bed39208b9e8cdc88cd55`; source-built Stage0: `2482b18cf6584a00115d256471fe50cd0da4d99a37087349cb6ec987091745bf`. The candidate relation is still a conservative side-table approximation: multi-root values crossing helper parameters/returns, fields, aggregates, precise branch replacement, CFG fixed points, canonical binder identities, reset epochs, other targets, and the full acceptance gate remain open.

**Verified S06 control-flow alias regression matrix (2026-09-24):** `test/repro/arena_alias_control_join_reset_leaks.elisa` covers three paths where a loop or match may leave a mutable Arena& alias at its original owner: a runtime-skipped `while`, an empty `for` range, and a selected wildcard `match` arm while another arm rebinds. The combined source requires owner-specific destroyed-region diagnostics for all three views and no LLVM output. `destroyed_view_lifetime_smoke.sh` passes this matrix at `-O0` and `-O2` against the freshly seeded Stage1 and self-hosted Gen2, alongside the conditional-rebind rejection and unrelated-region positive control. Region-scope (21 carrier-less cases), cross-module fallible-return parity, Gen2 self-host, shell syntax, diff whitespace, and freshness checks pass on the recorded compiler products above. This closes only the exercised control forms in this local owner-alias transfer; nested expression assignments, abrupt-edge precision, separate-function aliases, and a CFG fixed-point implementation remain open.

**Verified S06 precise `if`/`else` owner-set join (2026-09-24):** the owner-alias walker now snapshots scalar binder/root rows, analyzes each arm independently, restores the original rows in the existing caller-owned darrays, and merges the candidate roots by binder and root identity. This avoids both sequential-arm misclassification and storing temporary-region darray descriptors into caller state. A top-level join uses only roots present on a reachable arm; when nested in a loop or another uncertain control context, it also keeps the incoming roots so a skipped outer iteration/path remains represented. Malformed side-table lengths still trigger global invalidation and conservative traversal. `arena_alias_conditional_rebind_reset_leak.elisa` (then arm rebinds, else arm leaves the old owner) continues to reject the stale path. `arena_alias_both_if_arms_rebind_live.elisa` assigns `second` and `third` on its two arms, then resets the alias and reads a view tied to `first`; it now compiles and returns `65` because neither post-join root can designate `first`. The complete destroyed-view smoke, including alias-copy, skipped-while, empty-for, and selected-match-arm cases, passes on the seeded Stage1 and fresh self-host Gen2 at `-O0`/`-O2`; region-scope and cross-module fallible-return checks pass. `scripts/self_host_gen2.sh`, `bash -n`, `git diff --check`, and Gen2 freshness pass. Stage1 SHA-256: `654753672e042d83cc6cc398c8e9e45bf4da1cddabc545abe91be2add4dc8bf5`; self-host Gen2 `build/self_host_gen2/elisac-stage1-gen2`: `30ead7c9ff03cfe52bd62e0af6b5c1b719736a834afc9faba1979efd347ad2ba`; source-built Stage0: `2482b18cf6584a00115d256471fe50cd0da4d99a37087349cb6ec987091745bf`. This remains a local owner-root analysis rather than a complete CFG fixed point: early exits, unreachable arms, loops/backedges, nested expression/control statements, helper summaries, canonical BindingIds, aggregate aliases, reset epochs, other targets, and the full gate need continued work.
**Verified S06 precise unguarded-catch-all `match` owner-set join (2026-09-24):** each reachable statement-match arm now starts from a copy of the incoming Arena& alias roots, contributes its resulting roots to a separate merged set, and cannot contaminate the next arm's transfer. The incoming roots are omitted only when an unguarded wildcard or binding arm guarantees a selected arm for every value; guarded or non-exhaustive matches retain them, and existing outer uncertain-context handling remains conservative. The negative `selected_match_case` remains rejected because its catch-all leaves the alias at the original owner; new `arena_alias_guarded_match_rebind_reset_leak.elisa` proves a guarded binding cannot erase the possible no-arm path and is rejected by Stage1 and Gen2. New positive fixture `test/parity/fixtures/arena_alias_exhaustive_match_rebind_live.elisa` rebinds to `second` or `third` in an exhaustive match, resets the alias, and reads a view owned by `first`; it compiles and returns `65` at `-O0` and `-O2`. The complete `destroyed_view_lifetime_smoke.sh` passes on freshly seeded Stage1 and fresh Gen2; Gen2 `region_scope_smoke.sh` (21 carrier-less plus lexical cases) and `cross_module_fallible_return_smoke.sh` pass; Stage1/Gen2 freshness, `bash -n`, and `git diff --check` pass. Tested Stage1 SHA-256 `0b72354d32f5929f9957844844bbeefb111349338f9195a7c9870132bc0aabb7`; `build/self_host_gen2/elisac-stage1-gen2` SHA-256 `f5db4f7c7d671c26c1cdcf8da893b773c091b567690890d0dcb1ceee29d37099`; Stage0 SHA-256 `2482b18cf6584a00115d256471fe50cd0da4d99a37087349cb6ec987091745bf`; runtime object SHA-256 `838c3bc6f82379d13fb78f28996296c0d0cf05e9205f4dfaec33a2600db1333c`. This improves only name/line-based local Arena& alias precision: nested/general match value flow, alias binder identity, branch side-effect snapshots, loop fixed points, reset epochs, helper summaries, other targets, and the full gate remain open.
...
**Cross-repository worktree reconciliation (2026-09-24):** `Elisa-core` had one additional registered worktree, `/Users/torarinvikbjarko/.codex/worktrees/sview-liveness/Elisa-core`, on branch `codex/sview-liveness`. Its working tree was clean, its HEAD matched Elisa-core `main` at `5b7c899f` (`Track shallow darray aliases in sview lifetimes`), and `git rev-list main..codex/sview-liveness` was empty. No unique tracked, staged, or ordinary untracked changes remained to transfer. After that comparison, the redundant worktree was removed with `git worktree remove`; Elisa-core now registers only its main checkout. Its gains were already in main, and the compiler-side smoke coverage is recorded in the current S06/S07/S09 entries above.
...
**Compiler candidate-worktree audit (2026-09-24, current):** The detached `/private/tmp/elisa-compiler-f50ecf8` candidate is based at ancestor `f50ecf85`. Its dictionary fallback lowering and regression artifacts are already integrated in main at `e5007c11`; the closure-storage dependency changes and their regression coverage are integrated at `a7d6a0db` and `b7ebe332`. The candidate's extern-signature family-row capture is now also integrated with Stage0/Stage1 regressions (`extern_error_family_catch_smoke.sh`, `extern_error_catch_smoke.sh`). The remaining candidate-only backend edits change expression/statement catch tag lookup and payload mapping for composite error sets. They are not integrated: the expression/catch edits still regress `catch_result_type_smoke.sh`, and the combined-set repro remains declined by a fresh Stage1 even after extern-family metadata is captured. The candidate's error-tag helper and synthetic table builder are therefore not yet sufficient evidence for a safe import. Repros `test/repro/catch_subset_exhaustive_std.elisa` (extern call) and `test/repro/catch_combined_error_set_tags.elisa` (defined callee) preserve the known gap; no passing composite-set gate is claimed. The previously audited `/private/tmp/elisa-compiler-proof-b7ebe` checkout was clean at current main HEAD and had no unique source changes; it has since been removed from the worktree registration. Retain the candidate only while its unverified edits need repair or explicit exclusion, then remove it after a fresh source/status/process audit. Composite errorsets still need first-class canonical identity and ABI representation before their backend work can be considered complete.

**Composite-error candidate probe (2026-09-24):** the candidate later added a synthetic error-set table builder and a fresh Stage1 seed. The extern fieldless statement-catch parity smoke passed on that product, but `catch_result_type_smoke.sh`, the defined-callee composite runtime at `-O0`/`-O2`, and the extern composite-subset repro still failed/declined. With decline tracing enabled, the candidate reported `COMPOSITE missing family Bad4`: `capture_header_error_families` treated the comma before the second selected variant inside `BarError{Bad3, Bad4}` as a family-level comma. Stage0's `CanonicalizeErrorSetSelections` sorts family names before building tag codes and preserves each family's declaration order; source-order registration is not equivalent. The synthetic registry and catch/raise edits remain unimported until they pass exact ABI/runtime parity.

**Parser metadata follow-up (2026-09-24):** the scanner now excludes identifiers inside a selected-variant brace from `__error_set_family` rows, while retaining each `__error_variant_allow` entry. `parser_ast_smoke.sh` passes 111 structural tests, including an extern composite-set assertion for exactly the `FooError` and `BarError` families and the selected `Bad1`, `Bad3`, and `Bad4` variants. This narrow parser fix is integrated; it does not claim composite backend support. On a fresh candidate product (`assert_stage1_fresh.sh` passes), the fieldless extern statement-catch parity smoke passes and the declaration-only composite signature compiles, but `catch_result_type_smoke.sh` still declines `main@21 (control expression)`, and the minimal and exhaustive extern-subset catch fixtures each segfault (exit 139). The defined-callee composite repro also declines `foo@8810 (expression statement)`. The minimal crash repro and declaration-only control have been copied into main for follow-up. Source review found additional ABI blockers in the candidate registry: it assigns codes in source annotation order instead of Stage0's canonical family-name order; its one `restricted` flag is shared across families, so restricting one family can filter variants from another; and it sets the widest payload size from the full declared family even when the selected subset excludes wider payloads. These failures and independent layout/tag mismatches exclude the candidate backend edits from import until corrected and retested. The composite raise/catch ABI, canonical ordering, family-local subset selection, selected payload layout, and cross-compiler runtime checks remain open.

**Verified multi-family external payload-catch characterization (2026-09-25):** the new `test/repro/catch_multi_family_payload.elisa` declares an extern returning `error[FirstError, SecondError]` and uses an expression catch with both a fieldless first-family case and a payload-bearing second-family case. The fresh Stage0 compiles the fixture to LLVM accepted by `opt -passes=verify`. The current fresh Stage1 exits 2, reporting `backend emitted no functions for this unit; declined 1: use@14 (expression)` and writes no LLVM. This adds a minimal extern/multi-family/payload expression-catch shape to the known composite-error gap; it is a repro for future parity work, not a passing test or evidence that the candidate catch backend is safe to import.

**Fresh-main verification (2026-09-24):** after the parser fix and diagnostic repros were committed, `scripts/elisac_stage1.sh --seed` completed and produced Stage1 SHA-256 `b028a7b03f5de764f70f9d83271123067c6f0b98666e0026ce4a1c012d48876c`; `assert_stage1_fresh.sh` passes. Against this product, `extern_error_catch_smoke.sh` passes Stage0/Stage1 object and runtime parity, `catch_result_type_smoke.sh` returns 84, and `catch_exhaustive_smoke.sh` passes. The 111-test parser AST suite also passes with the new exact-family/subset assertion. This is the final targeted green set for the imported metadata fix; none of these tests closes the composite-set backend gaps above.

**Compiler worktree reconciliation and cleanup (2026-09-24):** the detached `/private/tmp/elisa-compiler-f50ecf8` candidate had no unique commits. Its dictionary fallback, closure-storage dependency, extern error-family metadata, and statement-catch parity gains were already committed on main; its parser family-scanner correction and stronger structural regression are now committed at `e3c29e73`. The minimal subset-catch crash repro and declaration-only composite-signature control were copied into main and committed at `b65ca3f7`. The synthetic composite backend was explicitly excluded after fresh-product parity checks and source review found regressions and family-order, family-selection, and selected-payload-layout mismatches; those remaining requirements are recorded above. After stopping the candidate's repeated seed and confirming no process or open working directory still used it, the worktree was removed. At that 2026-09-24 cleanup checkpoint, `git worktree list` contained only compiler main; Elisa-core also contained only its canonical main checkout. The current topology is recorded in the 2026-09-25 gains review. The unrelated untracked main-checkout files `.log` and `test/repro/field_ref_shadowed_param.elisa` were left untouched.

**Composite-signature parser identity milestone (2026-09-25):** the extern-header scanner now records the full source spelling of a composite or restricted `error[...]` clause under `__error_set_key`, paired with its declaring module under `__error_set_module`. `test_extern_composite_error_families_exclude_selected_variants` now asserts the exact key and module rows as well as the two families and three selected variants. `parser_ast_smoke.sh` passes all 111 structural tests using the pinned Stage0 compiler; the multi-family payload-catch repro also emits Stage0 LLVM accepted by `opt -passes=verify`. At this checkpoint only parser-side identity capture was qualified; later backend validation is recorded below.

**Verified composite error-set backend catch slice (2026-09-25):** integrated the candidate worktree's synthetic error-set registry, canonical family-name ordering, family-local variant restriction, fieldless ordinary-enum copying, selected-payload layout, and source-variant-to-union-row mapping for expression/statement catches and raises. Extern error metadata is now registered by source line as well, so a payload-bearing extern receives the same `%ErrSet` result type as a defined function. On a fresh main Stage1 product (SHA-256 `28bacadb2e152396f2ad13f040bc0723aff56bfd30cbc500394b782159326ccb`), `multi_family_payload_catch_smoke.sh` and `extern_composite_payload_catch_smoke.sh` pass Stage0/Stage1 runtime parity at `-O0` and `-O2`; both run `opt -passes=verify`. The defined-callee fixture reverses family order, restricts one family, excludes a wide array payload, and exercises fieldless and scalar-payload branches in both catch forms. The extern fixture exercises the payload struct signature and selected/unrestricted family combination. `catch_result_type_smoke.sh`, `catch_exhaustive_smoke.sh`, `extern_error_catch_smoke.sh`, `extern_error_family_catch_smoke.sh`, `aggregate_catch_result_smoke.sh`, and `qualified_error_set_smoke.sh` also pass (11/11 qualified error-set cases). The C-struct characterization remains red: Clang lowers the C stub to `i64 @external_probe(ptr, i64)`, while both Elisa compilers declare `{i32, i32} @external_probe(ptr, i64)` / `%ErrSet...`; linking that C stub returns process status 1 instead of the fixture's expected 42. The LLVM-level stub proves only Stage0/Stage1 parity for their current raw LLVM ABI and does not prove C ABI interoperability. Module-qualified composite ownership, duplicate family/variant spellings, aggregate payloads, raise/catch behavior across modules, and actual C ABI adaptation remain open. This is a narrow backend correctness milestone, not memory-safety qualification.

**Verified restricted-composite expression-catch regression (2026-09-25):** imported the candidate's minimized `catch_subset_minimal.elisa`, changed the catch from statement form to `return catch`, and added an executable `main` plus a typed LLVM ABI stub. The smoke executes a restricted two-family payload catch and confirms the canonical family order (`BarError` before `FooError`) gives `BarError.Bad4` discriminator 2 and payload 42. `composite_subset_expression_catch_smoke.sh` passes Stage0/Stage1 at `-O0` and `-O2`; both generated modules pass `opt -passes=verify`, and Stage1 emits no decline marker. This closes the exact previously crashing expression-catch shape only; native C ABI adaptation and the broader module/aggregate matrix remain open.

**Verified S18 payload error-set direct C return slice (2026-09-25):** Clang characterized the native C definition `ElisaErrorSet external_probe(int64_t *, int64_t)` as `i64 @external_probe(ptr, i64)` on arm64, while Elisa's internal restricted composite status remains `{ i32, i32 }`. The shared C return classifier now accepts an LLVM aggregate type directly, declares this extern with the native return type, and normalizes named extern error calls by storing the returned register value into a target-aligned slot and loading Elisa's internal status aggregate. The adapter is used by ordinary named calls, expression/statement `catch`, propagating `try`, `try ... else`, and error-union-value calls. `extern_composite_payload_c_abi_smoke.sh` links the real C stub and returns 42 for both a direct restricted-composite `catch` and a propagating `try` from both Stage1 LLVM and object outputs at `-O0`/`-O2`, with `opt -passes=verify`; `composite_subset_expression_catch_smoke.sh` likewise passes using a C stub for Stage1 and preserves the Stage0 LLVM-signature baseline. `extern_composite_payload_catch_smoke.sh` and the focused composite/error-set regressions pass. The old internal-LLVM stub is intentionally no longer treated as a Stage1 C-ABI oracle. This validates the arm64 direct `{i32, i32}` register return and named-call routes; indirect/function-valued extern calls, wider sret sets, the C layout contract for multiple payload variants, non-arm64 execution, and ownership-safe wrappers remain open.

**Verified S18 payload error-set sret return slice (2026-09-25):** the same classifier selects a typed hidden return pointer for a 32-byte `{ i32, i64, i64, i64 }` status. Extern declaration and call sites carry the LLVM `sret` type attribute, and named-call argument validation accounts for the hidden status pointer plus the success-output pointer. `extern_payload_error_set_sret_smoke.sh` links a real C function returning the matching four-field struct, verifies LLVM IR, and returns the selected third payload value 41 from both Stage1 LLVM and object outputs at `-O0`/`-O2`. The direct-register smoke, restricted-composite expression-catch smoke, multi-family and aggregate-catch tests, and `backend_native_smoke.sh` (521/521) pass after this change. This remains target-limited evidence for arm64 and named calls; external error function values/indirect calls, layouts whose C representation differs from Elisa's concatenated payload fields, x86-64/Wasm runtime checks, boundary ownership/lifetime contracts, and the remaining S18 audit are open.

**Verified S04 zeroed non-null-reference containment slice (2026-09-25):** a new mandatory semantic diagnostic rejects `zeroed` or omitted initialization for direct and alias-resolved non-null reference local declarations, including references reached through storage qualifiers, parentheses, and refinement wrappers; optional references retain zero/null as their valid absent representation. The check also covers direct global declarations and walks current nested statement bodies. It follows lexically collected type aliases with later declarations shadowing earlier ones and a bounded, fail-closed cycle/depth guard. A pre-fix Stage1 repro for `pointer: mutable i64&` with no initializer emitted `store ptr null` and a later `store i64 42` through that loaded pointer, so the check now includes omitted local initialization in this containment boundary. `test/parity/zeroed_reference_safety_smoke.sh` verifies that direct local, two-link alias, direct global, and omitted-local negative fixtures fail at `-O0`/`-O2`, emit the diagnostic, and leave no LLVM file. Its initialized-reference control, alongside a global zeroed optional reference, compiles and returns 42 with Stage0 and Stage1 at both optimization levels. A fresh Stage1 product (SHA-256 `4cc059fefb84081fc75cae6b10acf3710f537e50655e73a409ed7729823276b8`) passes `assert_stage1_fresh.sh`. `mutable_ref_scope_smoke.sh`, `arena_runtime_lifecycle_smoke.sh`, `arena_alloc_size_overflow_smoke.sh`, and `arena_cache_concurrency_smoke.sh` also pass; allocator tests exercise Stage0/Stage1 and their documented O0/O2 coverage. `bash -n`, `git diff --check`, and freshness checks pass. To remove an existing unsafe placeholder, `arena_take_free_block` now creates its split-header reference directly from the checked block offset through a narrow internal pointer helper; the scoped mutable-reference regression now starts from `null` in an optional reference. This is declaration-level containment, not a complete zero-validity or initialization rule: fields inside aggregates, aliases across imports/qualified module names, generic substitution, all expression/declaration forms, and path-sensitive reads/calls/returns are not covered by this slice. Stage0/Stage1 agreement is positive control evidence only; no full-gate, sanitizer, other-target, or memory-safety qualification is claimed. Keep S04/S10/S15 open.

**Fresh-product follow-up (2026-09-25):** the omitted-initializer diagnostic and its regression are included in the later fresh Stage1 product SHA-256 `a22e01fe780f7cdc1169edf31961b788baf86030c209be6e1aa9f8dabe432308`, which passes `assert_stage1_fresh.sh`. The expanded zeroed-reference suite, mutable-reference scope regression, and sview-region-tie suite pass on this product; the zeroed suite's valid initialized-reference/nullable control runs against Stage0 and Stage1 at `-O0`/`-O2`. The full `arena_runtime_lifecycle_smoke.sh`, `arena_alloc_size_overflow_smoke.sh`, `arena_cache_concurrency_smoke.sh`, and `runtime_string_view_safety_smoke.sh` pass on both compilers, with their `-O0`/`-O2` cases where specified. A direct Stage1-only rerun also covered those fixtures. This is still focused host evidence; full repository gates, sanitizers, other targets, and all broader S04/S10/S15 obligations remain open.

**Verified S04 aggregate zeroed-reference containment follow-up (2026-09-25):** the declaration check now follows aliases, storage qualifiers, parentheses, refinements, named-tuple elements, fixed-array element types, and recursively nested by-value struct fields. The expanded `test/parity/zeroed_reference_safety_smoke.sh` rejects direct fields, a three-level nested struct whose leaf field type is aliased, named tuples, and fixed arrays spelled `(mutable i64&)[2]`, alongside the prior direct/alias/global/omitted-local cases. The array test uses explicit parentheses because unparenthesized `i64&[2]` parses with a different shape. All negative cases require the semantic diagnostic and absence of LLVM output at `-O0` and `-O2`; an explicitly initialized reference-field struct and a zeroed optional-reference-field struct compile and return 42 on Stage1 at both levels. The smoke passes on Stage1 product SHA-256 `cc2f7a2247f506f2587c7143b0037413c75c9e26abd8f9ff95ef171bfe705ef9`; `assert_stage1_fresh.sh` passed at that checkpoint. `backend_native_smoke.sh` also passes 521/521 on product `72593507c5293deddf36230a8b446bd429cb905f4c5018caf745596b69bff649`, using the prior Stage0 only as an explicitly allowed test harness seed. The focused check confirms an open generic substitution gap: `test/repro/zeroed_nonnull_reference_generic_field.elisa` is accepted and emits a `%RefBox` `zeroinitializer` when `T` is instantiated as `mutable i64&`. Keep generic instantiation, enum zero tags/payloads, imported or shadowed nominal identity, assignments/returns/call arguments and other non-declaration value boundaries, and full CFG initialization open. The Core implementation covers more resolved types and now passes its focused semantic package, but `go test ./...` fails with standard-runtime placeholder migration and type issues described above; do not call this worktree integrated. The current Stage1 product predates subsequent runtime-standard-library edits and must be rebuilt before qualification. No full gate, sanitizer, Gen2, other-target, or memory-safety claim is made.

**Broader current-product check (2026-09-25):** `backend_native_smoke.sh` passes 521/521 native compile-and-run cases on the same fresh Stage1 source state, with its documented Stage0 skips retained. This is native backend compatibility evidence, not a full repository gate or a soundness qualification.

**Composite-catch candidate worktree reconciliation (2026-09-25):** `/Users/torarinvikbjarko/Documents/Coding Projects/Elisa Projects/elisa-compiler-worktrees/composite-error-catch` was audited before removal. Its branch tip `4f3f7354` is an ancestor of main `449f7e56`, so it had no unique commits. The dirty source manifest was compared against current main: the untracked synthetic registry and multi-family smoke matched integrated files byte-for-byte; remaining differences removed newer parser-key assertions, executable/reproducer coverage, and the verified composite-catch tests, or restored an older extern-family lookup. Those regressions were excluded. After confirming no process still used the path, the candidate worktree was removed. The detached compiler checkout at `/Users/torarinvikbjarko/Documents/Coding Projects/Elisa Projects/elisa-proof/build/toolchain/elisa-compiler-4f3f7354` is clean and has no unique commits, but remains registered as a dependency of the active proof build; it is not a source candidate and was retained.

**Verified S06 compiler-local arena-handle absence slice (2026-09-25; compiler commit `f8c38983`):** local declaration arena pins and deferred-cleanup snapshots now use `LLVMValueRef?` with `null` for “no explicit arena pin,” eliminating the parallel `local_has_declared_arena` boolean array. Call-site growth-arena selection likewise stores the best candidate as one optional handle instead of a zeroed LLVM handle plus `have_best`. The focused source diagnostic probe over the compiler found 189 remaining initialization/read diagnostics (81 invalid `zeroed` initializers and 108 uninitialized reads), down from 193 (84 and 109 respectively); `codegen_locals.elisa`, `codegen_cleanup.elisa`, and `codegen_growth_arena.elisa` now have none of those findings. The source probe still fails on those remaining findings, so this is not a clean-source analyzer pass. A pinned-Stage0/fresh-Stage1 `loop_region_reuse_smoke.sh` run passes all 15 runtime parity fixtures; `get_index_else_recovery.elisa` agrees in the differential harness and runs successfully on Stage1 at `-O0` and `-O2`. `assert_stage1_fresh.sh` passes for Stage1 SHA-256 `a8deb73bcecedf41938d612da07247a292d8f675db0a68f406f50db5ebded1d5`; runtime object SHA-256 is `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`. The deferred outer-region-growth fixture remains unqualified: pinned Stage0 rejects it and Stage1 reports a region-lifetime diagnostic, so it is not a parity/runtime pass. No WASM artifact-parity, full gate, sanitizer, or cross-target result is claimed; S06 remains open for canonical identity, complete flow, reset epochs, and all storage/call contexts.

**Verified S04/S10 generic-call handle initialization slice (2026-09-25; compiler commit `991f835b`):** generic-call and generic-UFCS lowering now represent sret destination slots and emitted call instructions as optional LLVM handles rather than `zeroed` placeholders. The code only passes an sret destination after successfully unwrapping the optional, and returns no emitted expression if a required handle is absent; it no longer relies on control-flow-sensitive reads of a maybe-uninitialized sret slot. The strict compiler-source probe drops from 189 initialization/read diagnostics (81 invalid `zeroed`, 108 uninitialized reads) to 183 (77 and 106); `codegen_generic_call.elisa` has no remaining findings in either category. A direct generic `darray[T]` return and a generic UFCS function returning a five-field `Big[T]` both compile, link, and return their expected values with pinned Stage0 and fresh Stage1 at `-O0` and `-O2` (8 executions total). The new `generic_ufcs_large_sret` case is added to `backend_smoke_behavior.sh`; the existing `generic_returns_darray` case covers direct generic sret. `assert_stage1_fresh.sh` passes for Stage1 SHA-256 `c74732dffd00525270bca5d035a0710343f4aa46c1f20ffe3f24be8686a0a7f2`; runtime object SHA-256 remains `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`. The full `backend_native_smoke.sh` harness did not run: its preliminary Stage0 build of `test/breadth/emit_native.elisa` fails on an existing `LLVMTargetMachineRef`/`void&` signature mismatch, before behavioral cases start. The remaining 183 source initialization/read findings and broader value-validity boundaries remain open; no full-gate, sanitizer, or cross-target claim is made.

**Verified S08 closure-environment type presence slice (2026-09-25; compiler commit `9596fef2`):** closure construction now represents the lifted capture-environment `LLVMTypeRef` as optional because noncapturing lambdas have no environment struct. The environment allocation and stores first unwrap the type; capture loading also requires it and declines/strips the lifted function body if captured values exist without a type, instead of generating capture loads from an absent type. This removes the invalid `zeroed` environment-type initializer and its conditional uninitialized read. `backend_obj_smoke.sh` now includes a Point-valued capture regression. The equivalent no-capture lambda, scalar capture, Point capture, and returned higher-order closure each compile, link, and return 42 using pinned Stage0 and fresh Stage1 at `-O0` and `-O2` (16 executions). The fresh Stage1 product SHA-256 is `8b3eda1a9fefd0f3fc552e5a4f8d27136da2fb18a9430f069710c341a70d8e08`; `assert_stage1_fresh.sh` passes and the runtime object remains `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`. The strict compiler-source probe falls from 183 findings (77 invalid `zeroed`, 106 uninitialized reads) to 181 (76 and 105); no findings remain for the environment type in `codegen_fn_values.elisa`, though other function-value dispatch slots in the file remain open. The standalone `backend_obj_smoke.sh` harness was not run because its Stage0-built driver fixture currently fails earlier on the LLVM target-machine C API type mismatch recorded above. This is an explicit absence/value-initialization slice, not closure lifetime, ownership, ABI, or callback safety completion.
...
- **Ancestor-relative Core identity milestone (2026-09-26, Core `45ea9245`; Stage1 open):** semantic and backend candidate generation now traverse lexical namespaces from nearest to outermost before alias and explicitly written candidates. Full semantic/backend packages pass. The regression resolves `Right::Message` from `Outer::Worker` to `Outer.Right.Message` while a conflicting root enum has a different payload arity. Candidate `/tmp/elisac-core-relative-candidate` SHA-256 `39f32c289646260dab0b40c33c37ed56cf912a2f6261729cc54044425e35abfc` was built before the commit and executes result 42 with verified LLVM at O0/O2; a wrong-owner return rejects without LLVM/object at O0/O2. Fixtures are preserved by `4efda861`. Stage1 still rejects the ancestor-relative qualifier: its semantic expression walker checks only the immediate `table.current_module`; its backend owner matchers also require a coordinated ancestor precedence audit. Full ancestor identity is therefore incomplete.

- **Measured ancestor-relative scalar identity defect (2026-09-26, still open):** `test/repro/enum_module_ancestor_scalar.elisa` gives root `Right.Message.Ready` ordinal 1 and ancestor `Outer.Right.Message.Ready` ordinal 0. Core candidate `39f32c289646260dab0b40c33c37ed56cf912a2f6261729cc54044425e35abfc` emits the ancestor tag 0. Retained Stage1 `ca466bb5` accepts the program but emits `store i32 1` and compares against 1 inside `Outer::Worker.answer`, selecting the root enum. Runtime result 42 would falsely appear correct because construction and comparison share the same wrong row. Close this with declaration-specific tag assertions, coordinated scalar annotation/constructor/pattern ranking, nearest-owner and wrong-owner controls; do not infer scalar correctness from the payload-only candidate. Packed head resolution and hierarchy alias rows retain the same direct-or-immediate-enclosing first-row strategy and require equivalent ranking/representation checks.

- **Independent ancestor scalar value oracle:** `enum_module_ancestor_const_values.elisa` uses `const enum Message of i32` and tests the extracted numeric value against literal zero as well as declaration-relative equality. Core candidate `39f32c289646260dab0b40c33c37ed56cf912a2f6261729cc54044425e35abfc` returns 42 at O0/O2; retained Stage1 `ca466bb5` returns 1 at both levels. Unlike paired constructor/pattern comparisons, the literal-value check cannot pass when both operations choose root tag 1. Preserve the ordinary enum reproducer separately: ordinary enum numeric casts are rejected by Core, and const-enum `is` syntax is not a variant-pattern substitute. Correct scalar annotation identity and head lookup before adding this oracle as a passing acceptance gate.
