...
| S14 | Operation-specific unsafe authority | In progress — lexical grants are operation-specific for aliasing, unchecked indexing, mutable globals, pointer arithmetic, raw extern calls, pointer casts, buffer reinterpretation, and stale-view reads; nested stale-view invalidation is propagated. Direct named calls, function-typed parameters, explicitly typed local callback aliases, and bare local callback `<-` rebinds check concrete Unsafe rows against exact active lexical grants, including contract-expression calls and branch joins. Function-type rows are retained in parser metadata keyed by the fn token offset. The transferred visitor follow-up adds statement `Match` traversal to the strict unsafe, pointer-arithmetic-grant, local-alias, and raw-extern walks; each arm gets a copy of incoming borrow facts and post-match facts are invalidated when any arm rebinds a tracked borrowed slot. The 49-case unsafe-grant smoke passes, including negative and exact-grant Match-arm cases. Generic functions now treat `.cast[...]` and generic type-constructor conversions as representation-unsafe, including casts after a parameter is copied to a local; same-base borrow identity and mutability weakening remain allowed when they do not add write capability, and explicit local grants still work. The 13-case generic-cast smoke passes, and the 581-case stage0 acceptance diff remains green. The cast walk is still partial for statement `Match`/`Contract` and nested expression forms; aggregate/field/global callback flow, method/UFCS dispatch, imported identity, generated operations, and the full operation inventory remain open. The flow is name-based and conservative, not the planned canonical binding-ID/dataflow analysis. |
| S15 | Sound proof/refinement/assertion handling | Planned |
| S16 | Effect, type, module, and global semantic closure | In progress — reference read/write capability and binding/rebind rules have positive/negative diagnostic fixtures; the 406-fixture diagnostic suite passes. `zeroed` construction now checks nominal private-field ownership so it cannot bypass a module-private type's constructor boundary; the 23-case private-fields smoke passes on stage0 and stage1. The runtime-carrier builtin exception for `Region` and `ArenaMark` now permits their source declarations while still diagnosing duplicate source declarations; the duplicate-declaration smoke passes, including the frontend/stdlib false-positive scan. Full type/effect/module/global closure remains open. |
