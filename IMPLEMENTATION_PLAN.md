...
**Verified qualified enum-valued field propagation case (2026-09-25):** `backend_smoke_behavior.sh` now covers a module-qualified decoder returning a const enum into a record field, then checks successful storage and that a later decode failure leaves the successful value intact. This case passes in the Stage1 native backend suite and the matching standalone fixture passes on Gen2. The full `backend_native_smoke.sh` run reached 520/521: its sole failure was the separate `packed_recursive_new_is_accepted` Stage0/Stage1 acceptance divergence (Stage0 accepts; Stage1 returns 2); that recursive-enum case does not use record-field assignment, and its cause remains open. The broad backend suite is therefore not fully green despite the new field-propagation case passing.
