...
| S10 | Arithmetic/conversion/value validity consistency | In progress — stage1 previously left unsupported address-of expressions in mutable global initializers at LLVM's implicit null value even though stage0 rejects them. Stage1 now marks that initializer unsupported and refuses the module; `global_ref_initializer_reject_smoke.sh` verifies both compilers reject without writing an object, stage1 reports the rejected global name, and both still accept/emit an ordinary constant global. This closes one expression shape only; the full global-initializer validity matrix, arithmetic edge cases, conversions, and target-width behavior remain open. |
