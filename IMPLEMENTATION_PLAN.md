...
The entries below identify tests to add unless an existing fixture is named explicitly. `test/repro/catch_multi_family_payload.elisa` is an existing characterization repro, not yet a passing backend gate. Place remaining cases under the current fixture/parity conventions or a new `test/safety/` harness integrated into `run_all.sh`.
...
| `extern_error_family_catch` | An extern is recognized as fallible, but its error-family row is absent when lowering ABI/catch dispatch. | Preserve family metadata and compile fieldless and payload catch paths on Stage0/Stage1; currently the fieldless case is covered, payload/layout variants remain open. |
