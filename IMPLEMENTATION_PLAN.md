...
| S10 | Arithmetic/conversion/value validity consistency | In progress — stage1 previously left unsupported address-of expressions in mutable global initializers at LLVM's implicit null value even though stage0 rejects them; it now marks that initializer unsupported and refuses the module. The global materializer also preserves signed floating-point literal spelling (including `-1.5`) instead of silently retaining LLVM's zero initializer; `backend_smoke_differential.sh` now contains a stage0/stage1 behavior case for this. The prior reject smoke verifies both compilers reject unsupported references without writing an object, stage1 reports the rejected global, and both still accept an ordinary constant global. This closes two expression shapes only; the full global-initializer validity matrix, arithmetic edge cases, conversions, and target-width behavior remain open. |
