...
- **Generic zeroed-reference substitution requalified on current Stage1:** the previously recorded generic-field bypass repro now fails on the current fresh product at both `-O0` and `-O2`, with the specific `pointer` field diagnostic and no LLVM file. The nearby `zeroed_generic_scalar_field.elisa` positive control compiles and returns 42 at both optimization levels under the 2 GiB Stage1 RSS cap. This supersedes the older `cc2f7a...` checkpoint's statement that this exact `RefBox[T]` instantiation was still accepted; it qualifies only this struct-field substitution boundary, not enums, all generic aliases, imported identity, other value boundaries, or CFG initialization. S04/S10/S15 remain open.
- **WASM facade width coverage and safer parity execution:** commits `9df7d12b` and `95dd8921` add edge-width parity coverage and signedness/conversion checks for i8/u8/i16/u16/i32/u32/i64/u64; `python3 test/parity/wasm_build_unit.py` passes all 34 tests. Commit `9b8ba316` makes the compiler-vs-Python artifact test require `ELISASCRIPT_VALIDATION_REAUTHORIZED=1`, an explicit Stage1 RSS cap no greater than 2 GiB, and this checkout's fresh Stage1 binary. Commit `99624055` replaces the platform-specific timeout executable with a Python supervisor that launches each command in a fresh session, samples combined process-tree RSS, bounds each command to 300 seconds and captured output to 64 MiB, and terminates the process group on limit or signal. The parity runner compares four fixtures, including the new width-edge case, across Wasm and all sidecars. `bash -n`, the 34 Python unit tests, the Stage1 freshness assertion, and the no-reauthorization exit-125 guard pass. The full artifact comparison remains pending because its explicit execution gate was not opened; do not count unit tests or the supervisor's guard as artifact parity evidence.
