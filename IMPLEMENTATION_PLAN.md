...
**Composite-error candidate probe (2026-09-24):** the candidate later added a synthetic error-set table builder and a fresh Stage1 seed. The extern fieldless statement-catch parity smoke passed on that product, but `catch_result_type_smoke.sh`, the defined-callee composite runtime at `-O0`/`-O2`, and the extern composite-subset repro still failed/declined. With decline tracing enabled, the candidate reported `COMPOSITE missing family Bad4`: `capture_header_error_families` currently treats the comma before the second selected variant inside `BarError{Bad3, Bad4}` as a family-level comma. The parser must separate top-level family delimiters from variant-subset delimiters before a composite registry can succeed. Stage0's `CanonicalizeErrorSetSelections` sorts family names before building tag codes and preserves each family's declaration order; source-order registration is not equivalent. This candidate remains unimported pending these fixes and exact ABI/runtime parity.
