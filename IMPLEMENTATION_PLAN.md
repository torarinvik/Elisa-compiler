...
**Open follow-up (2026-09-24, closure over relocatable darray storage):** `test/repro/sview_closure_after_growth.elisa` captures `bytes_view(values)` in a function value, grows `values`, then calls the closure. The current Stage1 product still emits LLVM at `-O0`, so the arena-destruction closure work above does not yet prove relocation safety for closure-held views. A corresponding unrelated-darray-growth control is needed when this is fixed. A first capture-provenance attempt reused the flat identifier collector and failed the Stage0 self-host seed: same-spelled lambda parameters and body locals were mistaken for outer captures, producing false `StorageDependencyInvalidated` diagnostics in backend and driver sources. That attempt was removed; the fix must resolve lambda free variables with lexical binding/scope information and carry each captured view's backing-owner provenance into the callable binding before growth invalidation. Keep this as an S06 blocker; do not infer closure safety from the arena-only tests, and do not claim a memory-safety qualification.
