...
**Progress (2026-09-23):** `Semantic::DeclId` now wraps the row identity assigned when a builtin or collected declaration enters `SymbolTable.symbols`; `Symbol.id` owns it, and `DefinitionReference` carries the typed identity rather than a raw index. `sema_smoke` now verifies row/index agreement for every collected symbol and validates a resolved function edge against its target row. The smoke passed with both stage0 and freshly seeded stage1; `assert_stage1_fresh.sh` and `semantic_gate_selfhost_smoke.sh` also passed. The `Symbol` field order follows the compiler's compact layout to avoid adding padding. This is only the declaration-identity foundation: lexical binding identities under shadowing, source-origin coverage for all binders/generated declarations, nominal type identity, region/loan/block identities, generic substitutions, cache fingerprints, and typed safety qualifiers remain open. Keep the S01 acceptance box unchecked until those obligations and test evidence are complete.
...
|---|---|---|
| S01 | Canonical identities and safety-qualified types | In progress — typed table-local `DeclId` is stored on every symbol and carried by `DefinitionReference`; the extended `sema_smoke` passed with stage0 and fresh stage1, plus the stage1 semantic gate. Lexical `BindingId`, other safety IDs, source origins, type qualifiers, substitutions, cache keys, persistent identity, and full acceptance remain open. |
...
| S10 | Arithmetic/conversion/value validity consistency | Planned |
...
| S12 | Packed-store and pool handle correctness | In progress — source inventory begun for raw pool pointers, the affine `Pooled[T]` region-pool wrapper, packed-store region/handle/index/tag tables, and dense/sparse row access. Generation, row-bound, store-identity, and target-width obligations remain unverified. |
| S13 | Unified cleanup semantics and lowering | Planned; existing cleanup implementation retained until validated replacement |
| S14 | Operation-specific unsafe authority | In progress — lexical grants are operation-specific for aliasing, unchecked indexing, mutable globals, pointer arithmetic, raw extern calls, pointer casts, buffer reinterpretation, and stale-view reads; nested stale-view invalidation is propagated. Direct named calls, function-typed parameters, explicitly typed local callback aliases, and bare local callback `<-` rebinds check concrete Unsafe rows against exact active lexical grants, including contract-expression calls and branch joins. Function-type rows are retained in parser metadata keyed by the fn token offset. The transferred visitor follow-up adds statement `Match` traversal to the strict unsafe, pointer-arithmetic-grant, local-alias, and raw-extern walks; each arm gets a copy of incoming borrow facts and post-match facts are invalidated when any arm rebinds a tracked borrowed slot. The 49-case unsafe-grant smoke passes, including negative and exact-grant Match-arm cases. Expression recursion remains partial; aggregate/field/global callback flow, method/UFCS dispatch, imported identity, generated operations, and the full operation inventory remain open. The flow is name-based and conservative, not the planned canonical binding-ID/dataflow analysis. |
| S15 | Sound proof/refinement/assertion handling | Planned |
| S16 | Effect, type, module, and global semantic closure | In progress — reference read/write capability and binding/rebind rules have been added with positive/negative diagnostic fixtures; the 406-fixture diagnostic suite passes. Full type/effect/module/global closure remains open. |
...
The packed-store state is an opaque `void&` at helper boundaries and carries allocation/decode regions, region indexes, raw pointer handles, row indexes, variant tags/slots, and prefix/side columns. Dense and sparse readers contain assertions for several index/region/capacity conditions, but `ctx_packed_store_read_variant_sparse_word` indexes the row using `word_offset` without a visible row-width check, and packed handle decoding validates capacity rather than an allocation generation or row extent. These are concrete validation obligations for S12, not yet demonstrated safe-code exploits: trace all callers/codegen-provided offsets, store reset/reuse behavior, 32-bit shift/narrowing semantics, and generated layouts, then add malformed-handle/offset, cross-store, reset/reuse, and target-width tests. Neither the allocator nor packed-store audit has yet exercised allocation-failure injection, sanitizer instrumentation, or the backend/target matrix.
