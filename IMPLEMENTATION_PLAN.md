...
**Inspection baseline:** compiler main `34c52dde` and Elisa-core main `06bb1c0f`; both contain committed zero-validity containment milestones, and Core now also commits the focused `AtomicSlot` lowering/validation slice. The detached proof-toolchain checkout remains clean at `4f3f7354`, an ancestor of compiler main, and is still referenced by `elisa-proof/AUDIT.md`; its gains are already in main, but retain the pinned build directory until that audit dependency is advanced and validated. Current Stage1 product SHA-256 is `4fa872b7e16c405fbb38b2f7e267362a9264900f27a082538c3f61d49b198ee2`; runtime object SHA-256 is `be0cc12439b3d4e473c0b924b447bb4665f0abf0237e8bab7db393077277c7aa`. `assert_stage1_fresh.sh` passes its mtime check and both targeted Stage1 atomic and sview smokes pass without the stale-product override, but timestamp freshness is not a source-content manifest and a recent fresh-seed attempt failed in Stage0 on the `Store.store_get` implementation signatures. The verified Core Stage0 hash remains `319ca9b80915a52e9a8b65211d47c9f64c12b63680ee78dc50540e2abe5ffae5`; `assert_stage0_fresh.sh` passes. Core commit `06bb1c0f` adds `AtomicSlot` recognition to its payload validation and LLVM lowering, updates safety-invalid test fixtures, and passes `go test ./src/semantic ./src/backend -count=1`; the Core same-name ordinary-function control remains missing, and dispatch is still partly spelling-based. The last uncached `go test ./... -count=1` run completed red in `elisacore/src`, `test/backend`, and `test/semantic`; see the distinct runtime and fixture diagnostics below. No full memory-safety audit, uncached acceptance-gate pass, or sanitizer run is claimed. Historical green counts are historical evidence only. Every implementation item below starts pending unless explicitly described as existing infrastructure; a named checker is not evidence that its entire problem class is solved.

## Current gains review and revised priorities — 2026-09-25

**Review checkpoint (updated 2026-09-25):** compiler main `34c52dde` and Core main `88cea3f5`, with a fresh Stage1/runtime-object pair and Core Stage0 built from the committed zeroed-validity candidate. Atomic candidates are validated but not yet committed: the exact direct same-name overload regression is fixed, while generalized resolved intrinsic identity remains open. This snapshot reconciles current candidate diffs, backend consumers, focused tests, and prior results; it does not certify the current combined source tree. Earlier dated measurements retain their original compiler/source scope. No S01–S28 package is promoted to complete by this review.
...
- **Store borrow contract is a newly confirmed plan gap:** the next attempted Stage0 seed exposed that `Store.store_get` requires a `T& @r` result tied to an `s: darray[T]& @r` receiver, while the concrete Deque stores its backing buffer as an independent `mutable T&?` field with no region identity. Qualifying the receiver cannot prove the buffer's lifetime, and darray growth adds an invalidation boundary even while the allocator region remains alive. Do not close this by weakening conformance or blindly adding `@r`; either redesign the public Store surface so reads do not leak untracked element borrows, or first add an explicit backing-owner/loan model that prevents use after growth, replacement, destruction, or region reset. Add paired positive/negative tests for darray and Deque reallocation and owner destruction before treating the protocol as safe. This currently blocks a fresh Stage0 → Stage1 seed and therefore blocks the next compiler-source qualification milestone.
...
| Atomics (S17) | Native lowering, nominal MemoryOrder checks, invalid literal-order rejection, pointer/bool/integer coverage, and threaded CAS tests. A candidate `AtomicSlot` path preserves those tested instructions but has a reproduced name-based user-type/function miscompile; it is not a safety gain to retain as closed. | Resolved runtime type and function identity, faithful dynamic-order behavior, all pointer result bindings, race-free safe APIs, or supported-target qualification. Runtime experiments that rename `atomic[T]` to `AtomicSlot[T]` must update semantic fencing and backend intrinsic identity together; otherwise ordinary field accesses can replace LLVM atomic operations and introduce a data race. |
...
- [x] Reconcile current registered worktrees for this review: compiler main plus the clean detached proof-toolchain checkout at `4f3f7354`, whose commit is already an ancestor of main; Core has only its canonical main checkout. The composite candidate was previously reconciled and removed. The proof checkout has no unique source gains but remains an active proof-build dependency, so retain it until the pin/build is advanced and validated. The Core checkout has no additional source worktrees to merge; zeroed-validity milestone `88cea3f5` passes focused semantic/backend packages and paired bootstrap safety smoke, while the uncached full suite remains red on runtime migration and fixture compatibility. Separate atomic changes remain pending because the exact identity regression is unresolved; no worktree should be removed as part of this checkpoint. Older dated cleanup records describe their historical state, not today's topology. The pre-existing untracked `.log` and `test/repro/field_ref_shadowed_param.elisa` remain untouched.
