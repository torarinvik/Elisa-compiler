...
- **Next verified S01/backend blocker to implement:** `generic_struct_index_of` still selects the first basename and generic instantiation memoization uses `(basename, arguments)`. On this current product, `/tmp/elisa-generic-owner-followup.elisa` declares `Wrong::Box[T]` with `padding` and `item`, then `Right::Box[T]` with only `item`; a `Right::Box[i64]` local emits `%Box__i64 = type { i64, i64 }`, selecting the wrong layout. Replace template selection, instance memoization, recursion tracking, declaration-owner field lookup, and qualified argument provenance with a shared template declaration identity. Add reversed declaration-order and simultaneous same-argument instances, field-layout/ABI assertions, caller/template alias-shadowing arguments, and valid recursive generic controls. Do not infer this backend obligation is fixed from the qualified-generic zero rejection above. Keep S04/S10/S15 open for remaining representation families, enum/generic layouts, full expression boundaries, imported visibility and canonical TypeId facts, Stage0 parity, and CFG initialization; full acceptance/sanitizer qualification remains unproven.
