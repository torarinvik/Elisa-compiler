...
**Inspection baseline:** compiler main `730aaad6` and Elisa-core main `88cea3f5`; the zeroed-validity/definite-assignment milestone is committed, while separate atomic candidates remain uncommitted. The detached proof-toolchain checkout remains clean at `4f3f7354` and is still used by the active proof build. Stage1 product SHA-256 `66087ece0eb184e08a52d01fbd1696fa203b69d6b4e74ddf29d404b9d0397705` passes `assert_stage1_fresh.sh`; its self-built runtime object SHA-256 is `63f88713e1bf9621110dc10edbed9a8039edde7ad1a9d6932b583e6b08f89cf7`. Core Stage0 built from the committed zeroed-validity candidate as `compiler/bin/elisac`, SHA-256 `319ca9b80915a52e9a8b65211d47c9f64c12b63680ee78dc50540e2abe5ffae5`, passes `assert_stage0_fresh.sh`. The paired Stage0/Stage1 zeroed-reference safety smoke passes at `-O0`/`-O2`, and the committed Core semantic/backend changes pass their focused Go packages. The last uncached `go test ./... -count=1` run completed red in `elisacore/src`, `test/backend`, and `test/semantic`; that full run preceded only the narrow unresolved-generic fail-closed refinement, for which both focused packages and the paired safety smoke were rerun. See the current gains checkpoint below for the distinct root diagnostics. No full memory-safety audit, uncached acceptance-gate pass, or sanitizer run is claimed. Historical green counts are historical evidence only. Every implementation item below starts pending unless explicitly described as existing infrastructure; a named checker is not evidence that its entire problem class is solved.

## Current gains review and revised priorities — 2026-09-25

**Review checkpoint (updated 2026-09-25):** compiler main `730aaad6` and Core main `88cea3f5`, with a fresh Stage1/runtime-object pair and Core Stage0 built from the committed zeroed-validity candidate. Separate atomic work remains uncommitted pending the exact identity regression fix. This snapshot reconciles current candidate diffs, backend consumers, focused tests, and prior results; it does not certify the current combined source tree. Earlier dated measurements retain their original compiler/source scope. No S01–S28 package is promoted to complete by this review.

### Latest verified continuation — 2026-09-25

- **Stage1 atomic lowering:** with the then-fresh Stage1 product, direct emission of `atomic_runtime_operations.elisa` and the runtime-atomics codegen fixture produced LLVM `load atomic`, `store atomic`, integer and pointer `atomicrmw`, bool/float atomic accesses, pointer `cmpxchg`, and `fence seq_cst`. The real four-thread CAS increment executable returned success at O0 and O2. `atomic_invalid_order_smoke.sh` passed at both levels, including the new negative `AtomicSlot` raw-access case, invalid-order cases, and a different-signature `i32` overload control. This validates those runtime shapes only.
- **Confirmed atomic identity regression:** `test/repro/atomic_same_named_slot_user_function.elisa` defines a top-level user `AtomicSlot[i64]` and an ordinary `load` returning 42. The candidate Stage1 product instead emitted an atomic load from the field and the executable returned 0 at both O0 and O2. A module-qualified same-name type probe returned 42, but the root-name collision is a demonstrated name-based intrinsic miscompile. Treat this as a blocking S01/S17 defect. Do not count the AtomicSlot spelling checks or their current regressions as a safe integration milestone until type and function intrinsic identities are carried through resolution and verified against same-signature user controls.
...
- **Atomic identity remains blocking:** `atomic_invalid_order_smoke.sh` passes its invalid-order, raw-access, typed-order, and different-signature overload cases. The exact same-signature `AtomicSlot[i64]` user `load` repro still exits 0 instead of 42 at both O0 and O2. The passing smoke does not cover that regression; exact resolved type and function identity is still required before S01/S17 can close.
