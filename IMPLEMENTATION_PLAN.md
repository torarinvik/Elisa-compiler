...
| S06 | Complete region/lifetime provenance and invalidation | In progress — destroyed-region analysis tracks region origins through view constructors, returned-view helpers, local copies/aliases, and view parameters, then rejects uses after explicit `destroy`; `destroyed_view_lifetime_smoke.sh` covers stale aliases and live last-use controls at Stage1 `-O0`/`-O2`. A new deepest-region join propagates the innermost active origin through candidate arguments of view-returning helpers and through supported aggregate/branch forms. `sview_region_multi_argument_smoke.sh` rejects the reproduced `choose_second(outer_view, inner_view)` store into the outer region on Stage1; the current Stage0 oracle still accepts that exact case, so this is a Stage1-only safety gain and a recorded parity gap. `sview_region_tie_smoke.sh` rejects erasure or mismatch of an explicitly declared `sview @r` input tie and accepts matching `@r` plus legacy unannotated forwarding on both stages; core commit `c9070b2` preserves the `sview` region in resolved types. This remains syntax/name-based: returned-parameter identity is approximated by considering all candidate arguments, complete projections and wrappers, CFG joins, reset/reallocation epochs, BindingIds, and precise interprocedural provenance summaries remain open. Unannotated `sview` forwarding stays allowed for existing standard-library APIs, so callers rely on the partial use-site analysis until those summaries exist. |
