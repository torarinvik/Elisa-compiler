...
**Compiler candidate-worktree audit (2026-09-24, current):** The detached `/private/tmp/elisa-compiler-f50ecf8` candidate is based at ancestor `f50ecf85`. Its dictionary fallback lowering and regression artifacts are already integrated in main at `e5007c11`; the closure-storage dependency changes and their regression coverage are integrated at `a7d6a0db` and `b7ebe332`. The candidate's extern-signature family-row capture is now also integrated with a Stage0/Stage1 regression (`extern_error_family_catch_smoke.sh`). The remaining candidate-only backend edits change expression/statement catch tag lookup and payload mapping for composite error sets. They are not integrated: the expression/catch edits regressed the existing `catch_result_type_smoke.sh` when tested as copied, and the combined-set repro remains declined by a fresh Stage1 even after extern-family metadata is captured. The candidate's new error-tag helper is therefore not yet sufficient evidence for a safe import. Repros `test/repro/catch_subset_exhaustive_std.elisa` (extern call) and `test/repro/catch_combined_error_set_tags.elisa` (defined callee) preserve the known gap; no passing gate is claimed for them. `/private/tmp/elisa-compiler-proof-b7ebe` is a clean detached checkout at current `main` HEAD, with no unique source changes, but an external proof build still uses it. Keep both checkouts until the remaining candidate code is either repaired and verified or explicitly excluded, and until no process references either path; remove only after a fresh status/diff/process audit. Composite errorsets still need first-class canonical identity and ABI representation before their backend work can be considered complete.
