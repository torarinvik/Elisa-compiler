...
The expanded zeroed-reference smoke now passes on paired fresh Stage0 and Stage1 products. It covers direct/alias/global/return boundaries, nested generic and ordinary fields, tuples, fixed arrays, `sview`, and positive initialized, nullable, and scalar-generic cases. The self-hosted checker still does not establish universal checks for every assignment RHS, call argument, aggregate/enum constructor, imported-module boundary, resolved shadowing case, or AST form.
...
1. **Contain confirmed invalid-value access first (S04/S10/S15).** The Stage1 product inspected during this review accepted the following source at O0 and emitted `store ptr null` followed by `store i64 42` through that loaded pointer:
   ...
   **Current containment status:** the original local repro is rejected before LLVM output. Direct and top-level alias-chain locals, direct globals, omitted local initialization, direct/generic reference returns, nested generic and ordinary struct fields, named tuples, fixed arrays, and `sview` are covered. Positive controls include nullable references, initialized references/reference-bearing aggregates, and nested/phantom generic scalars. `arena_take_free_block` no longer seeds a non-null temporary reference from `zeroed`; its split header reference is constructed from the already checked slot offset inside its existing narrow unsafe helper. The current checker also rejects unresolved/non-struct generic layouts rather than assuming they are zero-valid. This closes only the measured representations. Keep S04/S10/S15 open for full resolved identity across imports/shadowing, generic enum/alias and every invalid representation, all expression boundaries and AST forms, complete uninitialized read/drop/call checks, and path-sensitive initialization.
