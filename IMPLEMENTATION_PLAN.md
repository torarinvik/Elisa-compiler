...
| S06 | Complete region/lifetime provenance and invalidation | In progress — destroyed-region analysis now tracks region origins through view constructors, returned-view helpers, local copies/aliases, and view parameters, then rejects uses after explicit `destroy`. `destroyed_view_lifetime_smoke.sh` passes the stale copied-view negative plus live and last-use-before-destroy positive controls at Stage1 `-O0`/`-O2`; the runtime view smoke also checks Stage0/Stage1 `-O0`. The analysis is still name-based and partial: binding identity, complete expression/API provenance, branch/loop joins, aggregate fields, resets, and implicit lifetime boundaries remain open. |
| S07 | Container, view, iterator, and lending contracts | In progress — the JSON DOM now keeps `JsonValue`, `JsonMember`, and raw run-copy helpers private inside module `Json`, so safe clients can no longer fabricate address/count variants or call the raw copy helper; `using Json` preserves ordinary unqualified API names. Raw array/object-run builders are temporarily private until an opaque builder can retain its owning arena safely. This is one API boundary only: view lifetime, arena reset invalidation, iterator invalidation, and contracts for other containers remain open. |
| S08 | Closure, defer, and effect capture safety | Planned |
| S09 | Complete bounds and safe pointer formation | In progress — stage1 view reads/stores now use count guards and known-length slices clamp start/count to the receiver extent; `slice_bounds_smoke.sh` passed 112 checks at `-O0` and `-O2` against stage0 and stage1. Runtime string-view operations now validate signed/target-width lengths and positive-length null data before pointer formation or reads; the direct LLVM literal-equality specialization now guards non-null before byte comparisons, and file writing rejects malformed views. `runtime_string_view_safety_smoke.sh` passes malformed negative lengths, positive-length null data, and checked null-copy panic cases on Stage0 and Stage1 at `-O0`/`-O2`; the JSON DOM representation and raw pointer-copy helpers are private, blocking the reproduced safe-code forgery path. These are exercised native paths only; positive forged pointer extents, JSON arena lifetime after reset/free, other indexable forms, target widths, pointer arithmetic, and proof/backend coverage remain open. |
...
**Verified S09 positive-length null StringView slice (2026-09-23):** the null-view probe initially crashed Stage0 at `-O0` in the direct `string_view_eq(view, "safe")` specialization even though ordinary runtime helpers had guards; the lowering bypassed those helpers and reached the byte comparison. `StringView.data` is now represented as a nullable, read-only reference in both the source runtime and the Go compiler's builtin carrier model. Runtime equality, indexing, slicing, copying, hashing, names, collections, context comparison, Wasm comparison, and file writing reject positive-length null data before pointer use; the LLVM literal fast path branches on both matching length and non-null data before loads/`memcmp`. `runtime_string_view_safety_smoke.sh` passes Stage0 and fresh Stage1 at `-O0`/`-O2` for negative lengths, positive-length null data across helper and specialized paths, and the checked `string_view_copy` panic. `go test -count=1 ./src/semantic ./src/parser ./src/backend`, Stage0/Stage1 freshness, and `check_runtime_drift.sh` pass. Safe source can still forge an out-of-range non-null pointer/length pair because the public carrier has no extent provenance; full carrier opacity, FFI validation, target-width execution, and general pointer-range proofs remain open.
