...
The compiler and Core runtime copies also carry an incomplete atomic-wrapper experiment: `atomic_cell[T]` constructs initialized `AtomicSlot[T]` and `AtomicCell[T]` values rather than a zeroed generic slot. The first audit found that the rename would bypass exact-name atomic recognition. Follow-up edits now include `AtomicSlot` in the self-hosted raw-operation fence, payload check, reference binding classification, and LLVM lowering, with corresponding Core type-safety, raw-operation, and backend handling. New Core-focused tests pass for payload rejection, raw-operation/order diagnostics, and emitted atomic load/store/RMW/CAS instructions. This is a promising integration slice, not yet a verified cross-generation gain: Stage1 has not been freshly built, so source agreement and runtime behavior still need validation. Preserve nominal primitive identity as an open S01/S17 requirement; same-spelled user types/functions must not acquire intrinsic lowering accidentally. The Stage1 product is stale, and the latest seed stops on the runtime zeroed-placeholder diagnostics below.

The expanded zeroed-reference smoke now includes direct and generic reference returns, `sview` returns and fields, plus aggregate field/tuple/array cases. Its current product guard fails before tests because Stage1 is stale; a deliberate stale-product probe also showed that the old binary accepts the new direct-reference-return case, which is expected because that binary predates the source change. A fresh seed attempt using the current Core compiler stops when it checks bundled runtime zeroed placeholders in `InlineVec[T, N]`, `DictBucket[K, T]`, `SetBucket[T]`, generic heap storage, and debug event/trace records. Attempts to bootstrap from the previous Stage1 product also failed: the O3 build hit the existing 4 GiB memory guard, while O1 reached link and reported unresolved compiler/runtime symbols. No fresh product was published. Treat all newly added cases as pending regression coverage until those runtime representations are handled and a fresh compiler runs them. The current source-level attempt still does not establish checks for assignment RHS values, call arguments, array/enum constructors, values crossing imported-module boundaries, resolved shadowing, generic aggregate substitution, or every AST statement/value form.
