...
| S07 | Container, view, iterator, and lending contracts | In progress — the JSON DOM keeps its pointer-bearing `JsonValue`/`JsonMember` representation, raw accessors, and builders private. Public parsing and traversal now use opaque region-indexed `JsonValueHandle[@r]` values; borrowed string/key results carry `@r`, and serialization ties copied output to the caller's separate output arena. The Stage1 smoke rejects direct handle or copied-view use after named `arena_free`, `arena_reset`, and `arena_rewind`; live accesses and serialization controls pass at `-O0`/`-O2`. An in-progress storage-alias extension in the shared checkout tracks views through copied containers and wrapper values; `sview_relocation_smoke.sh` passes, but `diagnostics_smoke.sh` fails while compiling existing compiler sources with many new `StorageDependencyInvalidated` diagnostics, so this candidate is not qualified and must be narrowed or corrected before integration. This closes one library boundary and direct named-invalidation paths only: the general borrow checker, alias/indirect reset, iterator invalidation, mutation/version rules, builders, and other containers remain open; Stage0 parity was unavailable for the JSON commit. |
