...
- Update S04/S10/S15 follow-up work: module-qualified direct aliases for non-null references, `sview`, and `id[T]` now have focused Stage1 rejection evidence, and optional zeroed values containing reference-bearing payloads have a parity/runtime control. Still open: import/`using` alias identity, nested-relative/reopened namespace resolution, aliases to aggregate fields and generic substitutions beyond the tested forms, enums and other niche/tag layouts, assignments/returns/call arguments outside the tested declaration/read path, complete path-sensitive initialization, and all broader object-validity boundaries. Keep these safety packages open. The next semantic task is still a canonical resolved representation query over compiler-resolved types; the shared syntax alias walker reduces drift but does not close that task.
...
- This closes the reproduced current/enclosing-relative declaration-tree alias bypass for the tested representation families. Keep S04/S10/S15 open for Stage0 parity; import/`using` aliases; reopened-module ambiguity beyond the exercised `extend` fragments; aliases nested in aggregate fields or generic substitutions; enum and other niche/tag layouts; untested assignment/return/call boundaries; complete path-sensitive initialization; and the canonical TypeId-based resolved-representation service. Closest-owner and enclosing-owner precedence now have focused shadowing controls; add resolver identity tests as the implementation moves away from this syntax-level walk.
...
- **Remaining S01/backend obligations:** row IDs are local compilation identities, not persistent canonical TypeIds across imports/caches. The globally unique imported-template fallback remains; explicit visibility, phantom argument nominal identity, and generic-function declaration-owner coverage require further work. The initially reported module-local duplicate-alias prerequisite was a test error: `Local` is a builtin state-marker type name, so its redeclaration correctly rejects. Renaming that alias to `Padding` restores the intended caller/template alias-shadowing test. The strengthened fixture uses template-local `Padding = i32` and `Item = sview`, with root `Padding = i64` and `Item = i64`; both pinned Stage0 and the saved Stage1 product return 42 at O0/O2. Stage1 O0 LLVM independently shows `{ i32, i64 }` and `{ i64, i32 }`; O2 folds main to `ret i64 42`, so representation assertions apply at O0. This is fixture/plan correction with no compiler source change. Recursive optional rejection is frontend recursion-limit evidence, not proof that every backend pending-layout path was reached. Keep S04/S10/S15 open for all representation families, enum/generic layouts, expression boundaries, imported visibility, canonical facts, Stage0 parity, and CFG initialization. Full acceptance, self-host generations, differential corpus, and sanitizer qualification remain unproven.
...
1. **Contain confirmed invalid-value access first (S04/S10/S15).** The Stage1 product inspected during this review accepted the following source at O0 and emitted `store ptr null` followed by `store i64 42` through that loaded pointer:
