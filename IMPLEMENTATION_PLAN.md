...
**Progress (2026-09-23, commit `6cd7d46f`):** The same binder IDs now flow through view aliases, lambda captures, explicit `destroy`, and recognized arena free/reset/rewind invalidations. The shadowing regression confirms Stage1 permits reading an outer-region value after destroying an inner region with the same spelling, while still rejecting a view from the inner region; the controls pass at `-O0` and `-O2`. Stage1 also now substitutes generic region results for a uniquely named, two-part qualified call such as `Json::json_handle_from_result`, closing the qualified-call bypass in the JSON arena mismatch repro. A fresh Stage1 Gen2 build passes, `destroyed_view_lifetime_smoke.sh` passes, and `region_scope_smoke.sh` agrees with Stage0 on all 21 carrier-less annotations; Stage0's semantic oracle accepts the outer-region control and rejects the stale inner-region control. This checker-local row ID is not a persistent RegionId, arena alias/reset handling remains partial, qualified-call resolution still has incomplete nested/overloaded module identity, and provenance remains syntax-driven rather than CFG-complete. S06 remains open.
...
|---|---|---|
| S01 | Canonical identities and safety-qualified types | In progress — typed table-local `DeclId` is stored on every symbol and carried by `DefinitionReference`; the declaration-only slice passed stage0 and fresh stage1 plus the semantic gate. The current lexical `BindingId` extension passes the stage0 `-O2` smoke, including parameter/use and nested-shadow mapping; the same `-O2` smoke passes with a fresh stage1 product and the freshness assertion passes. A Stage1 structural-type slice now gives known generic region arguments distinct `Region` leaves and uses structural identity at declaration initializers, including targeted call-result substitution. `RegionId`, `LoanId`, `BlockId`, collision-safe lexical region identity, complete source origins, complete qualifier/substitution rules, cache keys, persistent identity, and full acceptance remain open. |
