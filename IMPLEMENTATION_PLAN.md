...
| S17 | Race-free concurrency and scoped task lifetimes | In progress — the shared arena region-cache list, bounded retention metadata, and reclaimed-region hint are serialized by a runtime atomic lock on POSIX and Windows; the independent-arena concurrency smoke passes at Stage0/Stage1 O0 and O2, alongside allocator lifecycle and overflow regressions. This closes only that runtime-global slice: same-arena ownership, remaining concurrency runtime globals/APIs, target-specific thread support, scoped task cleanup, memory-order validation, race instrumentation, and deterministic scheduler checks remain open. |
