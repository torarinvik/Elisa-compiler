...
**Stage0 safety-gain reconciliation (2026-09-24):** the three Elisa-core commits cited in S09 are already present on its `main` history, so they were retained in place rather than cherry-picked. The `memsafe/ref-write-parity` and `codex/transpiler-local-stage0-latest` tips are ancestors of their respective main histories. The unique `memsafe/nullable-scalar-deref` commit `875d75bd` was reviewed against Stage0 main and excluded because its delta removes packed-store per-row extent checks and their regressions. Both repositories currently report only their canonical registered worktree; the temporary baseline comparison worktree was removed after reproducing the same two runtime failures there. The adjacent Elisa-core checkout has a separate in-progress packed-store field-layout edit and generated build directory from another task; they were left untouched and were not included in this compiler commit.
