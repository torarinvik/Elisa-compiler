...
- **WASM facade width coverage and safer parity execution:** commits `9df7d12b` and `95dd8921` add edge-width parity coverage and signedness/conversion checks for i8/u8/i16/u16/i32/u32/i64/u64; `python3 test/parity/wasm_build_unit.py` passes all 34 tests. Commit `9b8ba316` makes the compiler-vs-Python artifact test require `ELISASCRIPT_VALIDATION_REAUTHORIZED=1`, an explicit Stage1 RSS cap no greater than 2 GiB, this checkout's fresh Stage1 binary, and a non-retrying `timeout -k 5 300`. It compares four fixtures, including the new width-edge case, across Wasm and all sidecars. `bash -n` passes, the Stage1 freshness assertion passes, and an invocation without reauthorization exits 125 before compiler launch as designed. The full parity comparison remains pending because its explicit execution gate was not opened; do not count unit tests or the fail-closed guard as artifact parity evidence.
- **Uncommitted process-group supervisor candidate:** the current working-tree edit redirects parity commands from the committed non-retrying `timeout` to `test/parity/run_bounded_stage1_command.sh`. That helper is not yet qualified: this macOS host has no `/usr/bin/setsid` or `/bin/setsid`, so the helper would fail closed even after parity reauthorization. Keep the comparison pending and do not commit the runner swap until process-group isolation has a supported launch path on this host and the supervisor receives bounded, non-runaway validation.
- **Current worktree integration and retention:** compiler main contains all committed gains from the registered compiler candidate checkouts. `/private/tmp/elisac-engine-validation-v2` at `f7edb529` had no unique commits or source diff; after its engine smoke exited and `lsof +D` found no users, the redundant checkout was removed. The clean proof checkout at `f7edb529` also has no unique source changes but remains until active proof-task usage is resolved. Retain the separate `4f3f7354` proof checkout while it is the revision pinned by the proof audit. Core has only its canonical main checkout. Do not remove active or pinned dependencies as part of gain integration.
