...
| `catch_combined_error_set_tags` | Composite error sets lose family identity or use family-local ordinals/layout when dispatching a combined result. | Preserve full set identity and map tag, owner, allowed subset, and payload layout consistently for propagation plus statement/expression catches at `-O0`/`-O2`. |
