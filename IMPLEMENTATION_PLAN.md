...
The expanded zeroed-reference smoke passes on paired fresh Stage0 and Stage1 products for the previously covered direct/alias/global/return boundaries, nested generic and ordinary fields, tuples, fixed arrays, `sview`, and positive initialized, nullable, and scalar-generic cases. A new Stage1-only extension covers module-qualified non-null reference alias chains, same-spelled aliases with distinct module identities, storage wrappers, and a nullable twin. The self-hosted checker still does not establish universal checks for every assignment RHS, call argument, aggregate/enum constructor, imported or `using`-aliased module boundary, relative nested-module lookup, resolved shadowing case, or AST form.
