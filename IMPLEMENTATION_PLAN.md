...
**Inspection baseline:** compiler main `80a1a4ff` and Elisa-core main `79a9919e`; both have additional uncommitted implementation candidates. The detached proof-toolchain checkout remains at `4f3f7354` and is still used by the active proof build. The Stage1 product at SHA-256 `27f1fdb1072a1794f08f4020e2bcc08cde84819cc486997bb05be42bbc4ac6e6` was source-fresh and passed the expanded reference smoke before the newest generic-field checker edits; it is now correctly rejected as stale against `src/semantic/check_uninitialized_zeroed.elisa`. This is a source-informed implementation plan, not a completed soundness audit. The 2026-09-24 full gate run stopped at meaningful failures and is documented in §M.5. No full safety audit or sanitizer run is claimed. Historical green counts are historical evidence only. Every implementation item below starts pending unless explicitly described as existing infrastructure; a named checker is not evidence that its entire problem class is solved.

## Current gains review and revised priorities — 2026-09-25

**Review checkpoint (updated 2026-09-25):** compiler main `80a1a4ff` and Core main `79a9919e`, both with local uncommitted compiler-compatibility/safety candidates. This snapshot reconciles the current candidate diffs, backend consumers, smoke cases, and prior results; it does not certify the current combined source tree. Earlier dated measurements retain their original compiler/source scope. No S01–S28 package is promoted to complete by this review.
...
- **Core atomic tests:** `go test ./src/semantic ./src/backend -run 'AtomicSlot|AtomicMemoryOrder|LegacyRawAtomics|CompareExchange|Zeroed' -count=1` passed after the latest Core candidate edits. The full `go test ./...` remains red for the previously listed runtime zeroed-placeholder and C-string bridge issues.
- **Generic zeroed-field hole:** before the current source edits, Stage1 accepted `RefBox[mutable i64&] = zeroed` and emitted LLVM. The self-hosted checker candidate now carries generic type-argument substitutions through nested generic struct fields and adds this fixture to the negative smoke, paired with `ValueBox[i64] = zeroed` as a safe positive control. The candidate reached Stage0 source checking without errors in the new helper, but it has not been exercised by a Stage1 built from these edits: seed attempts stop in the bundled runtime, and `assert_stage1_fresh.sh` rejects the old product. Treat this containment change and its tests as pending until a fresh Stage1 run verifies both negative and positive cases.
- **Bootstrap boundary:** the older Core Stage0 seed rejects nullable `DebugRefereeEvent`/trace globals; the newer Core `elisac` proceeds further but rejects generic inactive storage in `InlineVec[T,N]`, `DictBucket[K,T]`, `SetBucket[T]`, and heap slots. The wrappers that move inactive payloads into free-list nodes and zero-fill inactive buckets now place those transitions inside narrow `Unsafe.PointerCast` blocks, but the compiler generations do not agree on those exemptions. `build_runtime_object.sh` and the full runtime object therefore remain unverified. The plan should require a first-class uninitialized-storage/validity abstraction with state-guarded reads, rather than treating the current unsafe wrappers as closure.
...
Since the earlier checkpoint below, compiler main advanced through `b81620a5` with mandatory direct-reference initialization checks and then `0bd938a9`. The aggregate follow-up rejects direct and nested by-value struct fields, named tuples, and fixed-array elements containing non-null references; aliases and optional/initialized positive controls are included in the recorded verified slice. The current uncommitted checker extends the same containment attempt to function returns, generic type parameters, and `sview`, and the smoke has corresponding negative cases. That broader source has not been compiled into a current Stage1 product or passed as a combined smoke: the on-disk product is SHA-256 `cc2f7a2247f506f2587c7143b0037413c75c9e26abd8f9ff95ef171bfe705ef9`, and `assert_stage1_fresh.sh` reports it stale against the current semantic diagnostic sources. The check remains a syntax/declaration-oriented containment pass, not CFG initialization or universal invalid-representation analysis required by S04/S10/S15.

The canonical Elisa-core checkout has an uncommitted companion zeroed-validity checker covering resolved references, aggregate fields, generic instances, arrays, tuples, enums, and type parameters. The focused `go test ./src/semantic` and the later AtomicSlot semantic/backend test selection pass. The latest `go test ./...` still fails in integration tests. Its reported failures include generic zeroed placeholders in `InlineVec[T]`, dictionary/set buckets, heap slots, and debug records, plus C-string bridge type mismatches. The runtime has since moved the `atomic` call sites and replaced `result_seed` with reserved result storage; those edits have not yet cleared the full suite. Keep its source intact; it is not ready to commit until placeholder idioms are soundly migrated and the broader tests/runtime smokes pass.

The compiler and Core runtime copies carry the `AtomicSlot[T]` wrapper: `atomic_cell[T]` constructs initialized slots and cells. The first audit found that the rename could bypass exact-name atomic recognition. Follow-up edits include `AtomicSlot` in the self-hosted raw-operation fence, payload check, reference binding classification, and LLVM lowering, with corresponding Core type-safety, raw-operation, and backend handling. The latest Stage1 atomic IR/runtime/order/raw-access checks and focused Core tests are recorded in the continuation checkpoint above. This remains an integration slice, not complete atomic soundness: preserve nominal primitive identity as an open S01/S17 requirement; same-spelled user types/functions must not acquire intrinsic lowering accidentally. The current product is stale only because of subsequent zeroed-generic source edits, while a fresh product/runtime pair remains blocked by the bundled runtime placeholders.

The expanded zeroed-reference smoke includes direct and generic reference returns, `sview` returns and fields, plus aggregate field/tuple/array cases. A fresh Stage1 product at SHA-256 `27f1fdb1072a1794f08f4020e2bcc08cde84819cc486997bb05be42bbc4ac6e6` passed that smoke at O0/O2 before the newest generic-field candidate was added. Current source adds a concrete generic-aggregate rejection and scalar positive control; it is not yet included in verified evidence because the product freshness guard now fails. The compiler-source Stage0 gate is blocked by the runtime representations listed above. The current syntactic pass still does not establish checks for assignment RHS values, call arguments, array/enum constructors, values crossing imported-module boundaries, resolved shadowing, all generic aggregate forms, or every AST statement/value form.
...
1. **Contain confirmed invalid-value access first (S04/S10/S15).** The Stage1 product inspected during this review accepted the following source at O0 and emitted `store ptr null` followed by `store i64 42` through that loaded pointer:
   ...
   **Current containment status:** the original local repro is now rejected before LLVM output. Direct and top-level alias-chain locals, direct globals, and omitted local initialization are covered. The follow-up rejects direct and nested by-value struct fields and fixed-array elements containing non-null references; the nested-field fixture exercises a field type alias. A zeroed optional-reference field and an explicitly initialized reference-field aggregate remain positive controls. `arena_take_free_block` no longer seeds a non-null temporary reference from `zeroed`; its split header reference is constructed from the already checked slot offset inside its existing narrow unsafe helper. This closes those tested representations only. Keep S04/S10/S15 open for generic substitution, imported/qualified and shadowed type identity, enums and all other invalid zero representations, non-declaration expression boundaries, complete AST traversal, uninitialized reads/calls, and path-sensitive initialization.
...
- [x] Reconcile current registered worktrees for this review: compiler main plus the clean detached proof-toolchain checkout at `4f3f7354`, whose commit is already an ancestor of main; Core has only its canonical main checkout. The composite candidate was previously reconciled and removed. The proof checkout has no unique source gains but is retained as a proof-build dependency; verify dependency release and process use before removal. The Core checkout has an uncommitted zeroed-validity candidate; its last focused semantic package run passed, while the full integration suite failed and later runtime/checker edits remain unverified. It is not a registered worktree gain ready to merge. Older dated cleanup records describe their historical state, not today's topology. The pre-existing untracked `.log` and `test/repro/field_ref_shadowed_param.elisa` remain untouched.
