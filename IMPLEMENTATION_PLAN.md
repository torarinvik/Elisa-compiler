...
**Parser metadata follow-up (2026-09-24):** the scanner now excludes identifiers inside a selected-variant brace from `__error_set_family` rows, while retaining each `__error_variant_allow` entry. `parser_ast_smoke.sh` passes 111 structural tests, including an extern composite-set assertion for exactly the `FooError` and `BarError` families and the selected `Bad1`, `Bad3`, and `Bad4` variants. This narrow parser fix is integrated; it does not claim composite backend support. On the fresh candidate product, the fieldless extern statement-catch parity smoke passes and the declaration-only composite signature compiles, but `catch_result_type_smoke.sh` still declines `main@21 (control expression)`, and the minimal and exhaustive extern-subset catch fixtures each segfault (exit 139). Those failures exclude the candidate backend edits from import until repaired and retested. The composite raise/catch ABI, canonical ordering, payload layout, and cross-compiler runtime checks remain open.
