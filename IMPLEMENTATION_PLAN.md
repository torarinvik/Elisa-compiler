...
**Review checkpoint:** compiler main `965d33d6`, following direct C-return adaptation `d017df19`; Core main `c447c2ce0c68d1aacd64fa8c4a1d6deece01f344`. Both tracked working trees were clean when inspected. This review reconciles the implementation history, active ledger, source consumers, and test discovery; it does not rerun or certify the entire safety suite. Earlier dated measurements retain their original compiler/source scope. No S01–S28 package is promoted to complete by this review.
...
| Atomics (S17) | Native lowering, nominal MemoryOrder checks, invalid literal-order rejection, pointer/bool/integer coverage, and threaded CAS tests. | Exact primitive target identity, faithful dynamic-order behavior, all pointer result bindings, race-free safe APIs, or supported-target qualification. |
...
1. **Contain confirmed invalid-value access first (S04/S10/S15).** The Stage1 product inspected during this review accepted the following source at O0 and emitted `store ptr null` followed by `store i64 42` through that loaded pointer:
   ...
   **Current containment status:** the original local repro is now rejected before LLVM output. Direct and top-level alias-chain local annotations, a direct global annotation, and a direct local reference with an omitted initializer are covered, while a zeroed optional reference and a valid initialized reference remain positive controls. `arena_take_free_block` no longer seeds a non-null temporary reference from `zeroed`; its split header reference is constructed from the already checked slot offset inside its existing narrow unsafe helper. This closes those specific representations only. Keep S04/S10/S15 open for aggregate fields, generic/import-qualified resolution, complete declaration/value traversal, uninitialized reads and calls, and path-sensitive initialization.
...
**Verified S04 zeroed non-null-reference containment slice (2026-09-25):** a new mandatory semantic diagnostic rejects `zeroed` for direct and alias-resolved non-null reference local declarations, including references reached through storage qualifiers, parentheses, and refinement wrappers; optional references retain zero/null as their valid absent representation. The check also covers direct global declarations and walks current nested statement bodies. It follows lexically collected type aliases with later declarations shadowing earlier ones and a bounded, fail-closed cycle/depth guard. `test/parity/zeroed_reference_safety_smoke.sh` verifies that direct local, two-link alias, and direct global negative fixtures fail at `-O0`/`-O2`, emit the diagnostic, and leave no LLVM file. Its initialized-reference control, alongside a global zeroed optional reference, compiles and returns 42 with Stage0 and Stage1 at both optimization levels. A fresh Stage1 product (SHA-256 `4cc059fefb84081fc75cae6b10acf3710f537e50655e73a409ed7729823276b8`) passes `assert_stage1_fresh.sh`. `mutable_ref_scope_smoke.sh`, `arena_runtime_lifecycle_smoke.sh`, `arena_alloc_size_overflow_smoke.sh`, and `arena_cache_concurrency_smoke.sh` also pass; the latter allocator checks exercise Stage0/Stage1 and their documented O0/O2 coverage. `bash -n`, `git diff --check`, and freshness checks pass. To remove an existing unsafe placeholder, `arena_take_free_block` now creates its split-header reference directly from the checked block offset through a narrow internal pointer helper; the scoped mutable-reference regression now starts from `null` in an optional reference. This is declaration-level containment, not a complete zero-validity or initialization rule: fields inside aggregates, aliases across imports/qualified module names, generic substitution, all expression/declaration forms, omitted initialization, and path-sensitive reads/calls/returns are not covered by this slice. Stage0/Stage1 agreement is positive control evidence only; no full-gate, sanitizer, other-target, or memory-safety qualification is claimed. Keep S04/S10/S15 open.
