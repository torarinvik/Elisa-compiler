...
- **Current worktree integration and retention:** compiler main contains the committed gains from compiler candidates; proof-pinned compiler checkouts `f7edb529` and `4f3f7354` remain because the proof project and historical audit still depend on them. The proof-pinned Core `a3f3ea3d` worktree also remains active; current Core main `5793517d` is seven commits ahead of that pin. The standalone Core candidate clone, stale `worktrees/elisa-compiler-nw` snapshot, and linked `stage0-region-sview-darray-fix` checkout were audited, their useful regression coverage integrated or confirmed superseded, and the candidate directories/branch removed after no-open-user checks. The linked candidate's only additional source diff was an uncommitted environment-triggered debug print in darray growth-owner selection; it changed no semantics and was not retained. `/private/tmp/elisac-engine-validation-v2` was previously removed after its process/open-file checks. The clean proof checkouts are dependencies, not pending source candidates; retain active pins until the proof migration is validated.
...
- [x] Reconcile and classify registered and standalone worktrees: compiler proof checkouts at `4f3f7354` and `f7edb529`, Core proof pin `a3f3ea3d`, and the previously removed engine-validation checkout are active historical/proof dependencies or confirmed redundant. Audit the standalone Core candidate `worktrees/elisa-core-stage0-or-pattern` commit-by-commit; port the unintegrated region and match fixes, verify parser/nested-container work is already superseded, run semantic/backend/parser package tests, then remove the candidate and stale `worktrees/elisa-compiler-nw` snapshot after checking for open users. Core commits `12e8eaa0` and `60267978` contain the semantic/backend fixes; `d6f9b77a` and `5793517d` preserve the generic-sview darray allocator regression at IR and runtime-test levels. The linked `stage0-region-sview-darray-fix` worktree was also audited, its useful scenario retained, and the checkout/merged branch removed after a no-open-user check. Preserve active proof pins until migration validation. Core semantic/backend full-package tests remain green while the uncached full suite remains red on runtime migration and fixture compatibility. The pre-existing untracked compiler `.log` and `test/repro/field_ref_shadowed_param.elisa` remain untouched.
