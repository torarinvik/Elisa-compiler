...
grep -q "non-exhaustive match over \"E\"; missing variants: E.C" <<< "$out" || fail "missing variant not flagged: $out"
...
grep -q "non-exhaustive match over \"E\"; missing variants: E.C" <<< "$out" || fail "expr-form match not checked: $out"
out=$(printf "${E3}def q(e: E) -> i64:\n    r: i64 = when e:\n        E.A: 1\n        E.B: 2\n    return r\n" | "$RPT")
grep -q "non-exhaustive match over \"E\"; missing variants: E.C" <<< "$out" || fail "expr-form when not checked: $out"
out=$(printf "${E3}def r(e: E) -> i64:\n    return when e:\n        E.A: 1\n        E.B: 2\n" | "$RPT")
grep -q "non-exhaustive match over \"E\"; missing variants: E.C" <<< "$out" || fail "return-position when not checked: $out"
