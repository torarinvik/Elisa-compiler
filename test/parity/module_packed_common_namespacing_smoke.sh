...
FIXTURE="$ROOT/test/repro/module_packed_common_namespacing.elisa"
RUNTIME_OBJ="${ELISA_RUNTIME_OBJ:-$ROOT/build/runtime/elisacore_runtime.o}"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/elisa-packed-common-namespacing.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT INT TERM HUP

[[ -x "$STAGE0" ]] || { echo "module packed common namespacing smoke FAIL: stage0 unavailable" >&2; exit 1; }
[[ -x "$STAGE1" ]] || { echo "module packed common namespacing smoke FAIL: stage1 unavailable" >&2; exit 1; }
[[ -f "$RUNTIME_OBJ" ]] || { echo "module packed common namespacing smoke FAIL: runtime object unavailable" >&2; exit 1; }
...
echo "module packed common namespacing soundness OK: both stages preserve module-local common rows"
