...
if ! clang -o "$ROOT/build/easm_native_driver" "$ROOT/build/easm_native_driver.o" "$ROOT/test/parity/profile_hooks.c" -L"$("$LLVM_CONFIG" --libdir)" -lLLVM -Wl,-rpath,"$("$LLVM_CONFIG" --libdir)"; then
