...
grep -q "^P 0$" <<< "$out" || fail "single-line with had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "single-line with had diagnostics (constant unresolved?): $out"

# 2. Alternatives split across lines (continuation `|`), each with MULTIPLE constants.
out=$(printf 'const enum K of u8:\n    A\n    B\ndef f(k: K) -> i64:\n    return match k:\n        K.A with lo = 1, hi = 2\n        | K.B with lo = 3, hi = 4:\n            return lo + hi\n        _:\n            return 0\n' | "$RPT")
grep -q "^P 0$" <<< "$out" || fail "multi-line multi-decl with had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "multi-line multi-decl with had diagnostics: $out"

# 3. A `with`-less arm (plain or-pattern) is unchanged — no fan-out, still resolves clean.
out=$(printf 'const enum K of u8:\n    A\n    B\ndef f(k: K) -> i64:\n    return match k:\n        K.A | K.B:\n            return 1\n        _:\n            return 0\n' | "$RPT")
grep -q "^P 0$" <<< "$out" || fail "with-less arm regressed (parse): $out"
grep -q "^D 0$" <<< "$out" || fail "with-less arm regressed (diagnostics): $out"
...
for arm in 'K.A with x = 1 | K.B with y = 2' 'K.A with x = 1 | K.B'; do
    out=$(printf 'const enum K of u8:\n    A\n    B\ndef f(k: K) -> i64:\n    return match k:\n        %s:\n            return x\n        _:\n            return 0\n' "$arm" | "$RPT")
    grep -q "^P 0$" <<< "$out" && fail "R1 not raised for mismatched `with` sets [$arm]: $out"
    grep -q "must bind the same constants" <<< "$out" || fail "R1 message missing [$arm]: $out"
done
...
grep -q "^P 0$" <<< "$out" || fail "R1 false-positive on identical name-set (parse): $out"
grep -q "^D 0$" <<< "$out" || fail "R1 false-positive on identical name-set (diagnostics): $out"
