...
grep -q "^P 0$" <<< "$out" || fail "acyclic machine had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "acyclic machine had diagnostics: $out"
...
grep -q "^P 0$" <<< "$out" || fail "cyclic machine had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "cyclic machine had diagnostics: $out"
...
grep -q "^P 0$" <<< "$out" || fail "payload machine had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "payload machine had diagnostics (binding unresolved?): $out"
...
grep -q "^P 0$" <<< "$out" || fail "entry-payload machine had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "entry-payload machine had diagnostics: $out"
...
grep -q "^P 0$" <<< "$out" || fail "declared-out machine had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "declared-out machine had diagnostics: $out"
...
grep -q "^P 0$" <<< "$out" || fail "local typed arrow machine had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "local typed arrow machine had diagnostics: $out"
...
grep -q "^P 0$" <<< "$out" || fail "bad typed transition should remain structurally parseable: $out"
if grep -q "^D 0$" <<< "$out"; then
...
grep -q "^P 0$" <<< "$out" || fail "grouped local state declaration had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "grouped local state declaration had diagnostics: $out"
