#!/usr/bin/env bash
# Stage0/stage1 soundness regression: type aliases are keyed by lexical module owner.
set -euo pipefail
...
stage1_status=$?
set -e
test "$stage0_status" -eq 42
test "$stage1_status" -eq 42

echo "module type alias namespacing soundness OK: both stages preserve module-local aliases"
