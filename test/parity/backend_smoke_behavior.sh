...
# Atomics: match the standard operation signatures, including MemoryOrder. Calls with
# these canonical shapes lower to LLVM atomics instead of the deliberately non-atomic
# field-access bodies. A same-named non-atomic overload still follows ordinary resolution.
run_case atomic_store_load 'struct atomic[T]:\n    value: mutable T\n\nenum MemoryOrder:\n    Relaxed\n    Acquire\n    Release\n    AcqRel\n    SeqCst\n\ndef store(s: mutable atomic[i64]&, v: i64, order: MemoryOrder):\n    _ = order\n    s.value <- v\n\ndef load(s: atomic[i64]&, order: MemoryOrder) -> i64:\n    _ = order\n    return s.value\n\ndef main() -> i64:\n    a: mutable atomic[i64] = atomic[i64]{value: 0}\n    store(&a, 42, MemoryOrder.SeqCst)\n    return load(&a, MemoryOrder.SeqCst)\n'  42
run_case atomic_fetch_add 'struct atomic[T]:\n    value: mutable T\n\nenum MemoryOrder:\n    Relaxed\n    Acquire\n    Release\n    AcqRel\n    SeqCst\n\nextern fetch_add(s: mutable atomic[i64]&, v: i64, order: MemoryOrder) -> i64\nextern load(s: atomic[i64]&, order: MemoryOrder) -> i64\nextern store(s: mutable atomic[i64]&, v: i64, order: MemoryOrder)\n\ndef main() -> i64:\n    a: mutable atomic[i64] = atomic[i64]{value: 0}\n    store(&a, 40, MemoryOrder.SeqCst)\n    fetch_add(&a, 2, MemoryOrder.SeqCst)\n    return load(&a, MemoryOrder.SeqCst)\n'  42
run_case atomic_exchange 'struct atomic[T]:\n    value: mutable T\n\nenum MemoryOrder:\n    Relaxed\n    Acquire\n    Release\n    AcqRel\n    SeqCst\n\ndef exchange(s: mutable atomic[i64]&, v: i64, order: MemoryOrder) -> i64:\n    _ = order\n    old: i64 = s.value\n    s.value <- v\n    return old\n\ndef main() -> i64:\n    a: mutable atomic[i64] = atomic[i64]{value: 42}\n    return exchange(&a, 7, MemoryOrder.SeqCst)\n'  42
run_case non_atomic_fence_overload 'def fence(value: i64) -> i64:\n    return value + 2\n\ndef main() -> i64:\n    return fence(40)\n' 42
...
run_case try_error_set_widen 'error Before:\n    Pad\n\nerror Later:\n    Missing\n\ndef fail() -> i64 error[Later]:\n    raise Later.Missing\n    0\n\ndef relay() -> i64 error[Before, Later]:\n    try fail()\n    return 0\n\ndef main() -> i64:\n    return catch relay():\n        value: value\n        Later.Missing: 42\n        Before.Pad: 7\n' 42
run_case try_error_set_widen_payload 'error Before:\n    Pad\n\nerror Later:\n    Missing(code: i64)\n\ndef fail() -> i64 error[Later]:\n    raise Later.Missing(42)\n    0\n\ndef relay() -> i64 error[Before, Later]:\n    value: i64 = try fail()\n    return value\n\ndef main() -> i64:\n    return catch relay():\n        value: value\n        Later.Missing(code): code\n        Before.Pad: 7\n' 42
run_case catch_enum_context 'const enum State of u8:\n    Ready\n    Down\n\nerror Problem:\n    Failed\n\ndef state_or_fallback(flag: i64) -> State:\n    result: State = catch load_state(flag):\n        value: value\n        Problem.Failed: State.Down\n    result\n\ndef load_state(flag: i64) -> State error[Problem]:\n    raise Problem.Failed if flag < 0\n    return State.Ready\n\ndef main() -> i64:\n    success: State = state_or_fallback(1)\n    failure: State = state_or_fallback(-1)\n    return 42 if success == State.Ready and failure == State.Down else 0\n' 42
...
# Rule R (2026-09-22): the `mutable` in the TYPE is the write capability, and the legacy
# spelling `r: mutable T& = init` keeps it when `init` is a writable reference (`&g` of a
# mutable global is), so `r <- 5` WRITES THROUGH and `g` becomes 5 (stage0 875d75bd
# -emit obj + run = 5). Before rule R stage0 rebound `r` to a temporary and `g` stayed 100;
# that answer is the one this case used to pin.
run_case mutable_ref_local_plain_write_through 'global mutable g: mutable i64 = 100\n\ndef main() -> i64:\n    ref: mutable i64& = &g\n    ref <- 5\n    return g\n'  5

# A helper that only FORWARDS its `mutable darray[u8]&` to a function of ANOTHER module,
# reached through `using`. The region pre-pass resolved every bare call to the enclosing
# module, found no row there or at top level, and left the parameter unmarked: the helper
# took no caller arena, handed the callee its own `auto.region`, and freed the buffer's new
# storage on return. The self-hosted compiler printed rule-R diagnostics whose first bytes
# were zero. Exit 2 = the first byte was lost; stage0 875d75bd answers 42 for all three.
# Bare call, UFCS spelling, and a two-hop forward through a second `using`d module.
run_case region_forward_using_bare 'module Bytes:\n    public:\n        def push_sview(buffer: mutable darray[u8]&, view: sview) -> void can[Memory.Allocate, Abort.Panic]:\n            buffer.extend(view)\n\nusing Bytes\n\nmodule Render:\n    public:\n        def push_typed(buffer: mutable darray[u8]&, elem: sview) -> void:\n            can Memory.Allocate, Abort.Panic:\n                push_sview(buffer, "static ")\n                push_sview(buffer, elem)\n                push_sview(buffer, "& with enough text to force the darray to grow past its first block")\n\n        def render(buffer: mutable darray[u8]&) -> void:\n            can Memory.Allocate, Abort.Panic:\n                buffer.extend("cannot assign ")\n                push_typed(buffer, "u8")\n                buffer.extend(" to cstr")\n\ndef main() -> i64:\n    can Memory.Allocate, Abort.Panic:\n        buffer: mutable darray[u8] = []\n        Render::render(&buffer)\n        zeros: mutable i64 = 0\n        for b in buffer |zeros|:\n            zeros <- zeros + 1 if b == 0\n        return 1 if buffer.count != 98\n        return 2 if buffer[0] != 99\n        return 3 if zeros != 0\n        return 42\n' 42
run_case region_forward_using_ufcs 'module Bytes:\n    public:\n        def push_sview(buffer: mutable darray[u8]&, view: sview) -> void can[Memory.Allocate, Abort.Panic]:\n            buffer.extend(view)\n\nusing Bytes\n\nmodule Render:\n    public:\n        def push_typed(buffer: mutable darray[u8]&, elem: sview) -> void:\n            can Memory.Allocate, Abort.Panic:\n                buffer.push_sview("static ")\n                buffer.push_sview(elem)\n                buffer.push_sview("& with enough text to force the darray to grow past its first block")\n\n        def render(buffer: mutable darray[u8]&) -> void:\n            can Memory.Allocate, Abort.Panic:\n                buffer.extend("cannot assign ")\n                push_typed(buffer, "u8")\n                buffer.extend(" to cstr")\n\ndef main() -> i64:\n    can Memory.Allocate, Abort.Panic:\n        buffer: mutable darray[u8] = []\n        Render::render(&buffer)\n        zeros: mutable i64 = 0\n        for b in buffer |zeros|:\n            zeros <- zeros + 1 if b == 0\n        return 1 if buffer.count != 98\n        return 2 if buffer[0] != 99\n        return 3 if zeros != 0\n        return 42\n' 42
run_case region_forward_using_two_hops 'module Bytes:\n    public:\n        def push_sview(buffer: mutable darray[u8]&, view: sview) -> void can[Memory.Allocate, Abort.Panic]:\n            buffer.extend(view)\n\nusing Bytes\n\nmodule Mid:\n    public:\n        def relay(buffer: mutable darray[u8]&, view: sview) -> void:\n            can Memory.Allocate, Abort.Panic:\n                push_sview(buffer, view)\n\nusing Mid\n\nmodule Render:\n    public:\n        def push_typed(buffer: mutable darray[u8]&, elem: sview) -> void:\n            can Memory.Allocate, Abort.Panic:\n                relay(buffer, "static ")\n                relay(buffer, elem)\n                relay(buffer, "& with enough text to force the darray to grow past its first block")\n\n        def render(buffer: mutable darray[u8]&) -> void:\n            can Memory.Allocate, Abort.Panic:\n                buffer.extend("cannot assign ")\n                push_typed(buffer, "u8")\n                buffer.extend(" to cstr")\n\ndef main() -> i64:\n    can Memory.Allocate, Abort.Panic:\n        buffer: mutable darray[u8] = []\n        Render::render(&buffer)\n        zeros: mutable i64 = 0\n        for b in buffer |zeros|:\n            zeros <- zeros + 1 if b == 0\n        return 1 if buffer.count != 98\n        return 2 if buffer[0] != 99\n        return 3 if zeros != 0\n        return 42\n' 42
