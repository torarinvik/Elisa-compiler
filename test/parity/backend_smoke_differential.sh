...
diff_case enum_array_scoped_loop 'const enum Feature of u8:\n    Input\n    Rendering\n    Physics\n\ndef is_feature(value: Feature, target: Feature) -> bool:\n    return value == target\n\ndef contains_local(required: Feature[3], target: Feature) -> bool:\n    for index in 0..<3 |found: bool = false| -> found:\n        feature: Feature = required[index]\n        break true if feature == target\n\ndef contains_direct(required: Feature[3], target: Feature) -> bool:\n    for index in 0..<3 |found: bool = false| -> found:\n        break true if is_feature(required[index], target)\n\ndef main() -> i64:\n    required: Feature[3] = [Feature.Input, Feature.Rendering, Feature.Physics]\n    if contains_local(required, Feature.Rendering) and contains_direct(required, Feature.Physics):\n        return 42\n    return 0\n'
diff_case enum_array_application_and_match 'const enum Feature of u8:\n    Input\n    Rendering\n    Physics\n\ndef first_feature() -> Feature:\n    return Feature.Input\n\ndef choose_feature(flag: i64) -> Feature:\n    selected: Feature = match flag:\n        1: first_feature()\n        _: Feature.Physics\n    return selected\n\ndef contains_application(required: array[Feature, 3], target: Feature) -> bool:\n    for index in 0..<3 |found: bool = false| -> found:\n        feature: Feature = required[index]\n        break true if feature == target\n\ndef main() -> i64:\n    required: array[Feature, 3] = [Feature.Input, Feature.Rendering, Feature.Physics]\n    if contains_application(required, Feature.Physics) and choose_feature(0) == Feature.Physics:\n        return 42\n    return 0\n'
# A `return start` machine whose arms yield a const-enum value, and tuple returns whose
# nested sub-tuples carry a const-enum field (both declined before the tuple retry round).
diff_case machine_return_enum 'const enum Result of u8:\n    First\n    Second\n\ndef classify(x: i64) -> Result:\n    state Start\n    state Pos\n    state End\n    return start Start:\n        Start:\n            -> Pos if x > 0\n            -> End\n        Pos:\n            => Result.Second\n        End:\n            => Result.First\n\ndef main() -> i64:\n    return 42 if classify(5) == Result.Second and classify(-1) == Result.First else 0\n'
diff_case nested_tuple_enum_returns 'const enum Result of u8:\n    First\n    Second\n\ndef nested() -> (ok: bool, place: (kind: i64, base: i64), op: i64):\n    return (true, (1, 2), 3)\n\ndef nested_enum() -> (ok: bool, place: (kind: i64, base: i64, field: i64), op: Result):\n    return (true, (4, 5, 6), Result.Second)\n\ndef nested_inner_enum() -> (ok: bool, place: (kind: i64, op: Result), value: i64):\n    return (true, (7, Result.Second), 8)\n\ndef main() -> i64:\n    a: (ok: bool, place: (kind: i64, base: i64), op: i64) = nested()\n    b: (ok: bool, place: (kind: i64, base: i64, field: i64), op: Result) = nested_enum()\n    c: (ok: bool, place: (kind: i64, op: Result), value: i64) = nested_inner_enum()\n    total: i64 = a.place.kind + a.place.base + a.op + b.place.field + c.place.kind + c.value\n    return total + 13 if a.ok and b.ok and c.ok and b.op == Result.Second and c.place.op == Result.Second else 0\n'
