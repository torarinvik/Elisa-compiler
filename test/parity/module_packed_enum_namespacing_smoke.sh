...
[[ -x "$STAGE0" ]] || { echo "module packed enum namespacing smoke FAIL: stage0 unavailable" >&2; exit 1; }
[[ -x "$STAGE1" ]] || { echo "module packed enum namespacing smoke FAIL: stage1 unavailable" >&2; exit 1; }

"$STAGE0" -emit obj -O0 -o "$WORK/stage0.o" "$FIXTURE" >/dev/null 2>&1
[[ -f "$RUNTIME_OBJ" ]] || { echo "module packed enum namespacing smoke FAIL: runtime object unavailable" >&2; exit 1; }
