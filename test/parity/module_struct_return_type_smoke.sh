#!/usr/bin/env bash
# Stage0/stage1 regression: a module member's bare RETURN TYPE resolves in its
# own module.
#
# The signature was resolved with no owner set, so a function was declared as
# returning ANOTHER module's same-named struct -- fewer fields, so the call
# stored too few bytes and the tail read back 0.
#
# It MUST check the exit code. The program compiled clean on both stages and
# every field inside the shorter struct was right; only the tail was wrong.
set -euo pipefail

ROOT="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)"
STAGE0="${ELISACORE_BIN:-$ROOT/../../Go projects/structpy-tree/compiler/bin/elisac}"
STAGE1="$ROOT/scripts/elisac_stage1.sh"
FIXTURE="$ROOT/test/repro/module_struct_return_type.elisa"
TMP_DIR="$(mktemp -d)"
...
set -e
test "$stage0_status" -eq 80
test "$stage1_status" -eq 80

echo "module struct return-type parity OK: stage0 and stage1 return 80"
