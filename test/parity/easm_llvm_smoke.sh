...
if ! clang -o "$ROOT/build/easm_llvm_smoke" "$ROOT/build/easm_llvm_smoke.o" "$ROOT/test/parity/profile_hooks.c" -L"$("$LLVM_CONFIG" --libdir)" -lLLVM -Wl,-rpath,"$("$LLVM_CONFIG" --libdir)"; then
