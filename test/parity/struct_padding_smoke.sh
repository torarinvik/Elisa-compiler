...
grep -Fq 'struct "Padded" has 8 bytes of avoidable padding' <<< "$out" || { echo "missing padding warning: $out" >&2; exit 1; }
grep -Fq 'ordering fields as "value", "small", "flag"' <<< "$out" || { echo "wrong reorder suggestion: $out" >&2; exit 1; }

dense=$(printf 'struct Dense:\n    value: i64\n    small: i16\n    flag: bool\n' | "$RPT")
if grep -Fq 'avoidable padding' <<< "$dense"; then
