...
grep -q "undefined identifier \"i64\"" <<< "$out" && fail "cast type-arg flagged undefined: $out"

# 2. Same for a local receiver.
out=$(printf 'def f() -> i64:\n    a: i32 = 5\n    return a.cast[i64]\n' | "$RPT")
grep -q "undefined identifier" <<< "$out" && fail "local cast type-arg flagged undefined: $out"

# 3. A genuinely-undefined VALUE reference is still caught (guard is scoped, not blanket).
out=$(printf 'def f(x: i32) -> i64:\n    return bogusvalue\n' | "$RPT")
grep -q "undefined identifier \"bogusvalue\"" <<< "$out" || fail "real undefined identifier no longer caught: $out"
...
grep -q "undefined identifier \"missingidx\"" <<< "$out" || fail "ordinary index expr no longer resolved: $out"
