#!/usr/bin/env bash
# COMP-010: field-type lookup must respect module scope, nested module paths,
# and imported visibility while remaining conservative for qualified aliases.
set -euo pipefail
...
FIXTURE="$ROOT/test/repro/module_struct_field_scope_matrix.elisa"
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT

for tool in "$STAGE0" "$STAGE1_BIN"; do
    if [[ ! -x "$tool" ]]; then
        echo "module struct field scope matrix: missing compiler: $tool" >&2
        exit 2
    fi
done

# This source has no runtime include, so the bare object link is intentional.
...
if [[ "$stage0_rc" -ne 0 || "$stage1_rc" -ne 0 ]]; then
    echo "module struct field scope matrix FAILED: stage0=$stage0_rc stage1=$stage1_rc expected=0" >&2
    exit 1
fi

echo "module struct field scope matrix OK"
