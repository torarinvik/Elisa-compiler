...
[ -x "$ELISACORE_BIN" ] || { echo "inlinevec_real_smoke FAIL: no elisac" >&2; exit 1; }
[ -x "$LLVM_CONFIG" ] || { echo "inlinevec_real_smoke FAIL: no llvm-config" >&2; exit 1; }
[ -f "$STD/collections.elisa" ] || { echo "inlinevec_real_smoke FAIL: no collections.elisa at $STD" >&2; exit 1; }
