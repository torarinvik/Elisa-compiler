...
[ -x "$ELISACORE_BIN" ] || { echo "deque_real_smoke FAIL: no elisac" >&2; exit 1; }
[ -x "$LLVM_CONFIG" ] || { echo "deque_real_smoke FAIL: no llvm-config" >&2; exit 1; }
[ -f "$STD/deque.elisa" ] || { echo "deque_real_smoke FAIL: no deque std at $STD" >&2; exit 1; }
