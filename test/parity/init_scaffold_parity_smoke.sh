#!/usr/bin/env bash
# `elisac init` SCAFFOLD PARITY: same directory tree, same file bytes, same exit status.
#
...
check() {
    ...
    ( cd "$WORK/s0" && "$ELISACORE_BIN" init "$@" >/dev/null 2>&1 ); local rc0=$?
    ( cd "$WORK/s1" && "$STAGE1_BIN"     init "$@" >/dev/null 2>&1 ); local rc1=$?
    if [[ "$rc0" != "$rc1" ]]; then
        differ=$((differ + 1))
