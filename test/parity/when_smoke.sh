...
grep -q "^P 0$" <<< "$out" || fail "multi-column table had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "multi-column table had diagnostics: $out"

# 2. Integer ranges + `_` default, expression form bound to a local.
out=$(printf 'def classify(n: i64) -> i64:\n    x: i64 = when n:\n        0 -> 100\n        1..<10 -> 200\n        10..=99 -> 300\n        _ -> 400\n    return x\n' | "$RPT")
grep -q "^P 0$" <<< "$out" || fail "range table had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "range table had diagnostics: $out"
...
grep -q "^P 0$" <<< "$out" || fail "statement form had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "statement form had diagnostics: $out"
...
grep -q "^P 0$" <<< "$out" || fail "when-as-identifier had parse errors: $out"
grep -q "^D 0$" <<< "$out" || fail "when-as-identifier had diagnostics: $out"
