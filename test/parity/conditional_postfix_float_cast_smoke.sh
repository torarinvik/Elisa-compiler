...
nm -gU "$TMP_DIR/stage0.o" | grep  '_conditional_rate$' >/dev/null
nm -gU "$TMP_DIR/stage1.o" | grep  '_conditional_rate$' >/dev/null
if nm -u "$TMP_DIR/stage1.o" | grep  '_conditional_rate$' >/dev/null; then
