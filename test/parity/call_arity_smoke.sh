...
echo "$out" | grep -q "argument count mismatch\|wrong number of arguments" || fail "missing-required-arg not flagged: $out"

# 2. Too many arguments: `f(1, 2)` with `def f(n: i64)` MUST be flagged.
out=$(printf 'def f(n: i64) -> i64:\n    return n\n\ndef g() -> i64:\n    return f(1, 2)\n' | "$RPT")
echo "$out" | grep -q "argument count mismatch\|wrong number of arguments" || fail "extra-arg not flagged: $out"

# 3. Correct argument count must NOT be flagged.
out=$(printf 'def f(n: i64) -> i64:\n    return n\n\ndef g() -> i64:\n    return f(1)\n' | "$RPT")
echo "$out" | grep -q "argument count mismatch\|wrong number of arguments" && fail "false positive on correct arity: $out"

# 4. Overloaded function names with different arities are ambiguous (no error expected).
out=$(printf 'def f(n: i64) -> i64:\n    return n\n\ndef f(s: sview) -> i64:\n    return 0\n\ndef g() -> i64:\n    return f()\n' | "$RPT")
echo "$out" | grep -q "argument count mismatch\|wrong number of arguments" && fail "false positive on overload ambiguity: $out"

# 5. 0 FP across frontend + stdlib.
t=0
while IFS= read -r f; do
  c=$("$RPT" < "$f" 2>/dev/null | grep -cE "argument count mismatch|wrong number of arguments" || true)
  t=$((t + c))
...
echo "call-arity smoke OK: flags mismatched argument counts, silent on correct/overloaded, 0 FP across frontend+stdlib"
